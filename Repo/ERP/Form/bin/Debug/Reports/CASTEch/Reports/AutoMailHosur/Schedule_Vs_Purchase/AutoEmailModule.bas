...
Dim mSubject As String
    ...
    mSubject = " Schedule Vs Purchase Report AS ON DATE : " & mDateTime
...
Dim mCompanyCode As Long
    ...
    mHeadingline = 1
    mDate = Format(PubCurrDate, "DD/MM/YYYY")       ''DateAdd("d", -1, Format(PubCurrDate, "DD/MM/YYYY"))
    mDays = Day(mDate)
    ...
    exlobj.ActiveSheet.Cells(6, 41).Value = "Stock Qty as on " & mDate
    mHeadingline = 7
    ...
    For I = 1 To 500
        With exlobj.ActiveSheet
            ...
            mPartyCode = Trim(.Cells(mHeadingline, 1).Value)
            mItemCode = Trim(.Cells(mHeadingline, 5).Value)
'            mCompanyCode = Val(.Cells(mHeadingline, 43).Value)
            
            If mPartyCode = "" Then GoTo NextRec
            If mItemCode = "" Then GoTo NextRec
            
            If ValidateWithMasterTable(mPartyCode, "SUPP_CUST_CODE", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", pPubDbcn, MasterNo, , "COMPANY_CODE=" & mCompanyCode & "") = False Then
                GoTo NextRec
            End If
            ...
            If GetScheduleDetail(mCompanyCode, mItemCode, mPartyCode, mDate, mFYEAR, pPubDbcn, mScheduleNo, mAmendNo, mScheduleQty) = False Then GoTo ErrPart
            If GetTillDateScheduleDetail(mCompanyCode, mItemCode, mPartyCode, mDate, pPubDbcn, mTillDateScheduleQty) = False Then GoTo ErrPart
            ...
            .Cells(mHeadingline, 3).Value = mScheduleNo
            .Cells(mHeadingline, 4).Value = mAmendNo
            .Cells(mHeadingline, 39).Value = mTillDateScheduleQty
            .Cells(mHeadingline, 40).Value = mScheduleQty
            .Cells(mHeadingline, 41).Value = mStock
            
            For J = 1 To 31
                If J <= mLastDateofMonth Then
                    mMRRDate = J & "/" & Format(mDate, "MM/YYYY")
                    mMRRDate = Format(mMRRDate, "DD/MM/YYYY")
                    If CDate(mMRRDate) <= CDate(mDate) Then
                        mMRRQty = GetMRRDetail(mCompanyCode, mItemCode, mPartyCode, mMRRDate, mFYEAR, pPubDbcn)
                    Else
                        mMRRQty = 0
                    End If
                Else
                    mMRRQty = 0
                End If
                .Cells(mHeadingline, J + 6).Value = mMRRQty
            Next
NextRec:
