...
Friend Class frmFixAssetsRegNew
    ...
    Private Const ColCustomDuty As Short = 11
    Private Const ColInstCharges As Short = 12
    Private Const ColTotalCost As Short = 13
    Private Const ColTotalCost1 As Short = 14
    Private Const ColOPBal As Short = 15
    Private Const ColSALEVALUE1 As Short = 16
    Private Const ColDEP1 As Short = 17
    Private Const ColWDV1 As Short = 18
    Private Const ColSALEVALUE2 As Short = 19
    Private Const ColDEP2 As Short = 20
    Private Const ColWDV2 As Short = 21
    Private Const ColSALEVALUE3 As Short = 22
    Private Const ColDEP3 As Short = 23
    Private Const ColWDV3 As Short = 24
    Private Const ColPhyVari As Short = 25
    Private Const ColLoc As Short = 26
    Private Const ColTrnName As Short = 27
    Private Const ColBookType As Short = 28
    Private Const ColBookSubType As Short = 29
...
ErrPart:
    ...
    End Sub
    Private Sub frmFixAssetsRegNew_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        If FormActive = True Then Exit Sub

        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor

        Me.Text = "Fixed Assets Register (Company Act)"
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        FormActive = True
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmFixAssetsRegNew_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Dim MainClass_Renamed As Object
...
BSLError:
    ...
    End Sub
    Private Sub frmFixAssetsRegNew_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        Dim MainClass_Renamed As Object
        On Error GoTo ErrPart
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmFixAssetsRegNew_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Dispose()
        Me.Close()
...
EventExitSub:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For cntCol = ColBillAmount To ColWDV2
                .Col = cntCol
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        SqlStr = " SELECT  '', PV_NO, TO_CHAR(PV_DATE,'DD/MM/YYYY'),  " & vbCrLf & " MRR_NO, MRR_DATE, " & vbCrLf & " BILL_NO, BILL_DATE, " & vbCrLf & " ITEM_DESC, SUPP_CUST_NAME," & vbCrLf & " ITEM_VALUE, CD_AMOUNT, OTH_AMOUNT, " & vbCrLf & " 0, 0," & vbCrLf & " 0, 0, 0," & vbCrLf & " 0, 0, 0, " & vbCrLf & " 0, 0, 0, " & vbCrLf & " PHY_VARIFICATION, LOCATION, REMARKS, " & vbCrLf & " '','',0 "

        SqlStr = SqlStr & vbCrLf & " FROM AST_ASSET_TRN"
...
ERR1:
    ...
    Private Sub CalcSprdTotal()
        ...
        With SprdMain
            ...
            Call MainClass.AddBlankfpSprdRow(SprdMain, ColBillAmount)
            .Col = ColPartyName
            .Row = .MaxRows
            .Text = "GRAND TOTAL :"
...
FillErr2:
    ...
    Private Sub FillHeading()
        ...
        With SprdMain
            ...
            mDate = CStr(System.DateTime.FromOADate(DateAdd(Microsoft.VisualBasic.DateInterval.Month, 6, CDate(VB6.Format(RsCompany.Fields("START_DATE").Value, "DD/MM/YYYY"))).ToOADate - 1))

            .Col = ColTotalCost
            .Text = "Total Cost " & VB6.Format(RsCompany.Fields("START_DATE").Value, "DD/MM/YYYY") & " - " & VB6.Format(mDate, "DD/MM/YYYY")

            mDate = CStr(DateAdd(Microsoft.VisualBasic.DateInterval.Month, 6, CDate(VB6.Format(RsCompany.Fields("START_DATE").Value, "DD/MM/YYYY"))))
            .Col = ColTotalCost1
            .Text = "Total Cost " & VB6.Format(RsCompany.Fields("START_DATE").Value, "DD/MM/YYYY") & " - " & VB6.Format(RsCompany.Fields("END_DATE").Value, "DD/MM/YYYY")
            ...
            .Col = ColDEP1
            .Text = "DEP. @33.33% p.a." & Year(RsCompany.Fields("START_DATE").Value) & "-" & VB6.Format(RsCompany.Fields("END_DATE").Value, "YY")

            .Col = ColWDV1
            .Text = "W.D.V. as on " & VB6.Format(RsCompany.Fields("END_DATE").Value, "DD.MM.YYYY")
            ...
            .Col = ColDEP3
            .Text = "DEP. @25% p.a." & Year(RsCompany.Fields("START_DATE").Value) & "-" & VB6.Format(RsCompany.Fields("END_DATE").Value, "YY")
