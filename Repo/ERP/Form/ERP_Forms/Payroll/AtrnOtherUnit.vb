...
Friend Class frmAtrnOtherUnit										
    ...
    Private Function CheckExpHead(ByRef mAcctName As String) As Boolean
        ...
        CheckExpHead = False
        SqlStr = "Select BSGROUP.BSGROUP_ACCTTYPE " & vbCrLf & " FROM FIN_SUPP_CUST_MST,FIN_GROUP_MST ACMGROUP, " & vbCrLf & " FIN_BSGROUP_MST BSGROUP WHERE " & vbCrLf & " FIN_SUPP_CUST_MST.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FIN_SUPP_CUST_MST.COMPANY_CODE=ACMGROUP.COMPANY_CODE" & vbCrLf & " AND BSGROUP.COMPANY_CODE=FIN_SUPP_CUST_MST.COMPANY_CODE" & vbCrLf & " AND FIN_SUPP_CUST_MST.GROUPCODE=GROUP_Code " & vbCrLf & " AND GROUP_BSCodeDr=BSGROUP_Code " & vbCrLf & " AND BSGROUP_ACCTTYPE IN (" & ConIncome & "," & ConExpenses & ")" & vbCrLf & " AND FIN_SUPP_CUST_MST.SUPP_CUST_NAME = '" & MainClass.AllowSingleQuote(Trim(mAcctName)) & "'"
...
ErrPart:
    ...
    Private Sub ChkSTDS_CheckStateChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ChkSTDS.CheckStateChanged
        ...
        If ChkSTDS.CheckState = System.Windows.Forms.CheckState.Checked Then
            ...
            If Val(txtSTDSRate.Text) = 0 Then
                SqlStr = "SELECT STDS_PER FROM FIN_SUPP_CUST_MST " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND SUPP_CUST_NAME='" & MainClass.AllowSingleQuote(mAccountName) & "'"
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
                If RsTemp.EOF = False Then
    ...
    Private Sub chkTDS_CheckStateChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles chkTDS.CheckStateChanged
        ...
        If chkTDS.CheckState = System.Windows.Forms.CheckState.Checked Then
            ...
            If Val(txtJVTDSRate.Text) = 0 Then
                SqlStr = "SELECT TDS_PER FROM FIN_SUPP_CUST_MST " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND SUPP_CUST_NAME='" & MainClass.AllowSingleQuote(mAccountName) & "'"
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
                If RsTemp.EOF = False Then
    ...
    Private Function CheckPendingPDC() As Boolean
        ...
        End If
        If MainClass.ValidateWithMasterTable((txtPartyName.Text), "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND SUPP_CUST_TYPE='2'") = True Then
            xAccountCode = IIf(IsDBNull(MasterNo), -1, MasterNo)
        Else
            xAccountCode = CStr(-1)
        End If
        SqlStr = "SELECT 'VNO : ' || FIN_VOUCHER_HDR.VNO || ':' || 'CHQ NO : ' || CHEQUENO AS VNO FROM FIN_VOUCHER_HDR,FIN_VOUCHER_DET" & vbCrLf & " WHERE FIN_VOUCHER_HDR.MKEY=FIN_VOUCHER_DET.MKEY " & vbCrLf & " AND COMPANY_CODE =" & Val(lblCompanyCode.Text) & "" & " AND CHQDATE<=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " AND BOOKTYPE='" & xBookType & "'" & vbCrLf & " AND BOOKSUBTYPE='" & xBookSubType & "' AND BOOKCODE='" & xAccountCode & "' AND CANCELLED='N'"
...
ERR1:
    ...
    Private Sub cmdAuthorised_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdAuthorised.Click
        ...
        SqlStr = SqlStr & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND MKEY='" & RsTRNMain.Fields("mKey").Value & "'           "
        PubDBCn.Execute(SqlStr)
        PubDBCn.CommitTrans()
...
ERR1:
    ...
    Private Sub CmdDelete_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDelete.Click
        ...
        If lblReversalMade.Text = "Y" Then
            If MainClass.ValidateWithMasterTable(lblReversalMkey.Text, "REVERSAL_MKEY", "VNO", "FIN_VOUCHER_HDR", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If lblReversalVoucher.Text = "Y" Then
            mReversalVoucher = ""
            If MainClass.ValidateWithMasterTable(lblReversalMkey.Text, "MKEY", "VNO", "FIN_VOUCHER_HDR", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mReversalVoucher = MasterNo
            End If
        ...
        SqlStr = "DELETE FROM FIN_BILLDETAILS_TRN WHERE Mkey='" & RsTRNMain.Fields("mKey").Value & "' "
        PubDBCn.Execute(SqlStr)
        ...
        PubDBCn.Execute(SqlStr)
        SqlStr = "DELETE FROM FIN_POSTED_TRN  WHERE Mkey='" & RsTRNMain.Fields("mKey").Value & "'" & vbCrLf & " AND BookType='" & VB.Left(lblBookType.Text, 1) & "'" & vbCrLf & " AND BooksubType='" & VB.Right(lblBookType.Text, 1) & "' "
        PubDBCn.Execute(SqlStr)
        SqlStr = "DELETE FROM TDS_TRN WHERE Mkey='" & RsTRNMain.Fields("mKey").Value & "' AND BOOKCODE=-1 "
        PubDBCn.Execute(SqlStr)
        SqlStr = " DELETE FROM FIN_SalVoucher_TRN " & vbCrLf & " WHERE " & vbCrLf & " COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND mKey='" & RsTRNMain.Fields("mKey").Value & "'"
        ...
        If chkSuppBill.CheckState = System.Windows.Forms.CheckState.Checked Then
            ...
            If Val(txtModvatNo.Text) <> 0 Then
                SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND MODVATNO=" & txtModvatNo.Text & " AND ISCAPITAL='" & mIsCapital & "'  AND VNO='-1'"
                PubDBCn.Execute(SqlStr)
            End If
            '        If Val(txtServiceTaxNo.Text) <> 0 Then										
            '            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''" & vbCrLf _										
            ''                    & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf _										
            ''                    & " AND FYEAR=" & RsCompany.fields("FYEAR").value & "" & vbCrLf _										
            ''                    & " AND SERVNO=" & txtServiceTaxNo.Text & "  AND VNO='-1'"										
            '            PubDBCn.Execute SqlStr										
            '        End If										
            If Val(txtSTRefundNo.Text) <> 0 Then
                SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND STCLAIMNO=" & txtSTRefundNo.Text & "  AND VNO='-1'"
                PubDBCn.Execute(SqlStr)
            End If
        End If
        SqlStr = "UPDATE FIN_SUPP_PURCHASE_HDR SET ISFINALPOST='N',JVNO='-1',JVMKEY='-1'" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND JVMKEY='" & RsTRNMain.Fields("mKey").Value & "'"
        PubDBCn.Execute(SqlStr)
        SqlStr = "UPDATE FIN_SUPP_SALE_HDR SET ISFINALPOST='N',JVNO='-1',JVMKEY='-1'" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND JVMKEY='" & RsTRNMain.Fields("mKey").Value & "'"
        PubDBCn.Execute(SqlStr)
        With SprdMain
            For ii = 1 To .MaxRows
                .Row = ii
...
DelErrPart:
    ...
    Private Sub ReportOnTrnVoucher(ByRef Mode As Crystal.DestinationConstants)
        ...
        Else
            MainClass.ValidateWithMasterTable(Me.txtPartyName.Text, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "")
            mBookCode = MasterNo
        End If
        mSubTitle = ""
        mMultiLine = False
        ...
        If frmPrintVoucher.OptReceiptExcel.Checked = True Then
            If IsDate(TxtVDate.Text) Then
                MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "")
        ...
        ElseIf frmPrintVoucher.OptVoucher.Checked = True Then
            ...
            If lblBookType.Text = ConJournal Then
                ...
                If MainClass.ValidateWithMasterTable(CurMKey, "BANKVOUCHERMKEY", "BANKVOUCHERMKEY", "FIN_LCDISC_HDR", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mLCPayment = True
                End If
            End If
        ...
        ElseIf frmPrintVoucher.optRTGSLetter.Checked = True Then
            MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "")
            mAccountCode = MasterNo
            SqlStr = "SELECT * FROM FIN_SUPP_CUST_MST CMST WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND SUPP_CUST_CODE='" & MainClass.AllowSingleQuote(mAccountCode) & "'"
        ...
        If Mode = Crystal.DestinationConstants.crptToPrinter Then
            Select Case lblBookType.Text
                Case ConCashReceipt, ConCashPayment, ConBankReceipt, ConBankPayment
                    SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & ""
            ...
            For cntRow = 1 To .MaxRows - 1
                ...
                If mAccountName <> "" Then
                    If GetAccountBalancingMethod(mAccountName, False) = "D" Then
                        If IsDate(TxtVDate.Text) Then
                            If MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                                mAccountCode = MasterNo
                                pOpBal = GetOpeningBal(mAccountCode, (TxtVDate.Text))
                            Else
                                mAccountCode = "-1"
...
ErrPart:
    ...
    Private Function SelectQryForDNCNVoucher(ByRef mSqlStr As String) As String
        ...
        ''WHERE CLAUSE...										
        mSqlStr = mSqlStr & vbCrLf & " WHERE " & vbCrLf & " IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND " & vbCrLf & " IH.BOOKTYPE='" & VB.Left(ConDebitNote, 1) & "'" & vbCrLf & " AND IH.BOOKSUBTYPE='" & VB.Right(ConDebitNote, 1) & "'" & vbCrLf & " AND IH.APPROVED='Y'" & vbCrLf & " AND IH.MKEY=ID.MKEY(+)" & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE"
    ...
    Private Function ShowReport(ByRef mSqlStr As String, ByRef mMode As Crystal.DestinationConstants, ByRef mTitle As String, ByRef mSubTitle As String, ByRef mRptFileName As String, ByRef mNarration As String, ByRef mBankName As String, ByRef mAccountName As String, ByRef pPartyOpBal As String, ByRef pLastPaymentAmount As String, ByRef mChqNo As String, ByRef mParticularAmount As Double, Optional ByRef mLCPayment As Boolean = False) As Boolean
        ...
        If frmPrintVoucher.optRTGSLetter.Checked = True Then
            ...
            mBankAccountNo = ""
            If MainClass.ValidateWithMasterTable(Trim(txtPartyName.Text), "SUPP_CUST_NAME", "CUST_BANK_ACCT_NO", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mBankAccountNo = MasterNo
            End If
            If MainClass.ValidateWithMasterTable(Trim(txtPartyName.Text), "SUPP_CUST_NAME", "CUST_BANK_BANK", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mOurBankName = MasterNo
            End If
            If MainClass.ValidateWithMasterTable(Trim(txtPartyName.Text), "SUPP_CUST_NAME", "BANK_BRANCH_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            ...
            If MainClass.ValidateWithMasterTable(Trim(mAccountName), "SUPP_CUST_NAME", "CUST_BANK_ACCT_NO", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mPartyBankAcctNo = MasterNo
            End If
            If MainClass.ValidateWithMasterTable(Trim(mAccountName), "SUPP_CUST_NAME", "CUST_BANK_BANK", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mPartyBankName = MasterNo
            End If
            If MainClass.ValidateWithMasterTable(Trim(mAccountName), "SUPP_CUST_NAME", "BANK_BRANCH_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ErrPart:
    ...
    Private Function SelectQryForVoucher(ByRef mSqlStr As String, ByRef pMKey As String, ByRef mVNo As String, ByRef mVDate As Date, ByRef mBookType As String, ByRef mBookCode As String) As String
        ...
        Else
            mSqlStr = mSqlStr & vbCrLf & " AND TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
        End If
...
ErrPart:
    ...
    Private Function InsertTempTable(ByRef mVNo As String, ByRef mVDate As Date, ByRef mBookType As String, ByRef mAccountCode As String) As Boolean
        ...
        If RsCompany.Fields("AC_PR_AUTO_JV").Value = "Y" Then
        Else
            SqlStr = SqlStr & vbCrLf & " AND COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
        End If
    ...
    Private Function SelectQryForItem(ByRef mSqlStr As String, ByRef mVNo As String, ByRef mVDate As Date, ByRef mBookType As String, ByRef mBookCode As String) As String
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                ...
                If MainClass.ValidateWithMasterTable(pSupplierName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    pSupplierCode = MasterNo
                Else
                    pSupplierCode = ""
                End If
                If pSupplierCode <> "" Then
                    xSqlStr = " SELECT BILLNO, BILLDATE FROM  FIN_POSTED_TRN TRN" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TRN.VNO='" & mVNo & "'" & vbCrLf & " AND TRN.BookType='" & Mid(mBookType, 1, 1) & "'" & vbCrLf & " AND TRN.BookSubType='" & Mid(mBookType, 2, 1) & "'" & vbCrLf & " AND TRN.BOOKCODE='" & mBookCode & "'" & vbCrLf & " AND TRN.AccountCode='" & pSupplierCode & "'" & vbCrLf & " AND TRN.VDate=TO_DATE('" & VB6.Format(mVDate, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
                    ...
                    If RsTemp.EOF = False Then
                        Do While RsTemp.EOF = False
                            ...
                            End If
                            pSqlStr = pSqlStr & vbCrLf & " WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                ...
                If pSupplierCode <> "" Then
                    ...
                    pSqlStr = pSqlStr & vbCrLf & " SELECT BILLNO||BILLDATE FROM  FIN_POSTED_TRN TRN" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & ""
                    pSqlStr = pSqlStr & vbCrLf & " AND TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TRN.VNO='" & mVNo & "'" & vbCrLf & " AND TRN.BookType='" & Mid(mBookType, 1, 1) & "'" & vbCrLf & " AND TRN.BookSubType='" & Mid(mBookType, 2, 1) & "'" & vbCrLf & " AND TRN.BOOKCODE='" & mBookCode & "'" & vbCrLf & " AND TRN.AccountCode='" & pSupplierCode & "'" & vbCrLf & " AND TRN.VDate=TO_DATE('" & VB6.Format(mVDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'))"
                    PubDBCn.Execute(pSqlStr)
                End If
            Next
        End With
...
ErrPart:
    ...
    Private Function SelectQryForItemOld(ByRef mSqlStr As String, ByRef mVNo As String, ByRef mVDate As Date, ByRef mBookType As String, ByRef mBookCode As String) As String
        ...
        pSqlStr = pSqlStr & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND " & vbCrLf & " TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND " & vbCrLf & " TRN.COMPANY_CODE=PURCHMAIN.COMPANY_CODE AND " & vbCrLf & " TRN.FYEAR=PURCHMAIN.FYEAR AND " & vbCrLf & " TRN.AccountCode=PURCHMAIN.SUPP_CUST_CODE AND " & vbCrLf & " TRN.AccountCode =ACM.SUPP_CUST_CODE AND TRN.Company_Code =ACM.Company_Code AND " & vbCrLf & " TRN.BILLNO=PURCHMAIN.BILLNO AND " & vbCrLf & " PURCHMAIN.MKey=PURCHDETAIL.MKey AND"
...
ErrPart:
    ...
    Private Sub CmdSave_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSave.Click
        ...
        MsgInformation("This is Other Unit Voucher, Cann't be Save in This Unit.")
        Exit Sub
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            If lblBookType.Text = ConBankPayment Or lblBookType.Text = ConBankReceipt Or lblBookType.Text = ConPDCPayment Or lblBookType.Text = ConPDCReceipt Or lblBookType.Text = ConContra Then
                .set_ColWidth(ColAccountName, 27)
            Else
        ...
        MainClass.ProtectCell(SprdMain, 1, SprdMain.MaxRows, ColSaleBillPrefix, ColSaleBillDate)
        Exit Sub
ErrPart:
        'Resume										
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
...
ErrPart:
    ...
    Private Sub ViewGrid()
        ...
        MainClass.ButtonStatus(Me, XRIGHT, RsTRNMain, ADDMode, MODIFYMode, cmdAdd, cmdModify, cmdClose, cmdSave, cmdDelete, cmdSavePrint, cmdSavePrint, cmdPrint, CmdPreview, cmdSavePrint, CmdView, True)
    End Sub
    Private Sub frmAtrnOtherUnit_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Dim KeyCode As Short = eventArgs.KeyCode
        Dim Shift As Short = eventArgs.KeyData \ &H10000
        MainClass.DoFunctionKey(Me, KeyCode)
    End Sub
    Private Sub frmAtrnOtherUnit_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        ...
        End If
    End Sub
    Private Sub frmAtrnOtherUnit_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        On Error GoTo ERR1
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor
        ...
        Call frmAtrnOtherUnit_Activated(eventSender, eventArgs)
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
        '    Resume										
    End Sub
    Public Sub frmAtrnOtherUnit_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ErrPart
        Dim SqlStr As String = ""
        SprdMain.Refresh()
...
ERR1:
    ...
    Private Sub FormatSprdView()
        ...
        End With
    End Sub
    Private Sub frmAtrnOtherUnit_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        On Error Resume Next
        '    If PvtDBCn.State = adStateOpen Then										
        '        'PvtDBCn.Close										
        '        'Set PvtDBCn = Nothing										
...
ERR1:
    ...
    Private Sub NameSearch(ByRef Col As Integer, ByRef Row As Integer)
        ...
        SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
        Select Case Col
            Case ColAccountName
            ...
            Case ColSAC
                SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'"
        ...
        ElseIf Col = ColCC Then
            ...
            SqlStr = " SELECT IH.CC_DESC,IH.CC_CODE, ID.DEPT_CODE " & vbCrLf & " FROM FIN_CCENTER_HDR IH, FIN_CCENTER_DET ID " & vbCrLf & " WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND IH.COMPANY_CODE=ID.COMPANY_CODE AND IH.CC_CODE=ID.CC_CODE"
...
ERR1:
    ...
    Private Sub ChequeSearch(ByRef Col As Integer, ByRef Row As Integer)
        ...
        Else
            ...
            Exit Sub
        End If
        SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND BANKCODE='" & mBankCode & "'"
        If CurMKey = "" Then
    ...
    Private Sub SprdMain_LeaveCell(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_LeaveCellEvent) Handles SprdMain.LeaveCell
        ...
        Select Case eventArgs.col
            ...
            Case ColAccountName
                ...
                MainClass.ValidateWithMasterTable(pAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "")
                mAccountCode = MasterNo
                '            If IsDate(TxtVDate.Text) Then										
                ...
                If GetHeadType(pAccountName) = "L" Then
                    SprdMain.Col = ColEmp
                    SprdMain.Row = eventArgs.row
                    If MainClass.ValidateWithMasterTable(mAccountCode, "SUPP_CUST_CODE", "EMP_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        SprdMain.Text = MasterNo
                    End If
                    mEmpCode = Trim(SprdMain.Text)
                    If MainClass.ValidateWithMasterTable(mEmpCode, "EMP_CODE", "EMP_DEPT_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        SprdMain.Col = ColDept
                        SprdMain.Text = IIf(Trim(SprdMain.Text) = "", Trim(MasterNo), SprdMain.Text)
                    End If
                    If MainClass.ValidateWithMasterTable(mEmpCode, "EMP_CODE", "COST_CENTER_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        SprdMain.Col = ColCC
                        SprdMain.Text = IIf(Trim(SprdMain.Text) = "", Trim(MasterNo), SprdMain.Text)
                    End If
                End If
            ...
            Case ColSAC
                ...
                If mSACCode = "" Then
                    ...
                    SprdMain.Text = VB6.Format(0, "0.00")
                Else
                    If MainClass.ValidateWithMasterTable(mSACCode, "HSN_CODE", "HSN_CODE", "GEN_HSN_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'") = False Then
                        MsgInformation("Invalid SAC.")
                        MainClass.SetFocusToCell(SprdMain, eventArgs.row, ColSAC)
                        Exit Sub
                    End If
                    If MainClass.ValidateWithMasterTable(mSACCode, "HSN_CODE", "GST_RATE_OPT", "GEN_HSN_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'") = True Then
            ...
            Case ColDivisionCode
                ...
                If pAccountName <> "" Then
                    If MainClass.ValidateWithMasterTable(pAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        mAccountCode = MasterNo
            ...
            Case ColCGSTPer, ColSGSTPer, ColIGSTPer
                ...
                '                Exit Sub										
                '            End If										
                '            If MainClass.ValidateWithMasterTable(mHSNCode, "HSN_CODE", "GST_RATE_OPT", "GEN_HSN_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'") = True Then										
                '                mRateOption = MasterNo										
                '            Else										
                '                mRateOption = "N"										
                '            End If										
...
ErrSprdTotal:
    ...
    Private Function CheckMst(ByRef mCol As Integer, ByRef TabName As String, Optional ByRef pCode As String = "") As Boolean
        ...
        With SprdMain
            ...
            .Col = mCol
            If Trim(.Text) <> "" Then
                mSqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
...
ERR1:
    ...
    Private Function ShowDetailForm() As String
        ...
        With SprdMain
            ...
            .Col = ColCC
            If Trim(.Text) <> "" Then
                If MainClass.ValidateWithMasterTable(.Text, "CC_CODE", "CC_DESC", "FIN_CCENTER_HDR", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mCostCName = MasterNo
                Else
                    Exit Function
                End If
                ''MainClass.ValidateWithMasterTable .Text, "COST_CENTER_CODE", "COST_CENTER_DESC", "CST_CENTER_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""										
                mCostCode = Trim(.Text) ''MasterNo										
            End If
            .Col = ColDivisionCode
            If Val(.Text) <> 0 Then
                If MainClass.ValidateWithMasterTable(.Text, "DIV_CODE", "DIV_CODE", "INV_DIVISION_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If mHeadType = "T" Then
            ...
            If Trim(txtSection.Text) = "" Then
                If MainClass.ValidateWithMasterTable((txtPName.Text), "SUPP_CUST_NAME", "SECTIONCODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mSectionCode = MasterNo
                    If MainClass.ValidateWithMasterTable(mSectionCode, "CODE", "NAME", "TDS_SECTION_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        ElseIf mHeadType = "L" Then
            ...
            If UCase(SprdMain.Text) = "CR" Then Exit Function
            With frmLoanMaster
                If MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = False Then Exit Function
                mAccountCode = MasterNo
                If MainClass.ValidateWithMasterTable(mAccountCode, "SUPP_CUST_CODE", "EMP_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = False Then Exit Function
                mEmpCode = MasterNo
    ...
    Private Sub ShowTDSDetail()
        ...
        If RsTDSDetail.EOF = False Then
            With RsTDSDetail
                If MainClass.ValidateWithMasterTable(.Fields("ACCOUNTCODE").Value, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                ...
                If MainClass.ValidateWithMasterTable(.Fields("PARTYCODE").Value, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                ...
                If MainClass.ValidateWithMasterTable(.Fields("SECTIONCODE").Value, "Code", "Name", "TDS_Section_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ErrPart:
    ...
    Private Sub txtExpBillNo_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtExpBillNo.Validating
        ...
        If Trim(txtExpBillNo.Text) = "" Then
            ...
            GoTo EventExitSub
        End If
        SqlStr = " SELECT CMST.SUPP_CUST_NAME, IH.AUTO_KEY_EXPINV, " & vbCrLf & " IH.BILLNO, IH.EXPINV_DATE " & vbCrLf & " FROM FIN_EXPINV_HDR IH, FIN_SUPP_CUST_MST CMST " & vbCrLf & " WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE " & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE " & vbCrLf & " AND IH.AUTO_KEY_EXPINV=" & Val(txtExpBillNo.Text) & ""
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
...
EventExitSub:
    ...
    Private Sub txtExpPartyName_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtExpPartyName.Validating
        ...
        SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND SUPP_CUST_TYPE IN ('S','C')"
...
SearchErr:
    ...
    Private Sub txtImpMRRNo_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtImpMRRNo.Validating
        ...
        If Val(txtImpMRRNo.Text) = 0 Then
            ...
            GoTo EventExitSub
        End If
        SqlStr = " SELECT CMST.SUPP_CUST_NAME, IH.AUTO_KEY_MRR, " & vbCrLf & " IH.BILL_NO, IH.BILL_DATE " & vbCrLf & " FROM INV_GATE_HDR IH, FIN_SUPP_CUST_MST CMST " & vbCrLf & " WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE " & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE " & vbCrLf & " AND IH.AUTO_KEY_MRR=" & Val(txtImpMRRNo.Text) & ""
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If Not RsTemp.EOF Then
...
SearchErr:
    ...
    Private Function GetCashBookName() As String
        ...
        GetCashBookName = ""
        SqlStr = "SELECT SUPP_CUST_NAME FROM FIN_SUPP_CUST_MST" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND SUPP_CUST_TYPE = '1' AND STATUS='O'"
...
SearchErr:
    ...
    Private Function CheckAccountName(ByRef pAccountName As String, ByRef col2 As Integer, ByRef Row2 As Integer) As Boolean
        ...
        If pAccountName = "" Then
            Exit Function
        End If
        SqlStr = " SELECT SUPP_CUST_CODE FROM FIN_SUPP_CUST_MST WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND SUPP_CUST_NAME='" & MainClass.AllowSingleQuote(Trim(pAccountName)) & "'"
        If ADDMode = True Then
...
ERR1:
    ...
    Private Function CheckforAccountsType(ByRef mAccountName As String) As Boolean
        ...
        If MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, mAccountCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            If MainClass.ValidateWithMasterTable((txtPartyName.Text), "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, mBookCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
    ...
    Private Function CheckDivisionWiseDRCRMatch(ByRef mDRCRBal As Double, ByRef xDivName As String) As Boolean
        ...
        SqlStr = "SELECT DIV_CODE,DIV_DESC FROM INV_DIVISION_MST WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & " ORDER BY DIV_CODE"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTemp.EOF = False Then
            Do While RsTemp.EOF = False
...
ErrPart:
    ...
    Private Sub txtPartyName_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtPartyName.Validating
        ...
        SqlStr = " Select SUPP_CUST_NAME,SUPP_CUST_CODE,STATUS FROM FIN_SUPP_CUST_MST Where " & vbCrLf & " COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf & " AND SUPP_CUST_NAME = '" & MainClass.AllowSingleQuote(Trim(txtPartyName.Text)) & "'"
        Select Case lblBookType.Text
            Case ConBankPayment, ConBankReceipt, ConPDCPayment, ConPDCReceipt
        ...
        If lblBookType.Text = ConBankReceipt Or lblBookType.Text = ConBankPayment Then
            ...
            End If
        End If
        If MainClass.ValidateWithMasterTable(txtPartyName.Text, "VNAME", "VTYPE", "FIN_VOUCHERTYPE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND BOOKTYPE='" & VB.Left(lblBookType.Text, 1) & "'") = True Then
...
EventExitSub:
    ...
    Private Sub SearchTDSSection()
        On Error GoTo SearchErr
        'If MainClass.SearchMaster((txtSection.Text), "TDS_Section_MST", "NAME", "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        If MainClass.SearchGridMaster((txtSection.Text), "TDS_Section_MST", "NAME", "CODE", , , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
SearchErr:
    ...
    Private Sub txtSection_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtSection.Validating
        ...
        If MainClass.ValidateWithMasterTable((txtSection.Text), "NAME", "NAME", "TDS_Section_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = False Then
...
EventExitSub:
    ...
    Private Sub txtServProvided_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtServProvided.Validating
        ...
        SqlStr = " SELECT CODE, NAME, REVERSE_CHARGE_APP, REVERSE_CHARGE_PER, SERVICE_TAX_PER,SERVICE_TAX_ON_PER" & vbCrLf & " FROM FIN_SERVPROV_MST" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND NAME='" & MainClass.AllowSingleQuote((txtServProvided.Text)) & "'"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTemp.EOF = False Then
...
EventExitSub:
    ...
    Public Sub CheckVouchExistance()
        ...
        SqlStr = " Select * From FIN_VOUCHER_HDR WHERE " & vbCrLf _
            ...
            & " AND COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf _
...
ERR1:
    ...
    Private Sub FillGridFromTMSal(ByRef pYM As String, ByRef pArrear As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double)
        ...
        If MainClass.ValidateWithMasterTable(mDivisionCode, "DIV_CODE", "DIV_DESC", "INV_DIVISION_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mDivisionDesc = Trim(MasterNo)
        End If
        SqlStr = " Select ACM.SUPP_CUST_CODE, ACM.SUPP_CUST_NAME, SUM(AMOUNT) AS AMOUNT, DC,PARTICULARS" & vbCrLf & " FROM FIN_TMSal_TRN TRN, FIN_SUPP_CUST_MST ACM" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.COMPANY_CODE=ACM.COMPANY_CODE " & vbCrLf & " AND TRN.ACCOUNTCODE=ACM.SUPP_CUST_CODE " & vbCrLf & " AND YM='" & pYM & "' AND ISARREAR='" & pArrear & "' AND DIV_CODE=" & mDivisionCode & "" & vbCrLf & " AND BookType='" & pBookType & "' AND BookSubType='" & pBookSubType & "'" & vbCrLf & " GROUP BY ACM.SUPP_CUST_CODE,ACM.SUPP_CUST_NAME,DC,PARTICULARS ORDER BY DC DESC,ACM.SUPP_CUST_NAME"
            ...
            With SprdMain
                Do While Not RsTMSal.EOF
                    ...
                    If mAmount <> 0 Then
                        ...
                        End If
                        .Col = ColEmp
                        If MainClass.ValidateWithMasterTable(RsTMSal.Fields("SUPP_CUST_CODE"), "SUPP_CUST_CODE", "EMP_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ErrPart:
    ...
    Private Sub FillGridFromTMEnCash(ByRef pYM As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double)
        ...
        If RsTMSal.EOF = False Then
            ...
            If MainClass.ValidateWithMasterTable(ConESI, "TYPE", "ACCOUNTCODEPOST", "PAY_SALARYHEAD_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mESIPayableCode = Trim(MasterNo)
            End If
            If MainClass.ValidateWithMasterTable(mESIPayableCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mESIPayableName = Trim(MasterNo)
            End If
            If MainClass.ValidateWithMasterTable(mPFAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mPFAccountName = Trim(MasterNo)
            End If
            If MainClass.ValidateWithMasterTable(mESIAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            ...
            With SprdMain
                ...
                If mGrossAmount <> 0 Then
                    ...
                    .Col = ColDC
                    .Text = "Dr"
                    If MainClass.ValidateWithMasterTable(mDrAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                ...
                If mNetAmount <> 0 Then
                    ...
                    .Col = ColDC
                    .Text = "Cr"
                    If MainClass.ValidateWithMasterTable(mCrAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        mAccountName = Trim(MasterNo)
                    End If
                    .Col = ColAccountName
                    .Text = mAccountName
                ...
                If mVPFAmount <> 0 And mPFAmount > 0 Then
                    SqlStr = " SELECT ACCOUNTCODEPOST " & vbCrLf & " FROM PAY_SALARYHEAD_MST" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TYPE=" & ConVPFAllw & ""
                    MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
                    If RsTemp.EOF = False Then
                    ...
                    If mAccountCode <> "-1" Then
                        If MainClass.ValidateWithMasterTable(mAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                            mAccountName = Trim(MasterNo)
                        End If
                        .Row = cntRow
                        .Col = 1
                ...
                If mDedAmount <> 0 Then
                    SqlStr = "SELECT EMP_CODE, AMOUNT, ADD_DEDUCTCODE, ACCOUNTCODEPOST, TYPE " & vbCrLf & " FROM PAY_MONTHLY_TRN TRN, PAY_SALARYHEAD_MST MST " & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.PAYYEAR=" & pYM & " " & vbCrLf & " AND TRN.COMPANY_CODE=MST.COMPANY_CODE" & vbCrLf & " AND TRN.ADD_DEDUCTCODE=MST.CODE" & vbCrLf & " AND SAL_FLAG='E' AND AMOUNT<>0"
                    MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
                    If RsTemp.EOF = False Then
                        Do While RsTemp.EOF = False
                            ...
                            Select Case mSALType
                                Case CStr(ConAdvance)
                                    If MainClass.ValidateWithMasterTable(mEmpCode, "EMP_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HEADTYPE='L'") = True Then
                                        mAccountName = Trim(MasterNo)
                                    End If
                                Case CStr(ConLoan)
                                    If MainClass.ValidateWithMasterTable(mEmpCode, "EMP_CODE", "ADV_ACCOUNT_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                                ...
                                Case CStr(ConImprest)
                                    If MainClass.ValidateWithMasterTable(mEmpCode, "EMP_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HEADTYPE='I'") = True Then
                                        mAccountName = Trim(MasterNo)
                                    End If
                                Case Else
                                    mAccountCode = IIf(IsDBNull(RsTemp.Fields("ACCOUNTCODEPOST").Value), "-1", RsTemp.Fields("ACCOUNTCODEPOST").Value)
...
ErrPart:
    ...
    Private Sub FillGridFromTMPerks(ByRef pSalDate As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double)
        ...
        End If
        cntRow = 1
        SqlStr = " Select SUM(AMOUNT) AS AMOUNT, ADD_DEDUCTCODE" & vbCrLf & " FROM PAY_PERKS_TRN  TRN" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TO_CHAR(TRN.SAL_DATE,'YYYYMM')='" & VB6.Format(pSalDate, "YYYYMM") & "' AND TRN.BOOKTYPE='" & pBookType & "' AND TRN.DIV_CODE=" & mDivisionCode & ""
        ...
        If RsTMSal.EOF = False Then
            Do While RsTMSal.EOF = False
                ...
                If MainClass.ValidateWithMasterTable(mSalaryHeadCode, "CODE", "ACCOUNTCODEPOST", "PAY_SALARYHEAD_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mAccountCode = Trim(MasterNo)
                Else
                    mAccountCode = "-1"
                End If
            ...
            Loop
        End If
        SqlStr = " Select SUM(AMOUNT) AS AMOUNT" & vbCrLf & " FROM PAY_PERKS_TRN  TRN" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TO_CHAR(TRN.SAL_DATE,'YYYYMM')='" & VB6.Format(pSalDate, "YYYYMM") & "'  AND TRN.BOOKTYPE='" & pBookType & "'"
        If pBookSubType = "X" Then
            SqlStr = SqlStr & vbCrLf & " AND PAID_WEEK=1"
        ElseIf pBookSubType = "Y" Then
            SqlStr = SqlStr & vbCrLf & " AND PAID_WEEK=2"
...
ErrPart:
    ...
    Private Sub FillGridFromTMLTA(ByRef pSalDate As String, ByRef pEmpCode As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double)
        ...
        cntRow = 1
        mAmount = 0
        SqlStr = " Select NET_LTA_AMOUNT AS AMOUNT,FROM_DATE,TO_DATE" & vbCrLf & " FROM PAY_LTA_HDR  TRN" & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND TRN.FYEAR=" & lblELYear.Text & "" '& vbCrLf |            & " AND TO_CHAR(TRN.TO_DATE,'YYYYMM')='" & vb6.Format(pSalDate, "YYYYMM") & "'"										
        ...
        If RsTMSal.EOF = False Then
            ...
            If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "EMP_NAME", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ErrPart:
    ...
    Private Sub FillGridFromTMFullFinal(ByRef pSalDate As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double, ByRef pEmpCode As String)
        ...
        Else
            mEmpName = "-1"
        End If
        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "EMP_DEPT_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            pDeptCode = MasterNo
        Else
            pDeptCode = ""
        End If
        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "COST_CENTER_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mCCCode = MasterNo
        Else
            mCCCode = "001"
        End If
        ...
        mPFAmount = 0
        mESIAmount = 0
        SqlStr = " Select * " & vbCrLf & " FROM PAY_FFSETTLE_HDR IH" & vbCrLf & " WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND IH.EMP_CODE='" & VB6.Format(pEmpCode, "000000") & "'"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTemp.EOF = False Then
            'GROSS SALARY & Arrear ....										
            ...
            Do While RsTMSal.EOF = False
                ...
                If mAmount <> 0 Then
                    ...
                    If mType = ConAdvance Then
                        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HEADTYPE='L'") = True Then
                            mAccountCode = MasterNo
                        Else
                            mAccountCode = "-1"
                        End If
                    ElseIf mType = ConImprest Then
                        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HEADTYPE='I'") = True Then
                            mAccountCode = MasterNo
                        Else
                            mAccountCode = "-1"
                        End If
                    ElseIf mType = ConLoan Then
                        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "ADV_ACCOUNT_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                            mAccountCode = MasterNo
                        Else
                            mAccountCode = "-1"
                        End If
                    ...
                    ElseIf mType = ConWelfare Then
                        mNarration1 = "Employer Contribution:Double of Employee deduction"
                        If MainClass.ValidateWithMasterTable(mWFAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                            mAccountName = Trim(MasterNo)
                        Else
                            mAccountName = ""
                        End If
...
ErrPart:
    ...
    Private Function GetPensionWages(ByRef pYM As Integer, ByRef mArrear As String, ByRef mDivisionCode As Double) As Double
        ...
        GetPensionWages = 0
        SqlStr = ""
        SqlStr = " SELECT  SUM(PENSIONWAGES) As AMOUNT " & vbCrLf & " FROM PAY_PFESI_TRN " & vbCrLf & " WHERE " & vbCrLf & " COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf & " AND FYEAR =" & RsCompany.Fields("FYEAR").Value & " " & vbCrLf & " AND TO_CHAR(SAL_DATE,'YYYYMM')=" & pYM & "" & vbCrLf & " AND ISARREAR='" & mArrear & "'" & vbCrLf & " AND EMP_CODE = '" & lblEmpCode.Text & "'"
        '    SqlStr = SqlStr & vbCrLf _										
        ''        & " SELECT DISTINCT EMP_CODE " & vbCrLf _										
        ''        & " FROM PAY_SAL_TRN " & vbCrLf _										
        ''        & " WHERE " & vbCrLf _										
        ''        & " COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf _										
...
UpdateAccountPostingHeadErr:
    ...
    Private Sub FillGridFromTMFullFinalOld(ByRef pSalDate As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef pEmpCode As String)
        ...
        Else
            pDeptCode = ""
        End If
        If MainClass.ValidateWithMasterTable(pEmpCode, "EMP_CODE", "DIV_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ErrPart:
    ...
    Private Sub FillGridFromVoucherSal(ByRef pYM As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mDivisionCode As Double, ByRef pEmpCode As String)
        ...
        mPFAmount = 0
        mESIAmount = 0
        SqlStr = " Select DISTINCT PAYABLESALARY, WDAYS " & vbCrLf & " FROM PAY_SALVOUCHER_TRN" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND EMP_CODE='" & pEmpCode & "' AND TO_CHAR(SAL_DATE,'YYYYMM')=" & pYM & ""
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTemp.EOF = False Then
        ...
        End If
        SqlStr = " Select SUM(PAYABLEAMOUNT) AS PAYABLEAMOUNT, SALHEADCODE,TYPE " & vbCrLf & " FROM PAY_SALVOUCHER_TRN ID, PAY_SALARYHEAD_MST SMST" & vbCrLf & " WHERE ID.COMPANY_CODE=SMST.COMPANY_CODE" & vbCrLf & " AND ID.SALHEADCODE=SMST.CODE" & vbCrLf & " AND ID.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND ID.EMP_CODE='" & pEmpCode & "' AND TO_CHAR(SAL_DATE,'YYYYMM')=" & pYM & "" & vbCrLf & " AND SMST.ADDDEDUCT=" & ConDeduct & ""
        SqlStr = SqlStr & vbCrLf & " AND PAYABLEAMOUNT<>0"
        SqlStr = SqlStr & vbCrLf & " GROUP BY SALHEADCODE,TYPE"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTMSal, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTMSal.EOF = False Then
            ...
            Loop
        End If
        SqlStr = " Select SUM(PAYABLEAMOUNT) AS PAYABLEAMOUNT " & vbCrLf & " FROM PAY_SALVOUCHER_TRN ID, PAY_SALARYHEAD_MST SMST" & vbCrLf & " WHERE ID.COMPANY_CODE=SMST.COMPANY_CODE" & vbCrLf & " AND ID.SALHEADCODE=SMST.CODE" & vbCrLf & " AND ID.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND ID.EMP_CODE='" & pEmpCode & "' AND TO_CHAR(SAL_DATE,'YYYYMM')=" & pYM & "" & vbCrLf & " AND SMST.ADDDEDUCT=" & ConEarning & ""
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If RsTemp.EOF = False Then
            'GROSS SALARY & Arrear ....										
...
ErrPart:
    ...
    Private Sub Show1()
        ...
        If MainClass.ValidateWithMasterTable((RsTRNMain.Fields("BOOKCODE").Value), "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If MainClass.ValidateWithMasterTable(mPartyCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If MainClass.ValidateWithMasterTable(mServiceCode, "CODE", "NAME", "FIN_SERVPROV_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
ShowErrPart:
    ...
    Private Sub CopyToTempPRDetail()
        ...
        SqlStr = "Insert Into FIN_TEMPBILL_TRN  ( " & vbCrLf _
            ...
            & " ISREGDNO, STFORMCODE, STDUEFORMCODE,TAXABLE_AMOUNT,PONO,DIV_CODE,REF_NO,TEMPMKEY,BILL_TO_LOC_ID,BILL_COMPANY_CODE " & vbCrLf _
            & " )"
        SqlStr = SqlStr & vbCrLf & " SELECT '" & MainClass.AllowSingleQuote(PubUserID) & "' , " & vbCrLf _
            ...
            & " STFORMDATE, STFORMAMT, STDUEFORMNAME, " & vbCrLf _
            & " STDUEFORMNO, STDUEFORMDATE, STDUEFORMAMT, " & vbCrLf _
            & " ISREGDNO, STFORMCODE, STDUEFORMCODE,TAXABLE_AMOUNT,PONO,DIV_CODE,REF_NO," & pProcessKey & ",BILL_TO_LOC_ID, BILL_COMPANY_CODE" & vbCrLf _
            & " FROM FIN_BILLDETAILS_TRN Where MKey='" & CurMKey & "'"
        PubDBCn.Execute(SqlStr)

        SqlStr = "INSERT INTO FIN_TEMP_SERVICE_TRN  ( " & vbCrLf & " USERID, SUBROWNO, ACCOUNTCODE, " & vbCrLf & " RO, BILLNO, BILLDATE, BILLAMOUNT, " & vbCrLf & " TAX_ON, SERVICETAX_AMT, CESS_AMT, " & vbCrLf & " SERV_PROV, ISSERVICECLAIM, SERVNO, " & vbCrLf & " SERVDATE, SERVICE_PER, CESS_PER, " & vbCrLf & " SHE_CESS_PER,SHE_CESS_AMT, SERVICE_PER_PROV, SERVICE_PER_REC, " & vbCrLf & " SERVICETAX_REC_AMT, CESS_REC_AMT, SHE_CESS_REC_AMT, " & vbCrLf & " SWACHH_CESS_PER, SWACHH_CESS_AMOUNT, SWACHH_CESS_AMOUNT_REC, " & vbCrLf & " KK_CESS_PER, KK_CESS_AMOUNT, KK_CESS_AMOUNT_REC)"
...
ERR1:
    ...
    Private Sub ShowDetail()
        ...
        Do While RsTRNDetail.EOF = False
            ...
            SprdMain.Col = ColAccountName
            If MainClass.ValidateWithMasterTable(mNewAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
    ...
    Private Function FieldsVerification() As Boolean
        ...
        If MODIFYMode = True Then
            If InStr(1, mIsAuthorisedUser, "S") = 0 Then
                ...
                End If
            End If
            If MainClass.ValidateWithMasterTable(RsTRNMain.Fields("mKey").Value, "JVMKEY", "VNO", "FIN_SUPP_PURCHASE_HDR", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If Trim(txtPartyName.Text) <> "" Then
            If MainClass.ValidateWithMasterTable(txtPartyName.Text, "SUPP_CUST_Name", "STATUS", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                If MasterNo = "C" Then
...
NextLine:
        ...
        Next
        If lblBookType.Text = ConBankPayment Then
            If MainClass.ValidateWithMasterTable(txtPartyName.Text, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mBankCode = MasterNo
            End If
        End If
        For cntRow = 1 To SprdMain.MaxRows - 1
            ...
            End If
            SprdMain.Col = ColAccountName
            If MainClass.ValidateWithMasterTable(SprdMain.Text, "SUPP_CUST_Name", "SUPP_CUST_Code", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mAcctCode = MasterNo
                If lblBookType.Text = ConBankPayment And ADDMode = True Then
                    If MainClass.ValidateWithMasterTable(mAcctCode, "SUPP_CUST_CODE", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND STOP_BANK='Y'") = True Then
                        MsgBox("Bank Payment Cann't Be Made for Such Customer/Supplier, So cann't be saved", MsgBoxStyle.Information)
                        FieldsVerification = False
                        Exit Function
                    End If
                    mLenderBankCode = ""
                    If MainClass.ValidateWithMasterTable(mAcctCode, "SUPP_CUST_CODE", "LENDER_BANK_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                ...
                If CheckExpHead((SprdMain.Text)) = True Then
                    ...
                    '                                End If
                    '                            End If
                    '                            If MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "HSN_CODE", "HSN_CODE", "GEN_HSN_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'") = False Then
                    '                                MsgInformation("Invalid SAC.")
                    '                                MainClass.SetFocusToCell(SprdMain, cntRow, ColSAC)
                    '                                Exit Function
                    '                            Else
            ...
            If chkCancelled.CheckState = System.Windows.Forms.CheckState.Unchecked Then
                SprdMain.Col = ColAccountName
                If MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "SUPP_CUST_NAME", "HEADTYPE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mHeadType = MasterNo
                Else
                    mHeadType = ""
                End If
                ...
                ElseIf GetHeadType(Trim(SprdMain.Text)) = "T" And VB.Left(mDC, 1) = "C" Then
                    ...
                    If Trim(TxtTDSAccount.Text) = "" Then
                        ...
                        Exit Function
                    End If
                    If MainClass.ValidateWithMasterTable(TxtTDSAccount.Text, "SUPP_CUST_NAME", "SUPP_CUST_NAME", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HeadType IN ('T','1')") = False Then
                    ...
                    If mHeadType = "T" Then ''Only TDS not Salary TDS  '06-aug-2010  ''lblBookType.text = ConJournal And CHECK IN BANK ALSO										
                        If MainClass.ValidateWithMasterTable(txtPName.Text, "SUPP_CUST_NAME", "PAN_NO", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                            mPANNo = MasterNo
                        Else
                            mPANNo = ""
                        End If
                ...
                ElseIf GetHeadType(Trim(SprdMain.Text)) = "S" And VB.Left(mDC, 1) = "D" Then
                    ...
                    '                If MainClass.ValidateWithMasterTable(mAcctCode, "ADV_ACCOUNT_CODE", "EMP_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then										
                    '                    mEmpCode = MasterNo										
        ...
        If lblBookType.Text = ConJournal Then
            If ADDMode = True Then
                If chkTDS.CheckState = System.Windows.Forms.CheckState.Checked Then
                    If MainClass.ValidateWithMasterTable(mPartyName, "SUPP_CUST_NAME", "PAN_NO", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        mPANNo = MasterNo
                    Else
                        mPANNo = ""
                    End If
        ...
        If lblBookType.Text = ConJournal Then
            For ii = 1 To SprdMain.MaxRows - 1
                ...
                If MainClass.ValidateWithMasterTable(Trim(mServiceGL), "SUPP_CUST_NAME", "HEADTYPE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            ...
            If mHeadType = "4" Then
                If Trim(txtServProvided.Text) = "" Then
                    ...
                    Exit Function
                End If
                If MainClass.ValidateWithMasterTable(Trim(txtServProvided.Text), "NAME", "CODE", "FIN_SERVPROV_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = False Then
    ...
    Private Function CheckAdvanceTdsDeduct() As Boolean
        ...
        For cntRow = 1 To SprdMain.MaxRows - 1
            ...
            If MainClass.ValidateWithMasterTable((SprdMain.Text), "SUPP_CUST_Name", "SUPP_CUST_Code", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                mAcctCode = MasterNo
            End If
            SqlStr = "SELECT DISTINCT BILLDATE" & vbCrLf & " FROM FIN_POSTED_TRN" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR>=" & RsCompany.Fields("FYEAR").Value - 1 & " AND FYEAR<=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND ACCOUNTCODE='" & mAcctCode & "'" & vbCrLf & " AND TRNTYPE IN ('A','O') AND DC='D'" & vbCrLf & " AND MKEY IN (" & vbCrLf & " SELECT MKEY FROM FIN_POSTED_TRN A, FIN_SUPP_CUST_MST B" & vbCrLf & " WHERE A.COMPANY_CODE=B.COMPANY_CODE" & vbCrLf & " AND A.ACCOUNTCODE=B.SUPP_CUST_CODE" & vbCrLf & " AND A.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND A.FYEAR>=" & RsCompany.Fields("FYEAR").Value - 1 & " AND A.FYEAR<=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND B.HEADTYPE='T' AND A.DC='C'" & vbCrLf & " )"
            MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
            If RsTemp.EOF = False Then
...
ERR1:
    ...
    Private Function GetExpVNo(ByRef pExpBillNo As Double, ByRef pVNo As String) As Boolean
        ...
        Dim RsTemp As ADODB.Recordset = Nothing
        GetExpVNo = False
        SqlStr = " SELECT VNO " & vbCrLf & "  FROM FIN_VOUCHER_HDR IH" & vbCrLf & "  WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & "  AND IH.EXP_BILL_NO=" & Val(CStr(pExpBillNo)) & ""
        If MODIFYMode = True Then
...
ErrPart:
    ...
    Private Function CheckLastestVDate(ByRef mVDate As Date, ByRef mVType As String) As Boolean
        ...
        SqlStr = "SELECT VDATE FROM FIN_VOUCHER_HDR " & vbCrLf & " WHERE Company_Code=" & Val(lblCompanyCode.Text) & " AND " & vbCrLf & " FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND " & vbCrLf & " BookType='" & mBookType & "' AND " & vbCrLf & " BookSubType='" & mBookSubType & "' AND " & vbCrLf & " VTYPE='" & MainClass.AllowSingleQuote(Trim(mVType)) & "' AND " & vbCrLf & " VDATE>TO_DATE('" & VB6.Format(mVDate, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
...
CheckLastestVDateErr:
    ...
    Private Function CheckLastestVNo(ByRef mVNo As Integer, ByRef mVDate As Date, ByRef mVType As String) As Boolean
        ...
        End If
        ''Checl Previous VNO...										
        SqlStr = "SELECT Max(VNOSeq) AS VNoSeq FROM FIN_VOUCHER_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND " & vbCrLf & " FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND " & vbCrLf & " BookType='" & mBookType & "' AND " & vbCrLf & " BookSubType='" & mBookSubType & "' AND " & vbCrLf & " VTYPE='" & MainClass.AllowSingleQuote(Trim(mVType)) & "' AND " & vbCrLf & " VDATE<TO_DATE('" & VB6.Format(mVDate, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
    ...
    Private Function Update1() As Boolean
        ...
        Select Case lblBookType.Text
            ...
            Case Else
                ...
                End If
        End Select
        If MainClass.ValidateWithMasterTable(txtExpPartyName.Text, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mExpPartyCode = MasterNo
        End If
        If MainClass.ValidateWithMasterTable(txtImpPartyName.Text, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mImpPartyCode = MasterNo
        End If
        ...
        If MainClass.ValidateWithMasterTable(Trim(txtServProvided.Text), "NAME", "CODE", "FIN_SERVPROV_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        If ADDMode = True Then
            ...
            CurMKey = Val(lblCompanyCode.Text) & VB6.Format(RsCompany.Fields("FYEAR").Value) & VB6.Format(mRowNo)
            SqlStr = " INSERT INTO FIN_VOUCHER_HDR ( " & vbCrLf _
                ...
                & " IS_REVERSAL_MADE, IS_REVERSAL_VOUCHER, REVERSAL_MKEY) VALUES ( "

            SqlStr = SqlStr & vbCrLf _
                & " '" & CurMKey & "'," & Val(lblCompanyCode.Text) & "," & vbCrLf _
                ...
                & " '" & mReverseChargeApp & "', '" & lblReversalMade.Text & "','" & lblReversalVoucher.Text & "','" & lblReversalMkey.Text & "')"
        ...
        If lblBookType.Text = ConJournal Or lblBookType.Text = ConContra Then
        Else
            xSqlStr = "SELECT DIV_CODE FROM INV_DIVISION_MST WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
                ...
                Do While RSDiv.EOF = False
                    ...
                    If mVAmount <> 0 Then
                        ...
                        If UpdateTRN(PubDBCn, CurMKey, mRowNo, mSubRowNo, mBookCode, mVType, mBookType, mBookSubType, mBookCode, mVnoStr, (TxtVDate.Text), mVnoStr,
                                     (TxtVDate.Text), mVAmount, mDrCr, "P", "", "", CStr(-1), CStr(-1), CStr(-1), CStr(-1), "", "", "P", "", "", (txtNarration.Text), "",
...
ErrPart:
    ...
    Private Function UpdateEMIVoucher(ByRef pBookType As String, ByRef pBookSubType As String, ByRef pVType As String, ByRef pVDate As String, ByRef pBookCode As String, ByRef pExpPartyCode As String, ByRef pImpPartyCode As String, ByRef pNoOfEMI As Integer, ByRef mTotalNoOfEMI As Integer) As Boolean
        ...
        pCurMKey = Val(lblCompanyCode.Text) & VB6.Format(RsCompany.Fields("FYEAR").Value) & VB6.Format(pRowNo)
...
ErrPart:
    ...
    Private Function UpdateVoucherSalary(ByRef pAcPosting As String, Optional ByRef xVNo As String = "") As Boolean
        ...
        End If
        SqlStr = SqlStr & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND EMP_CODE='" & VB6.Format(lblEmpCode.Text, "000000") & "'" & vbCrLf & " AND TO_CHAR(SAL_DATE,'YYYYMM')='" & VB6.Format(TxtVDate.Text, "YYYYMM") & "'"
        PubDBCn.Execute(SqlStr)
...
ErrPart:
    ...
    Private Function GetBillDetailAmount(ByRef mPRowNo As Integer, ByRef mSuppCustName As String, ByRef mDivCode As Double, ByRef mDC As String, ByRef mSuppCustAmount As Double) As Boolean
        ...
        If MainClass.ValidateWithMasterTable(mSuppCustName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            xAcctCode = MasterNo
        Else
            xAcctCode = CStr(-1)
        End If
...
ERR1:
    ...
    Private Function UpdateSalVoucher(ByRef pDBCn As ADODB.Connection, ByRef pMKey As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef mVNo As String, ByRef mDivisionCode As Double) As Boolean
        ...
        Dim mBankCode As String
        Dim pVNo As String
        SqlStr = " DELETE FROM FIN_SalVoucher_TRN " & vbCrLf & " WHERE " & vbCrLf & " COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND mKey=" & pMKey & " "
        If Val(lblELYear.Text) <> 0 Then
...
ErrPart:
    ...
    Private Function UpdateFullNFinal(ByRef pAcPosting As String) As Boolean
        On Error GoTo ErrPart
        Dim SqlStr As String = ""
        SqlStr = " UPDATE PAY_FFSETTLE_HDR SET AC_POSTING='" & pAcPosting & "'" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND EMP_CODE='" & VB6.Format(lblEmpCode.Text, "000000") & "'"
        PubDBCn.Execute(SqlStr)
...
ErrPart:
    ...
    Private Function UpdateLTA(ByRef pAcPosting As String) As Boolean
        ...
        SqlStr = " UPDATE PAY_LTA_HDR SET AC_POSTING='" & pAcPosting & "'," & vbCrLf & " CHQ_NO='" & mChqNo & "', CHQ_DATE=TO_DATE('" & VB6.Format(mChqDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), BANK_NAME='" & MainClass.AllowSingleQuote((txtPartyName.Text)) & "'" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & lblELYear.Text & "" & vbCrLf & " AND EMP_CODE='" & VB6.Format(lblEmpCode.Text, "000000") & "'"
        PubDBCn.Execute(SqlStr)
        UpdateLTA = True
        Exit Function
ErrPart:
    ...
    Private Function UpdateJVDetail(ByRef mMkey As String, ByRef mRowNo As Integer, ByRef mBookCode As String, ByRef mVType As String, ByRef mVNo As String, ByRef mVDate As String, ByRef pNarration As String, ByRef pDBCn As ADODB.Connection) As Boolean
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                .Row = cntRow
                .Col = ColDC
                If VB.Left(UCase(.Text), 1) = "C" Then
                    .Col = ColAccountName
                    mAccountName = Trim(.Text)
                    .Col = ColDivisionCode
        ...
        If mAccountName <> "" Then
            mPRRowNo = cntRow
            mDC = "D"
            mAccountCode = IIf(MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_Name", "SUPP_CUST_Code", "FIN_SUPP_CUST_MST", pDBCn, mAccountCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mAccountCode, -1)
    ...
    Public Function UpdateSuppPRDetail(ByRef pDBCn As ADODB.Connection, ByRef pMKey As String, ByRef pRowNo As Integer, ByRef pTRNDtlSubRow As Integer, ByRef pAccountCode As String, ByRef pBookCode As String, ByRef pVType As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef pVNo As String, ByRef pVDate As String, ByRef pTrnDC As String, ByRef pTrnAmount As Double, ByRef pChequeNo As String, ByRef pChqDate As String, ByRef pCostCCode As String, ByRef pDeptCode As String, ByRef pEmpCode As String, ByRef pExpCode As String, ByRef pIBRNo As String, ByRef pClearDate As String, ByRef pLocked As String, ByRef pNarration As String, ByRef xAmount As Double, ByRef xRemarks As String, ByRef mDivisionCode As Double) As Boolean
        ...
        If RsTempPRDetail.EOF = False Then
            ...
            Do While RsTempPRDetail.EOF = False
                ...
                mBillCompanyCode = IIf(IsDBNull(RsTempPRDetail.Fields("BILL_COMPANY_CODE").Value), Val(lblCompanyCode.Text), RsTempPRDetail.Fields("BILL_COMPANY_CODE").Value) ''
                ...
                SqlStr = "INSERT INTO FIN_BILLDETAILS_TRN ( " & vbCrLf _
                    ...
                    & " '" & mLocCode & "', " & Val(lblCompanyCode.Text) & ", '" & VB.Left(lblBookType.Text, 1) & "'," & Val(mBillCompanyCode) & ") "
    ...
    Private Function UpdateTDSCreditDetail(ByRef pMKey As String, ByRef pVNoStr As String, ByRef pBookType As String, ByRef pBookSubType As String) As Boolean
        ...
        For cntRow = 1 To SprdMain.MaxRows
            ...
            If VB.Left(UCase(Trim(SprdMain.Text)), 1) = "C" Then
                SprdMain.Col = ColAccountName
                mPartyName = Trim(SprdMain.Text)
                If MainClass.ValidateWithMasterTable(mPartyName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                    mPartyCode = MasterNo
                End If
                If MainClass.ValidateWithMasterTable(mPartyName, "SUPP_CUST_NAME", "SECTIONCODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
...
UpdateError:
    ...
    Private Function UpdateTDSDetail(ByRef pDBCn As ADODB.Connection, ByRef pMKey As String, ByRef pVNoStr As String, ByRef pBookType As String, ByRef pBookSubType As String, ByRef pCancelled As String) As Boolean
        ...
        If TxtTDSAccount.Text = "" Then
            ...
            Exit Function
        End If
        If MainClass.ValidateWithMasterTable((TxtTDSAccount.Text), "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND HeadType IN ('T','1')") = True Then
            mAccountCode = MasterNo
        Else
            UpdateTDSDetail = True
            Exit Function
        ...
        Else
            mPartyCode = "-1"
        End If
        If MainClass.ValidateWithMasterTable(txtSection.Text, "Name", "Code", "TDS_Section_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mSectionCode = MasterNo
        Else
            mSectionCode = -1
        End If
...
ERR1:
    ...
    Private Function UpdateDetail(ByRef mMkey As String, ByRef mRowNo As Integer, ByRef mBookCode As String, ByRef mVType As String, ByRef mVNo As String, ByRef mVDate As String, ByRef pNarration As String, ByRef pDBCn As ADODB.Connection) As Boolean
        ...
        pDBCn.Execute(SqlStr)
        If (lblBookType.Text = ConBankPayment Or lblBookType.Text = ConPDCPayment) Then
            SqlStr = "UPDATE FIN_CHEQUE_MST SET " & vbCrLf & " CHEQUE_STATUS='O'," & vbCrLf & " VMKEY=''," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "', " & vbCrLf & " ModDate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND VMKEY='" & MainClass.AllowSingleQuote(Trim(mMkey)) & "'"
            PubDBCn.Execute(SqlStr)
        End If
        mSameVNo = False
        With SprdMain
            For I = 1 To .MaxRows
                ...
                If mAccountName <> "" Then
                    ...
                    .Col = ColAccountName
                    mAccountCode = IIf(MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_Name", "SUPP_CUST_Code", "FIN_SUPP_CUST_MST", pDBCn, mAccountCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mAccountCode, -1)
                    If MainClass.ValidateWithMasterTable(mAccountCode, "ACCOUNTPOSTCODE", "ISFIXASSETS", "FIN_INVTYPE_MST", pDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CATEGORY='P'") = True Then
                    ...
                    .Col = ColCC
                    mCCCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "CC_CODE", "CC_CODE", "FIN_CCENTER_HDR", pDBCn, mCCCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mCCCode, -1)
                    .Col = ColExp
                    mExpCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "COST_CENTER_CODE", "COST_CENTER_CODE", "CST_CENTER_MST", pDBCn, mExpCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mExpCode, -1)
                    .Col = ColDept
                    mDeptCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "DEPT_CODE", "DEPT_CODE", "PAY_DEPT_MST", pDBCn, mDeptCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mDeptCode, -1)
                    .Col = ColDivisionCode
                    mDivisionCode = IIf(MainClass.ValidateWithMasterTable(Val(SprdMain.Text), "DIV_CODE", "DIV_CODE", "INV_DIVISION_MST", pDBCn, mDivisionCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mDivisionCode, -1)
                    .Col = ColEmp
                    mEmpCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "EMP_CODE", "EMP_CODE", "PAY_EMPLOYEE_MST", pDBCn, mEmpCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mEmpCode, -1)
                    .Col = ColIBRNo
                    mIBRNo = .Text
                    .Col = ColSAC
                    If MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "HSN_CODE", "HSN_CODE", "GEN_HSN_MST", pDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CODETYPE='S'") = True Then
                        mSAC = .Text
                    Else
                        mSAC = ""
                    End If
                    ...
                    'End If
                    If MainClass.ValidateWithMasterTable(mAccountCode, "SUPP_CUST_CODE", "HEADTYPE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
                        If MasterNo = "L" Then
...
ErrDetail:
    ...
    '                    & " AND TRN.COMPANY_CODE<>" & Val(lblCompanyCode.Text) & " AND TB.UserID='" & PubUserID & "'" & vbCrLf _
    ...
    '                    & " AND TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf _
    ...
    '                    mBillCompanyCode = IIf(mBillCompanyCode <= 0, Val(lblCompanyCode.Text), mBillCompanyCode)
    ...
    '                        & " " & Val(lblCompanyCode.Text) & ", '" & VB.Left(lblBookType.Text, 1) & "'," & mBillCompanyCode & " ) "
    '                    pDBCn.Execute(SqlStr)
    '                    If pTRNType = "N" Then
    '                        pBillType = "B"
    '                    ElseIf pTRNType = "B" Or pTRNType = "O" Or pTRNType = "A" Then
    ...
    Private Function UpdateAutoJvMain() As Boolean
        ...
        'txtNarration.Text = Trim(Replace(txtNarration.Text, vbCrLf, ""))
        'SqlStr = "SELECT MAX(VNOSeq) From FIN_VOUCHER_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND BookType='" & mBookType & "'" & vbCrLf & " AND BookSubType='" & mBookSubType & "'" & vbCrLf & " AND " & vbCrLf & " VTYPE='" & MainClass.AllowSingleQuote(mVType) & "'"
        ...
        With SprdMain
            For I = 1 To .MaxRows
                ...
                If mAccountName <> "" Then
                    ...
                    mDeptCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "DEPT_CODE", "DEPT_CODE", "PAY_DEPT_MST", pDBCn, mDeptCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mDeptCode, -1)
                    .Col = ColDivisionCode
                    mDivisionCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "DIV_CODE", "DIV_CODE", "INV_DIVISION_MST", pDBCn, mDivisionCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mDivisionCode, -1)
                    .Col = ColEmp
                    mEmpCode = IIf(MainClass.ValidateWithMasterTable(Trim(SprdMain.Text), "EMP_CODE", "EMP_CODE", "PAY_EMPLOYEE_MST", pDBCn, mEmpCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True, mEmpCode, -1)
                    .Col = ColIBRNo
                    mIBRNo = .Text
                    .Col = ColClearDate
                    mClearDate = .Text
                    SqlStr = "INSERT INTO FIN_VOUCHER_DET ( " & vbCrLf & " COMPANYCODE, MKey,PRROWNO,SubRowNo,DC,AccountCode, " & vbCrLf & " ChequeNo,ChqDate,CostCCode, " & vbCrLf & " DeptCode,EmpCode,EXP_CODE,IBRNo,Amount,ClearDate,PARTICULARS,DIV_CODE )" & vbCrLf & " VALUES ( " & Val(lblCompanyCode.Text) & ",  " & vbCrLf & " '" & mMkey & "', " & mPRRowNo & ", " & vbCrLf & " " & mSubRowNo & ",'" & mDC & "', '" & mAccountCode & "'," & vbCrLf & " '" & MainClass.AllowSingleQuote(mChequeNo) & "'," & vbCrLf & " TO_DATE('" & VB6.Format(mChqDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf & " '" & mCCCode & "', '" & mDeptCode & "', '" & mEmpCode & "', '" & mExpCode & "', " & vbCrLf & " '" & MainClass.AllowSingleQuote(mIBRNo) & "'," & mAmount & "," & vbCrLf & " TO_DATE('" & VB6.Format(mClearDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'),'" & MainClass.AllowSingleQuote(mParticulars) & "'," & mDivisionCode & " )"
                    PubDBCn.Execute(SqlStr)
        ...
        If RsTempPRDetail.EOF = False Then
            ...
            Do While RsTempPRDetail.EOF = False
                ...
                SqlStr = SqlStr & vbCrLf _
                    ...
                    & " '" & mLocCode & "', " & Val(lblCompanyCode.Text) & ",'" & VB.Left(lblBookType.Text, 1) & "'," & Val(lblCompanyCode.Text) & ") "
                pDBCn.Execute(SqlStr)
                If pTRNType = "N" Then
                    pBillType = "B"
                ElseIf pTRNType = "B" Or pTRNType = "O" Or pTRNType = "A" Then
...
ErrDetail:
    ...
    Private Function UpdateFixedAsset(ByRef mMkey As String, ByRef mVNo As String, ByRef mVDate As String, ByRef xAccountCode As String, ByRef mAmount As Double, ByRef mDC As String, ByRef pNarration As String) As Boolean
        ...
        If MainClass.ValidateWithMasterTable(xAccountCode, "ACCOUNTPOSTCODE", "CODE", "FIN_INVTYPE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND CATEGORY='P'") = True Then
            mTRNType = MasterNo
        Else
            mTRNType = "-1"
        End If
        With SprdMain
            For I = 1 To .MaxRows
                ...
                If mAccountName <> "" Then
                    ...
                    .Col = ColDC
                    mDC = UCase(VB.Left(.Text, 1))
                    mSuppCode = IIf(MainClass.ValidateWithMasterTable(mAccountName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, mSuppCode, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND SUPP_CUST_TYPE IN ('S','C')") = True, mSuppCode, -1)
...
ErrPart:
    ...
    Private Function CheckVType() As Boolean
        ...
        End If
        CheckVType = False
        SqlStr = "SELECT VTYPE,VNAME FROM FIN_VOUCHERTYPE_MST  " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND BOOKTYPE='" & VB.Left(lblBookType.Text, 1) & "'" & vbCrLf & " AND FOR_HO='H'" & vbCrLf & " AND VTYPE='" & MainClass.AllowSingleQuote(Trim(txtVType.Text)) & "'" & vbCrLf & " ORDER BY VTYPE"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RS, ADODB.LockTypeEnum.adLockReadOnly)
        If RS.EOF = False Then
            CheckVType = True
        End If
...
ErrPart:
    ...
    Private Function GetVType(Optional ByRef mChkVtype As String = "") As String
        ...
        Dim SqlStr As String = ""
        Dim RS As ADODB.Recordset = Nothing
        SqlStr = "SELECT VTYPE,VNAME FROM FIN_VOUCHERTYPE_MST  " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & ""
...
ErrPart:
    ...
    Private Sub SearchVType()
        On Error GoTo ErrPart
        Dim SqlStr As String = ""
        SqlStr = " COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND BOOKTYPE='" & VB.Left(lblBookType.Text, 1) & "'"
        ...
        If RsTemp.EOF = False Then
            Do While RsTemp.EOF = False
                ...
                If RS.EOF = False Then
                    ...
                    Loop
                End If
                SqlStr = " SELECT DISTINCT TRN.VNO,VTYPE " & vbCrLf & " FROM FIN_POSTED_TRN TRN " & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf & " AND TRN.BookType ='" & ConCreditNoteBook & "'" & vbCrLf & " AND ACCOUNTCODE='" & MainClass.AllowSingleQuote(lAccountCode) & "' " & vbCrLf & " AND TRN.BILLNO='" & Trim(mBillNo) & "'" & vbCrLf & " AND TRN.BILLDATE =TO_DATE('" & VB6.Format(mBillDate, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RS, ADODB.LockTypeEnum.adLockReadOnly)
                If RS.EOF = False Then
                    Do While RS.EOF = False
                        mDrVno = IIf(IsDBNull(RS.Fields("VNO").Value), "", Mid(RS.Fields("VNO").Value, 3))
...
ErrPart:
    ...
    Private Function GETDNCNNoInVoucher() As String
        ...
        SqlStr = " SELECT DISTINCT TRN.VNO||':'||TRN.FYEAR AS VNO" & vbCrLf & " FROM FIN_POSTED_TRN TRN " & vbCrLf & " WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & " " & vbCrLf & " AND (TRN.BookType='" & ConDebitNoteBook & "' " & vbCrLf & " OR TRN.BookType='" & ConCreditNoteBook & "') AND " & vbCrLf & " AccountCode='" & lAccountCode & "' AND TRN.BILLNO||TRN.BILLDATE IN (SELECT BILLNO||BILLDATE FROM FIN_POSTED_TRN" & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND " & vbCrLf & " FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND  " & vbCrLf & " VNO='" & mVNo & "' AND " & vbCrLf & " VDate=TO_DATE('" & VB6.Format(TxtVDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') AND " & vbCrLf & " BookType='" & Mid(lblBookType.Text, 1, 1) & "' AND " & vbCrLf & " BookSubType='" & Mid(lblBookType.Text, 2, 1) & "' AND " & vbCrLf & " BOOKCODE='" & mBookCode & "' AND " & vbCrLf & " AccountCode<>'" & mBookCode & "') "
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RS, ADODB.LockTypeEnum.adLockReadOnly)
        If RS.EOF = False Then
            Do While RS.EOF = False
    ...
    Private Function UpDateSuppBill(ByRef xVnoStr As String) As Boolean
        ...
        With SprdMain
            ...
            For cntRow = 1 To .MaxRows
                ...
                If MainClass.ValidateWithMasterTable(mPartyName, "SUPP_CUST_NAME", "WITHIN_STATE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND WITHIN_STATE='N' AND SUPP_CUST_TYPE IN ('S','C')") = True Then
                    mISCST = "Y"
                    Exit For
                End If
            Next
        ...
        If xModvatNo <> 0 Then
            SqlStr = " SELECT * FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND MODVATNO=" & xModvatNo & " AND ISCAPITAL='" & pISCapital & "'" & vbCrLf & " AND ISMODVAT='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'"
            MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
            If RsTemp.EOF = True Then
                ...
                Exit Function
            End If
            SqlStr = " SELECT JVNO FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND MODVATNO=" & xModvatNo & " AND ISCAPITAL='" & pISCapital & "'" & vbCrLf & " AND ISMODVAT='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'" & vbCrLf & " AND JVNO<>'" & pVNoStr & "' AND JVNO IS NOT NULL"
            MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
            If RsTemp.EOF = False Then
                ...
                Exit Function
            End If
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='" & mFinalPost & "',JVNO='" & xVnoStr & "'," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND MODVATNO=" & xModvatNo & " AND ISCAPITAL='" & pISCapital & "'" & vbCrLf & " AND ISMODVAT='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'"
            PubDBCn.Execute(SqlStr)
        End If
        If xServNo <> 0 Then
            SqlStr = " SELECT * FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND SERVNO=" & xServNo & " AND SERVICE_REFUND='" & xIsServTaxRefund & "'" & vbCrLf & " AND ISSERVCLAIM='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'"
            MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
            If RsTemp.EOF = True Then
                MsgInformation("Not Such Modvat Entry.")
                UpDateSuppBill = False
                Exit Function
            End If
            SqlStr = " SELECT JVNO FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND SERVNO=" & xServNo & " AND SERVICE_REFUND='" & xIsServTaxRefund & "'" & vbCrLf & " AND ISSERVCLAIM='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'" & vbCrLf & " AND JVNO<>'" & pVNoStr & "' AND JVNO IS NOT NULL"
            MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
            If RsTemp.EOF = False Then
                mPrevJVNo = IIf(IsDBNull(RsTemp.Fields("JVNO").Value), "", RsTemp.Fields("JVNO").Value)
                MsgInformation("Modvat Entry Already Save Agt. JVno " & mPrevJVNo)
                ...
                Exit Function
            End If
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='" & mFinalPost & "',JVNO='" & xVnoStr & "'," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND SERVNO=" & xServNo & " AND SERVICE_REFUND='" & xIsServTaxRefund & "'" & vbCrLf & " AND ISSERVCLAIM='Y' AND ISPLA='" & xIsPLA & "' AND VNO='-1'"
            PubDBCn.Execute(SqlStr)
        End If
        If xSTRefundNo <> 0 Then
            SqlStr = " SELECT * FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND STCLAIMNO=" & xSTRefundNo & " AND VNO='-1' "
            If mISCST = "N" Then
            ...
            If RsTemp.EOF = True Then
                ...
                Exit Function
            End If
            SqlStr = " SELECT JVNO FROM FIN_PURCHASE_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND STCLAIMNO=" & xSTRefundNo & " AND VNO='-1' " & vbCrLf & " AND JVNO<>'" & pVNoStr & "' AND JVNO IS NOT NULL"
            If mISCST = "N" Then
                SqlStr = SqlStr & " AND ISSTREFUND='Y'"
            Else
                SqlStr = SqlStr & " AND ISCSTREFUND='Y'"
            ...
            If RsTemp.EOF = False Then
                ...
                Exit Function
            End If
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='" & mFinalPost & "',JVNO='" & xVnoStr & "'," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND STCLAIMNO=" & xSTRefundNo & " AND VNO='-1' "
            If mISCST = "N" Then
                SqlStr = SqlStr & " AND ISSTREFUND='Y'"
            Else
                SqlStr = SqlStr & " AND ISCSTREFUND='Y'"
...
ErrPart:
    ...
    Private Function ClearJVNo(ByRef mVnoStr As String) As Boolean
        ...
        If xPrevModvatNo <> 0 Then
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND MODVATNO=" & xPrevModvatNo & " AND ISCAPITAL='" & xPrevISCapital & "' AND JVNO='" & mVnoStr & "'" & vbCrLf & " AND ISMODVAT='Y' AND ISPLA='" & xPrevISPLA & "' AND VNO='-1'"
            PubDBCn.Execute(SqlStr)
        End If
        If xPrevSTRefundNo <> 0 Then
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND ISSTREFUND='Y'" & vbCrLf & " AND STCLAIMNO=" & xPrevSTRefundNo & "  AND JVNO='" & mVnoStr & "' AND VNO='-1' "
            PubDBCn.Execute(SqlStr)
        End If
        If xPrevServNo <> 0 Then
            SqlStr = "UPDATE FIN_PURCHASE_HDR SET ISFINALPOST='N',JVNO=''," & vbCrLf & " UPDATE_FROM='H'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "',Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND ISSERVCLAIM='Y'" & vbCrLf & " AND SERVNO=" & xPrevServNo & "  AND ISPLA='" & xPrevISPLA & "' AND SERVICE_REFUND='" & xPrevServTaxRefund & "' AND JVNO='" & mVnoStr & "' AND VNO='-1' "
            PubDBCn.Execute(SqlStr)
        End If
        ClearJVNo = True
        Exit Function
ErrPart:
    ...
    Private Sub SearchExpPartyName()
        On Error GoTo SearchErr
        Dim SqlStr As String = ""
        SqlStr = "COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND SUPP_CUST_TYPE IN ('S','C') AND STATUS='O'"
...
SearchErr:
    ...
    Private Function GetTDSChallanMade(ByRef pMKey As String, ByRef pChallanNo As String) As Boolean
        ...
        SqlStr = " SELECT CHALLAN.REFNO " & vbCrLf & "  FROM TDS_TRN TRN, TDS_CHALLAN CHALLAN" & vbCrLf & "  WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & "  AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & "  AND TRN.COMPANY_CODE=CHALLAN.COMPANY_CODE" & vbCrLf & "  AND TRN.FYEAR=CHALLAN.FYEAR" & vbCrLf & "  AND TRN.CHALLANMKEY=CHALLAN.MKEY" & vbCrLf & "  AND TRN.MKEY='" & pMKey & "'"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If Not RsTemp.EOF Then
...
ErrPart:
    ...
    Private Function GetServiceClaimMade(ByRef pMKey As String, ByRef pClaimNo As String) As Boolean
        ...
        SqlStr = " SELECT SERVNO " & vbCrLf & "  FROM FIN_SERVTAXDETAILS_TRN TRN" & vbCrLf & "  WHERE TRN.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & "  AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND ISSERVICECLAIM='Y'" & vbCrLf & "  AND TRN.MKEY='" & pMKey & "'"
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        If Not RsTemp.EOF Then
...
ErrPart:
    ...
    Private Function GetImpMRRNo(ByRef pMRRNo As Double, ByRef pVNo As String) As Boolean
        ...
        Dim RsTemp As ADODB.Recordset = Nothing
        GetImpMRRNo = False
        SqlStr = " SELECT VNO " & vbCrLf & "  FROM FIN_VOUCHER_HDR IH" & vbCrLf & "  WHERE IH.COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & "  AND IH.IMP_MRR_NO=" & Val(CStr(pMRRNo)) & ""
        If MODIFYMode = True Then
            SqlStr = SqlStr & vbCrLf & " AND MKEY<>" & RsTRNMain.Fields("MKEY").Value & ""
        End If
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
...
ErrPart:
    ...
    Private Function CheckValidPartyPanNo(ByRef pPartyName As String) As Boolean
        ...
        If MainClass.ValidateWithMasterTable(pPartyName, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            xAccountCode = MasterNo
        Else
            CheckValidPartyPanNo = False
            Exit Function
        End If
        If MainClass.ValidateWithMasterTable(xAccountCode, "SUPP_CUST_CODE", "WITHIN_COUNTRY", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            If Trim(MasterNo) = "N" Then
                CheckValidPartyPanNo = True
                Exit Function
            End If
        End If
        If MainClass.ValidateWithMasterTable(xAccountCode, "SUPP_CUST_CODE", "SUPP_CUST_TYPE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
        ...
        Else
            ...
            Exit Function
        End If
        If MainClass.ValidateWithMasterTable(xAccountCode, "SUPP_CUST_CODE", "PAN_NO", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & Val(lblCompanyCode.Text) & "") = True Then
            mPANNo = MasterNo
            If Trim(mPANNo) <> "" Then
        ...
        Else
            ...
            Exit Function
        End If
        mSqlStr = " Select A.BILLNO, A.BILLDATE " & vbCrLf & " FROM FIN_TEMPBILL_TRN A, FIN_POSTED_TRN B, TDS_TRN C  " & vbCrLf & " WHERE B.COMPANY_CODE=" & Val(lblCompanyCode.Text) & " AND B.FYEAR=" & RsCompany.Fields("FYEAR").Value & " AND A.UserID='" & PubUserID & "' AND TEMPMKEY=" & pProcessKey & "" & vbCrLf & " AND A.AccountCode='" & xAccountCode & "'" & vbCrLf & " AND A.BookType='" & lblBookType.Text & "'" & vbCrLf & " AND A.BILLNO=B.BILLNO" & vbCrLf & " AND A.BILLDATE=B.BILLDATE" & vbCrLf & " AND A.ACCOUNTCODE=B.ACCOUNTCODE" & vbCrLf & " AND B.COMPANY_CODE=C.COMPANY_CODE AND B.MKEY=C.MKEY" & vbCrLf & " AND B.BOOKTYPE=C.BOOKTYPE AND B.BOOKSUBTYPE=C.BOOKSUBTYPE"
...
ErrPart:
    ...
    Private Sub chkESI_CheckStateChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles chkESI.CheckStateChanged
        ...
        If chkESI.CheckState = System.Windows.Forms.CheckState.Checked Then
            ...
            If Val(txtESIRate.Text) = 0 Then
                SqlStr = "SELECT ESI_PER FROM FIN_SUPP_CUST_MST " & vbCrLf & " WHERE COMPANY_CODE=" & Val(lblCompanyCode.Text) & "" & vbCrLf & " AND SUPP_CUST_NAME='" & MainClass.AllowSingleQuote(mAccountName) & "'"
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
                If RsTemp.EOF = False Then
