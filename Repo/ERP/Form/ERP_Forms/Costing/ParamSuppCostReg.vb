...
Friend Class frmParamSuppCostReg
    ...
    Private Const ColGrossWt As Short = 7
    Private Const ColScrapWt As Short = 8
    Private Const ColNetWt As Short = 9

    Private Const ColGrossRMCost As Short = 10
    Private Const ColScrapRMCost As Short = 11
    Private Const ColNetRMCost As Short = 12
    Private Const ColBOPCost As Short = 13
    Private Const ColProcessACost As Short = 14
    Private Const ColProcessBCost As Short = 15
    Private Const ColOtherCost As Short = 16
    Private Const ColOperationCost As Short = 17
    Private Const ColToolCost As Short = 18
    Private Const ColToolQty As Short = 19
    Private Const ColToolPcCost As Short = 20
    Private Const ColNetCost As Short = 21
    Private Const ColPONO As Short = 22
    Private Const ColPODate As Short = 23
    Private Const ColPOWEF As Short = 24
    Private Const ColPORate As Short = 25
    Private Const ColDiff As Short = 26
    ...
    Private Sub ReportOnPrint(ByRef Mode As Crystal.DestinationConstants)
        ...
        mTitle = "Supplier Costing Register"
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamSuppCostReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
    ...
    Private Sub frmParamSuppCostReg_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmParamSuppCostReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamSuppCostReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamSuppCostReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
        Me.Dispose()
...
EventExitSub:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For cntCol = ColGrossWt To ColNetCost
                .Col = cntCol
...
ErrPart:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT  " & vbCrLf & " IH.SUPP_CUST_CODE, CMST.SUPP_CUST_NAME, " & vbCrLf & " IH.ITEM_CODE, IMST.ITEM_SHORT_DESC, IMST.CUSTOMER_PART_NO, " & vbCrLf & " TO_CHAR(IH.WEF,'DD/MM/YYYY') AS WEF,ITEM_GROSS_WT,ITEM_SCRAP_WT,ITEM_NET_WT, IH.RM_GROSSCOST,SCRAP_COST, IH.RM_NETCOST,IH.PART_COST, " & vbCrLf & " PROCESS_A_COST, PROCESS_B_COST, OTHERCHARGES, " & vbCrLf & " OPR_COST, TOOL_COST,TOOL_QTY,TOOL_COST_PER_PC,NET_COST,'','','',0,0 "

        ''''FROM CLAUSE...		
        MakeSQL = MakeSQL & vbCrLf & " FROM PRD_BOP_COST_HDR IH, FIN_SUPP_CUST_MST CMST, INV_ITEM_MST IMST"

        ''''WHERE CLAUSE...		
        MakeSQL = MakeSQL & vbCrLf & " WHERE IH.COMPANY_CODE =" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE" & vbCrLf & " AND IH.COMPANY_CODE=IMST.COMPANY_CODE" & vbCrLf & " AND IH.ITEM_CODE=IMST.ITEM_CODE"
        ...
        MakeSQL = MakeSQL & vbCrLf & " AND WEF = (SELECT MAX(A.WEF) FROM PRD_BOP_COST_HDR A" & vbCrLf & " WHERE A.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND A.COMPANY_CODE=CMST.COMPANY_CODE " & vbCrLf & " AND A.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE" & vbCrLf & " AND A.COMPANY_CODE=IMST.COMPANY_CODE" & vbCrLf & " AND A.ITEM_CODE=IMST.ITEM_CODE " & vbCrLf & " AND A.WEF <= TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD/MMM/YYYY") & "','DD-MON-YYYY')) "

        '    MakeSQL = MakeSQL & vbCrLf _		
        ''            & " GROUP BY SUPP_CUST_CODE, SUPP_CUST_NAME, " & vbCrLf _		
        ''            & " ITEM_CODE, ITEM_SHORT_DESC, CUSTOMER_PART_NO"		

        MakeSQL = MakeSQL & vbCrLf & " ORDER BY CMST.SUPP_CUST_NAME,IH.ITEM_CODE "
        Exit Function
ERR1:
        MsgInformation(Err.Description)
    End Function
