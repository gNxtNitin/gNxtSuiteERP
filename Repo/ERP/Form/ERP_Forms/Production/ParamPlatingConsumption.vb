...
Friend Class frmParamPlatingConsumption
    ...
    Private Const ColPlatingCons As Short = 14
    Private Const ColTotalCons As Short = 15
...
ErrPart:
    ...
    Private Sub frmParamPlatingConsumption_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPlatingConsumption_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamPlatingConsumption_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPlatingConsumption_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles MyBase.FormClosed
        FormActive = False
        Me.Hide()
        Me.Dispose()
        Me.Close()
...
LedgError:
    ...
    Private Function MakeSQL(mType As String) As String
        ...
        MakeSQL = " SELECT ''," & vbCrLf & " IGH.AUTO_KEY_GATE," & vbCrLf & " TO_CHAR(IGH.GATE_DATE,'DD/MM/YYYY')," & vbCrLf _
            & " IGH.MRR_NO," & vbCrLf & " TO_CHAR(IGH.MRRDATE,'DD/MM/YYYY')," & vbCrLf & " SCMST.SUPP_CUST_NAME," & vbCrLf _
            & " CMST.SUPP_CUST_NAME," & vbCrLf _
            & " IGH.BILL_NO , IGH.BILL_DATE, " & vbCrLf _
            & " IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC," & vbCrLf _
            & " IGD.ITEM_UOM, TO_CHAR(IGD.BILL_QTY), " & vbCrLf _
            & " (SELECT BUFFING_RATE FROM INV_BUFFINGITEM_TRN A WHERE A.COMPANY_CODE=IGH.COMPANY_CODE AND A.ITEM_CODE=IGD.ITEM_CODE " & vbCrLf _
            & " AND A.WEF_DATE = (SELECT MAX(WEF_DATE) FROM INV_BUFFINGITEM_TRN WHERE COMPANY_CODE=A.COMPANY_CODE AND ITEM_CODE=A.ITEM_CODE AND WEF_DATE<=IGH.GATE_DATE)) AS CONS_PER_UNIT," & vbCrLf _
            & " IGD.BILL_QTY * (SELECT BUFFING_RATE FROM INV_BUFFINGITEM_TRN A WHERE A.COMPANY_CODE=IGH.COMPANY_CODE AND A.ITEM_CODE=IGD.ITEM_CODE " & vbCrLf _
            & " AND A.WEF_DATE = (SELECT MAX(WEF_DATE) FROM INV_BUFFINGITEM_TRN WHERE COMPANY_CODE=A.COMPANY_CODE AND ITEM_CODE=A.ITEM_CODE AND WEF_DATE<=IGH.GATE_DATE)) AS CONS_PER_UNIT," & vbCrLf _
            & " IGH.AUTO_KEY_GATE"
        ...
        MakeSQL = MakeSQL & vbCrLf & " FROM INV_GATEENTRY_HDR IGH, INV_GATEENTRY_DET IGD," & vbCrLf _
            & " FIN_SUPP_CUST_MST CMST, FIN_SUPP_CUST_MST SCMST, INV_ITEM_MST INVMST"
        ...
        MakeSQL = MakeSQL & vbCrLf & "AND INVMST.ITEM_CODE  IN (SELECT DISTINCT ITEM_CODE FROM  INV_BUFFINGITEM_TRN WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & ")"
...
EventExitSub:
    ...
    Private Sub CreateGridHeader()
        ...
        Try
            ...
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColPlatingCons - 1).Header.Caption = "Nickel Consumption / Unit (In Grams)"
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColTotalCons - 1).Header.Caption = "Total Nickel Consumption (In Grams)"
            ...
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColPlatingCons - 1).Style = UltraWinGrid.ColumnStyle.Double
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColTotalCons - 1).Style = UltraWinGrid.ColumnStyle.Double


            UltraGrid1.DisplayLayout.Bands(0).Columns(ColQty - 1).CellAppearance.TextHAlign = HAlign.Right
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColPlatingCons - 1).CellAppearance.TextHAlign = HAlign.Right
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColTotalCons - 1).CellAppearance.TextHAlign = HAlign.Right
            ...
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColPlatingCons - 1).Width = 100
            UltraGrid1.DisplayLayout.Bands(0).Columns(ColTotalCons - 1).Width = 100
    ...
    Private Sub UltraGrid1_InitializeLayout(sender As Object, e As InitializeLayoutEventArgs) Handles UltraGrid1.InitializeLayout
        ...
        e.Layout.Bands(0).Summaries.Add("ColTotalCons", SummaryType.Sum, e.Layout.Bands(0).Columns(ColTotalCons - 1))


        ''Set the display format to be just the number 
        e.Layout.Bands(0).Summaries("ColQty").DisplayFormat = "{0:###0.00}"
        e.Layout.Bands(0).Summaries("ColTotalCons").DisplayFormat = "{0:###0.00}"
        ...
        e.Layout.Bands(0).Summaries("ColTotalCons").Appearance.TextHAlign = HAlign.Right
