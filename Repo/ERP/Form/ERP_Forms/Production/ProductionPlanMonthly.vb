...
Friend Class frmProductionPlanMonthly
    Inherits System.Windows.Forms.Form
    Dim RsProdPlanMain As ADODB.Recordset
    Dim RsProdPlanDetail As ADODB.Recordset
    ...
    Dim mCopyQty As Integer

    Private Const ConRowHeight As Short = 18

    Private Const ColDate As Short = 1
    Private Const ColIPlanQty1 As Short = 2
    Private Const ColDPlanQty1 As Short = 3
    Private Const ColProdLoss1 As Short = 4
    Private Const ColPlanStart1 As Short = 5

    Private Function GetSchdQty(ByVal pItemCode As String, ByVal pPlanDate As String) As Double
        ...
        SqlStr = " SELECT SUM(ITEM_QTY) AS ITEM_QTY " & vbCrLf _
            ...
            & " AND To_CHAR(IH.SCHLD_DATE,'MON-YYYY')='" & UCase(Format(pPlanDate, "MMM-YYYY")) & "'"


        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)

        If RsTemp.EOF = False Then
            GetSchdQty = IIf(IsDBNull(RsTemp.Fields("ITEM_QTY").Value), 0, RsTemp.Fields("ITEM_QTY").Value)
        End If
        Exit Function
ErrPart:
        GetSchdQty = 0
    ...
    Private Sub cmdAdd_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdAdd.Click
        ...
        Else
            ADDMode = False
            MODIFYMode = False
            If RsProdPlanMain.EOF = False Then RsProdPlanMain.MoveFirst()
            Clear1()
            Show1()
        End If
        Exit Sub
AddErr:
    ...
    Private Sub PopulateMode(ByRef mMode As Boolean)
        cmdSched.Enabled = mMode
        CmdDaily.Enabled = mMode
        cmdOnceWeek.Enabled = mMode
        cmdTwiceWeek.Enabled = mMode
        cmdTwiceMonth.Enabled = mMode
    End Sub
    ...
    Private Sub CmdDaily_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDaily.Click
        ...
        SqlStr = " SELECT COUNT(1) AS TOT_HOLIDAYS FROM PAY_HOLIDAY_MST " & vbCrLf _
            ...
            & " AND TO_CHAR(HOLIDAY_DATE,'MMYYYY')='" & VB6.Format(txtPlanDate.Text, "MMYYYY") & "'"
        ...
        End If
        mWorkingdays = MainClass.LastDay(Month(CDate(txtPlanDate.Text)), Year(CDate(txtPlanDate.Text))) - mHolidays
        mSchdQty = Val(txtPlanningQty.Text) / mWorkingdays
        mBalanceSchQty = Val(txtPlanningQty.Text) - (mSchdQty * (mWorkingdays - 1))
        ...
        With SprdMain
            For mRow = 1 To .MaxRows
                ...
                'mBalanceSchQty = mBalanceSchQty - Val(mSchdQty)

                If IsHoliday(mSerialDate) = False Then
                    For mCol = 2 To .MaxCols
                        .Col = mCol

                        If mRow = .MaxRows Then
                            .Text = CStr(mBalanceSchQty)
                        Else
                            .Text = CStr(mSchdQty)
                        End If
...
ErrPart:
    ...
    Private Function IsHoliday(ByVal pDate As String) As Boolean
...
LedgError:
    ...
    Private Sub CmdDelete_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdDelete.Click
        ...
        If Not RsProdPlanMain.EOF Then
            ...
            If MsgQuestion("Want to Delete ? ") = CStr(MsgBoxResult.Yes) Then
                PubDBCn.Cancel()
                PubDBCn.BeginTrans()
                If InsertIntoDelAudit(PubDBCn, "PRD_PRODPLAN_HDR", (txtPlanNo.Text), RsProdPlanMain) = False Then GoTo DelErrPart
...
ModifyErr:
    ...
    Private Sub cmdOnceWeek_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdOnceWeek.Click
...
AutogenErr:
    ...
    Private Sub cmdSched_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSched.Click
...
ErrPart:
    ...
    Private Sub cmdTwiceMonth_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdTwiceMonth.Click
        Call FillQty(2, 14, , False)
        'txtPlanningQty.Enabled = False
    End Sub

    Private Sub cmdTwiceWeek_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdTwiceWeek.Click
    ...
    Private Sub frmProductionPlanMonthly_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ...
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsProdPlanDetail, ADODB.LockTypeEnum.adLockReadOnly)
...
ERR1:
    ...
    Private Sub frmProductionPlanMonthly_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmProductionPlanMonthly_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For I = 2 To .MaxCols
                ...
                .TypeEditLen = RsProdPlanMonDetail.Fields("PROD_LOSS").DefinedSize
                ...
                .CellType = SS_CELL_TYPE_EDIT
                .TypeEditCharSet = SS_CELL_EDIT_CHAR_SET_ASCII
                .TypeEditLen = RsProdPlanMonDetail.Fields("PLAN_START").DefinedSize
                .TypeEditCharCase = SS_CELL_EDIT_CASE_UPPER_CASE
                .TypeEditMultiLine = True
                .ColHidden = True
...
ERR1:
    ...
    Private Sub SetTextLengths()
        ...
        txtPlanNo.MaxLength = RsProdPlanMain.Fields("AUTO_KEY_PRODPLAN").Precision
        txtProductCode.MaxLength = RsProdPlanMain.Fields("PRODUCT_CODE").DefinedSize
        txtPlanDate.MaxLength = RsProdPlanMain.Fields("SCHLD_DATE").Precision - 6
        txtPlanningQty.MaxLength = RsProdPlanMain.Fields("CUST_ORD_QTY").Precision
        txtRemarks.MaxLength = RsProdPlanMain.Fields("REMARKS").DefinedSize
...
FillERR:
    ...
    Private Function FieldsVarification() As Boolean
        ...
        If ADDMode = False And MODIFYMode = False And FieldsVarification Then
            ...
            Exit Function
        End If
        If MODIFYMode = True And RsProdPlanMain.EOF = True Then Exit Function
...
err_Renamed:
    ...
    Private Sub frmProductionPlanMonthly_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        RsProdPlanMain.Close()
        RsProdPlanMain = Nothing
        RsProdPlanDetail.Close()
        RsProdPlanDetail = Nothing
    ...
    Private Sub txtPlanningQty_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtPlanningQty.TextChanged
        MainClass.SaveStatus(Me.CmdSave, ADDMode, MODIFYMode)
    End Sub

    Private Sub txtPlanningQty_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles txtPlanningQty.KeyPress
        Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
...
EventExitSub:
    ...
    Private Sub txtPlanningQty_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtPlanningQty.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        'If PubSuperUser <> "S" Then
...
EventExitSub:
    ...
    Private Sub Show1()
        ...
        If Not RsProdPlanMain.EOF Then
            ...
            txtPlanNo.Text = IIf(IsDbNull(RsProdPlanMain.Fields("AUTO_KEY_PRODPLAN").Value), "", RsProdPlanMain.Fields("AUTO_KEY_PRODPLAN").Value)
            txtProductCode.Text = IIf(IsDBNull(RsProdPlanMain.Fields("PRODUCT_CODE").Value), "", RsProdPlanMain.Fields("PRODUCT_CODE").Value)
            txtProductCode_Validating(txtProductCode, New System.ComponentModel.CancelEventArgs(False))
            txtPlanDate.Text = IIf(IsDbNull(RsProdPlanMain.Fields("SCHLD_DATE").Value), "", RsProdPlanMain.Fields("SCHLD_DATE").Value)
            txtPlanningQty.Text = IIf(IsDBNull(RsProdPlanMain.Fields("CUST_ORD_QTY").Value), "", RsProdPlanMain.Fields("CUST_ORD_QTY").Value)

            mLastDate = GetWorkingDays(txtPlanDate.Text)  ''MainClass.LastDay(Month(txtPlanDate.Text), Year(txtPlanDate.Text))
            txtCapacity.Text = GetLineCapacityQty(txtProductCode.Text, "", txtPlanDate.Text) * mLastDate

            txtRemarks.Text = IIf(IsDbNull(RsProdPlanMain.Fields("REMARKS").Value), "", RsProdPlanMain.Fields("REMARKS").Value)
            chkStatus.CheckState = IIf(IsDbNull(RsProdPlanMain.Fields("PLAN_STATUS").Value) Or RsProdPlanMain.Fields("PLAN_STATUS").Value = "N", System.Windows.Forms.CheckState.Unchecked, System.Windows.Forms.CheckState.Checked)
