...
Friend Class frmParamSalesDSReg
    ...
    Private Const ColItemDesc As Short = 9
    Private Const ColItemPartNo As Short = 10
    Private Const ColUnit As Short = 11
    Private Const ColWeek1Qty As Short = 12
    Private Const ColWeek2Qty As Short = 13
    Private Const ColWeek3Qty As Short = 14
    Private Const ColWeek4Qty As Short = 15
    Private Const ColWeek5Qty As Short = 16
    Private Const ColTotQty As Short = 17
    Private Const ColRate As Short = 18
    Private Const ColValue As Short = 19
    Private Const ColValueN2 As Short = 20
    ...
    Private Sub ReportonPO(ByRef Mode As Crystal.DestinationConstants)
        ...
        mTitle = "Sales Delivery Schedule Register"
    ...
    Private Sub DisplayTotals()
        ...
        With SprdMain
            ...
            For cntRow = 1 To .MaxRows
                ...
                .Col = ColWeek1Qty
                mWeek1Qty = mWeek1Qty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColWeek2Qty
                mWeek2Qty = mWeek2Qty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColWeek3Qty
                mWeek3Qty = mWeek3Qty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColWeek4Qty
                mWeek4Qty = mWeek4Qty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColWeek5Qty
                mWeek5Qty = mWeek5Qty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColTotQty
                TotQty = TotQty + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColValue
                mValue = mValue + CDbl(IIf(IsNumeric(.Text), .Text, 0))

                .Col = ColValueN2
                mValue1 = mValue1 + CDbl(IIf(IsNumeric(.Text), .Text, 0))
            Next

            Call MainClass.AddBlankfpSprdRow(SprdMain, ColScheduleMonth)
            .Row = .MaxRows
            .Col = colSupplier
            .Text = "TOTAL :"
            ...
            .Row = .MaxRows
            .Col = ColWeek1Qty
            .Text = VB6.Format(mWeek1Qty, "0.00")

            .Col = ColWeek2Qty
            .Text = VB6.Format(mWeek2Qty, "0.00")

            .Col = ColWeek3Qty
            .Text = VB6.Format(mWeek3Qty, "0.00")

            .Col = ColWeek4Qty
            .Text = VB6.Format(mWeek4Qty, "0.00")

            .Col = ColWeek5Qty
            .Text = VB6.Format(mWeek5Qty, "0.00")

            .Col = ColTotQty
            .Text = VB6.Format(TotQty, "0.00")

            .Col = ColValue
            .Text = VB6.Format(mValue, "0.00")

            .Col = ColValueN2
            .Text = VB6.Format(mValue1, "0.00")
...
ERR1:
        ...
        'Resume
    End Sub
    Private Sub frmParamSalesDSReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamSalesDSReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
        ...
        MsgInformation(Err.Description)
    End Sub
    Private Sub frmParamSalesDSReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamSalesDSReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
    ...
    Private Sub SprdMain_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_DblClickEvent) Handles SprdMain.DblClick
        ...
        SqlStr = "SELECT * from DSP_DELV_SCHLD_HDR WHERE AUTO_KEY_DELV=" & xDSNo & "" '' AND DELV_AMEND_NO=" & xAmendPONo & ""
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)

        If RsTemp.EOF = False Then
            frmSalesDS.MdiParent = Me.MdiParent
            frmSalesDS.Show()
            '        frmDS.lblBookType.text = RsTemp!PUR_TYPE & RsTemp!ORDER_TYPE

            frmSalesDS.frmSalesDS_Activated(Nothing, New System.EventArgs())

            frmSalesDS.txtDSNo.Text = RsTemp.Fields("AUTO_KEY_DELV").Value
            '        frmDS.txtAmendNo = RsTemp!AMEND_NO

            frmSalesDS.txtDSNo_Validating(Nothing, New System.ComponentModel.CancelEventArgs(False))
...
EventExitSub:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            .ColHidden = IIf(optSummaryItem.Checked = True, True, False)
            ...
            For cntCol = ColWeek1Qty To ColValueN2
                .Col = cntCol
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        Dim mLastDay As Short
        Dim mDate As String

        mDate = VB6.Format(txtDateTo.Text, "DD/MM/YYYY")


        mLastDay = MainClass.LastDay(Month(CDate(mDate)), Year(CDate(mDate)))
        ...
        If optShow(2).Checked = True Then
            ...
            If optDetail.Checked = True Then
                MakeSQL = " SELECT ''," & vbCrLf _
                        ...
                        & " GetITEMPARTNO(IH.COMPANY_CODE,IH.AUTO_KEY_SO,IH.SO_AMEND_NO,ID.ITEM_CODE) AS PARTNO," & vbCrLf _
                        ...
                        & " TO_CHAR(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* (SUM(PLANNED_QTY + (PLANNED_QTY*2/" & mLastDay & ")))), " & vbCrLf _
                        & " IH.AUTO_KEY_DELV "
            Else
                MakeSQL = " SELECT ''," & vbCrLf _
                       & " ''," & vbCrLf _
                       & " '', TO_CHAR(IH.SCHLD_DATE,'MON-YYYY')," & vbCrLf _
                       & " '',"

                If optSummary.Checked = True Then
                    MakeSQL = MakeSQL & vbCrLf _
                        & " IH.SUPP_CUST_CODE, CMST.SUPP_CUST_NAME,"
                ...
                MakeSQL = MakeSQL & vbCrLf _
                       ...
                       & " TO_CHAR(SUM(PLANNED_QTY)), " & vbCrLf _
                       & " '', " & vbCrLf _
                       & " TO_CHAR(SUM(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* PLANNED_QTY)), " & vbCrLf _
                       & " TO_CHAR(SUM(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* (PLANNED_QTY + (PLANNED_QTY*2/" & mLastDay & ")))), " & vbCrLf _
                       & " '' "
                End If

            ''FROM CLAUSE...
            MakeSQL = MakeSQL & vbCrLf & " FROM DSP_DELV_SCHLD_HDR IH, DSP_DAILY_SCHLD_DET ID," & vbCrLf _
                & " FIN_SUPP_CUST_MST CMST, INV_ITEM_MST INVMST"
        Else
            If optDetail.Checked = True Then
                MakeSQL = " SELECT ''," & vbCrLf & " IH.AUTO_KEY_DELV," & vbCrLf _
                        ...
                        & " TO_CHAR(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* (ITEM_QTY + (ITEM_QTY*2/" & mLastDay & "))), " & vbCrLf _
                        & " IH.AUTO_KEY_DELV "
            Else
                MakeSQL = " SELECT ''," & vbCrLf _
                        & " ''," & vbCrLf _
                ...
                MakeSQL = MakeSQL & vbCrLf _
                        & " TO_CHAR(SUM(WEEK1_QTY)), " & vbCrLf _
                        & " TO_CHAR(SUM(WEEK2_QTY)),TO_CHAR(SUM(WEEK3_QTY)),TO_CHAR(SUM(WEEK4_QTY)), " & vbCrLf _
                        & " TO_CHAR(SUM(WEEK5_QTY)),TO_CHAR(SUM(ITEM_QTY)), " & vbCrLf _
                        & " '', " & vbCrLf _
                        & " TO_CHAR(SUM(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* ITEM_QTY)), " & vbCrLf _
                         & " TO_CHAR(SUM(GetSORATE(IH.COMPANY_CODE,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO,ID.ITEM_CODE)* (ITEM_QTY + (ITEM_QTY*2/" & mLastDay & ")))), " & vbCrLf _
                        & " '' "
            End If
        ...
        If optShow(0).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf & " AND IH.DELV_SCHLD_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IH.DELV_SCHLD_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ElseIf optShow(1).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf & " AND IH.SCHLD_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IH.SCHLD_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        Else
            MakeSQL = MakeSQL & vbCrLf & " AND ID.SERIAL_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND ID.SERIAL_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        End If

        ''ORDER CLAUSE...
        If optDetail.Checked = True Then
            If optShow(2).Checked = True Then
                MakeSQL = MakeSQL & vbCrLf & " GROUP BY" & vbCrLf _
                    & " IH.COMPANY_CODE, IH.AUTO_KEY_DELV,IH.DELV_SCHLD_DATE,IH.AUTO_KEY_SO, IH.SUPP_CUST_CODE," & vbCrLf _
                    & " TO_CHAR(IH.DELV_SCHLD_DATE,'DD/MM/YYYY'), TO_CHAR(ID.SERIAL_DATE,'MON-YYYY')," & vbCrLf _
                    & " DELV_AMEND_NO, IH.SO_AMEND_NO," & vbCrLf _
        ...
        Else
            MakeSQL = MakeSQL & vbCrLf & " GROUP BY" & vbCrLf _
                    & " TO_CHAR(IH.SCHLD_DATE,'MON-YYYY'),"
            ...
            Else
                MakeSQL = MakeSQL & vbCrLf _
                    & " ID.ITEM_CODE, INVMST.ITEM_SHORT_DESC, " & vbCrLf _
                    & " GetITEMPARTNO(IH.COMPANY_CODE,IH.AUTO_KEY_SO,IH.SO_AMEND_NO,ID.ITEM_CODE) ," & vbCrLf _
                    & " INVMST.ISSUE_UOM"
            End If
        ...
        Else
            ...
            Else
                MakeSQL = MakeSQL & vbCrLf & "ORDER BY ID.ITEM_CODE, INVMST.ITEM_SHORT_DESC"
            End If
