    ...
    Private Sub cboDivision_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cboDivision.SelectedIndexChanged
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                ...
                .Col = ColStockQty
                .Text = CStr(GetBalanceStockQty(mItemCode, (txtSlipDate.Text), mUnit, (txtToDept.Text), "ST", "", ConSH, mDivisionCode, ConStockRefType_BDM, Val(txtSlipNo.Text))) '''+ GetSavedQty(pItemCode)
            Next
        End With
    ...
    Private Sub CmdDelete_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdDelete.Click
        ...
        If Not RsBreakDownMain.EOF Then
            ...
            If MsgQuestion("Want to Delete ? ") = CStr(MsgBoxResult.Yes) Then ' User chose Yes.
                ...
                PubDBCn.BeginTrans()
                If InsertIntoDelAudit(PubDBCn, "MAN_BREAKDOWN_HDR", (txtSlipNo.Text), RsBreakDownMain) = False Then GoTo DelErrPart
                If UpdateMachine(True) = False Then GoTo DelErrPart
                If DeleteStockTRN(PubDBCn, ConStockRefType_BDM, CStr(Val(lblMkey.Text))) = False Then GoTo DelErrPart
                PubDBCn.Execute("DELETE FROM MAN_BREAKDOWN_DET WHERE AUTO_KEY_BDSLIP=" & Val(lblMkey.Text) & "")
                PubDBCn.Execute("DELETE FROM MAN_BREAKDOWN_HDR WHERE AUTO_KEY_BDSLIP=" & Val(lblMkey.Text) & "")
                PubDBCn.CommitTrans()
...
DelErrPart:
    ...
    Private Sub CmdModify_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdModify.Click
        ...
        If CmdModify.Text = ConcmdmodifyCaption Then
            If PubSuperUser = "U" Then
                mCompleteDate = IIf(IsDBNull(RsBreakDownMain.Fields("CCOMPLET_DATE").Value), "", RsBreakDownMain.Fields("CCOMPLET_DATE").Value)

                If mCompleteDate <> "" Then MsgBox("Number has been completed, So cann't be modified") : Exit Sub
            End If
            ADDMode = False
            MODIFYMode = True
...
ErrorHandler:
    ...
    Private Function Update1() As Boolean
        ...
        '        & " TO_DATE('" & IIf(mSlipDate = "/  /", "", VB6.Format(mSlipDate, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
        '& " TO_DATE('" & IIf(mSlipTime = ":", "", mSlipTime) & "', 'HH24:MI')," & vbCrLf _
        '& " TO_DATE('" & IIf(mDeputDate = "/  /", "", VB6.Format(mDeputDate, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
        '& " TO_DATE('" & IIf(mDeputTime = ":", "", mDeputTime) & "', 'HH24:MI')," & vbCrLf _
        '& " TO_DATE('" & IIf(mComptDate = "/  /", "", VB6.Format(mComptDate, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
        '& " TO_DATE('" & IIf(mComptTime = ":", "", mComptTime) & "', 'HH24:MI')," & vbCrLf _
        '& " TO_DATE('" & IIf(mCComptDate = "/  /", "", VB6.Format(mCComptDate, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
        '& " TO_DATE('" & IIf(mCComptTime = ":", "", mCComptTime) & "', 'HH24:MI'), " & vbCrLf _

        Dim mSlipDate As String
        Dim mSlipTime As String
        Dim mDeputDate As String
        Dim mDeputTime As String
        Dim mComptDate As String
        Dim mComptTime As String
        Dim mCComptDate As String
        Dim mCComptTime As String

        If VB6.Format(txtDeputDate.Text, "DD/MM/YYYY") = "  /  /" Then
            mDeputDate = ""
        Else
            mDeputDate = txtDeputDate.Text
        End If

        If VB6.Format(txtDeputTime.Text, "HH:MM") = "  :" Then
            mDeputTime = ""
        Else
            mDeputTime = txtDeputTime.Text
        End If

        If VB6.Format(txtComptDate.Text, "DD/MM/YYYY") = "  /  /" Then
            mComptDate = ""
        Else
            mComptDate = txtComptDate.Text
        End If

        If VB6.Format(txtComptTime.Text, "HH:MM") = "  :" Then
            mComptTime = ""
        Else
            mComptTime = txtComptTime.Text
        End If

        If VB6.Format(txtCComptDate.Text, "DD/MM/YYYY") = "  /  /" Then
            mCComptDate = ""
        Else
            mCComptDate = txtCComptDate.Text
        End If

        If VB6.Format(txtCComptTime.Text, "HH:MM") = "  :" Then
            mCComptTime = ""
        Else
            mCComptTime = txtCComptTime.Text
        End If
        ...
        If ADDMode = True Then
            ...
            SqlStr = " INSERT INTO MAN_BREAKDOWN_HDR " & vbCrLf & " (AUTO_KEY_BDSLIP,COMPANY_CODE,FYEAR," & vbCrLf & " SLIP_DATE,BRK_DWN_TIME,FROM_DEPT_CODE,TO_DEPT_CODE," & vbCrLf & " MACHINE_NO,COMPLAINT_BY,SUSPECTED_REASON,PROBLEM_FACED," & vbCrLf & " DEPU_EMP_CODE,DEPU_DATE,DEPU_TIME,ITEM_CONSUMED,DEPU_REMARKS,SLIP_RECEIVED_BY, " & vbCrLf & " COMPLETION_DATE,COMPLETION_TIME,COMPLETION_REMARKS,AUTO_KEY_DLYRPT, " & vbCrLf & " CCOMPLET_DATE,CCOMPLET_TIME,CCOMPLAINT_BY,TOTHOUR,TOTCHOUR, " & vbCrLf & " ADDUSER, ADDDATE, MODUSER, MODDATE, DIV_CODE) "

            SqlStr = SqlStr & vbCrLf _
                            & " VALUES ( " & vbCrLf _
                            & " " & mSlipNo & "," & RsCompany.Fields("COMPANY_CODE").Value & "," & RsCompany.Fields("FYEAR").Value & ", " & vbCrLf _
                            & " TO_DATE('" & IIf(txtSlipDate.Text = "/  /", "", VB6.Format(txtSlipDate.Text, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
                            & " TO_DATE('" & IIf(txtSlipTime.Text = ":", "", txtSlipTime.Text) & "', 'HH24:MI')," & vbCrLf _
                            & " '" & MainClass.AllowSingleQuote(txtFromDept.Text) & "','" & MainClass.AllowSingleQuote(txtToDept.Text) & "'," & vbCrLf _
                            & " '" & MainClass.AllowSingleQuote(txtMachineNo.Text) & "','" & MainClass.AllowSingleQuote(txtCompldBy.Text) & "'," & vbCrLf _
                            ...
                            & " TO_DATE('" & VB6.Format(mDeputDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                            & " TO_DATE('" & mDeputTime & "', 'HH24:MI')," & vbCrLf _
                            ...
                            & " TO_DATE('" & VB6.Format(mComptDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                            & " TO_DATE('" & mComptTime & "', 'HH24:MI')," & vbCrLf _
                            & " '" & MainClass.AllowSingleQuote(txtRemarks.Text) & "',''," & vbCrLf _
                            & " TO_DATE('" & VB6.Format(mCComptDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                            & " TO_DATE('" & mCComptTime & "', 'HH24:MI'), " & vbCrLf _
        ...
        ElseIf MODIFYMode = True Then
            SqlStr = " UPDATE MAN_BREAKDOWN_HDR SET " & vbCrLf _
                    & " AUTO_KEY_BDSLIP=" & mSlipNo & ", " & vbCrLf _
                    & " SLIP_DATE=TO_DATE('" & IIf(txtSlipDate.Text = "/  /", "", VB6.Format(txtSlipDate.Text, "DD-MMM-YYYY")) & "','DD-MON-YYYY'), " & vbCrLf _
                    & " BRK_DWN_TIME=TO_DATE('" & IIf(txtSlipTime.Text = ":", "", txtSlipTime.Text) & "', 'HH24:MI')," & vbCrLf _
                    ...
                    & " MACHINE_NO='" & MainClass.AllowSingleQuote(txtMachineNo.Text) & "',COMPLAINT_BY='" & MainClass.AllowSingleQuote(txtCompldBy.Text) & "'," & vbCrLf _
                    ...
                    & " DEPU_DATE=TO_DATE('" & VB6.Format(mDeputDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                    & " DEPU_TIME=TO_DATE('" & mDeputTime & "', 'HH24:MI')," & vbCrLf _
                    ...
                    & " COMPLETION_DATE=TO_DATE('" & VB6.Format(mComptDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                    & " COMPLETION_TIME=TO_DATE('" & mComptTime & "', 'HH24:MI')," & vbCrLf _
                    & " COMPLETION_REMARKS='" & MainClass.AllowSingleQuote(txtRemarks.Text) & "',AUTO_KEY_DLYRPT=''," & vbCrLf _
                    & " CCOMPLET_DATE=TO_DATE('" & VB6.Format(mCComptDate, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                    & " CCOMPLET_TIME=TO_DATE('" & mCComptTime & "', 'HH24:MI')," & vbCrLf _
        ...
        If UpdateMachine(False) = False Then GoTo ErrPart
...
ErrPart:
    ...
    Private Function SendMail(ByRef pFlag As Object) As Boolean
        ...
        mSubject = "Machine Break Down - Department :" & Trim(lblFromDept.Text) & " From Dated : " & VB6.Format(txtSlipDate.Text) & ""

        If pFlag = "REG" Then
            mBodyText = "<html><body><b><font size=11, color=Red>Machine Break Down</font></b><br />" & "<b>Slip No       : </b>" & Trim(txtSlipNo.Text) & "<br />" & "<b>Department    : </b>" & Trim(lblFromDept.Text) & "<br />" & "<b>Dated         : </b>" & VB6.Format(txtSlipDate.Text) & "<br />" & "<b>Time          : </b>" & Trim(txtSlipTime.Text) & "<br />" & "<b>Machine No    : </b>" & Trim(txtMachineNo.Text) & "<br />" & "<b>Complained By : </b>" & Trim(lblCompldBy.Text) & "<br />" & "<b>Problem       : </b>" & Trim(txtReason.Text) & "<br />" & "</body></html>"
        Else
            mBodyText = "<html><body><b><font size=11, color=Blue>Machine Break Down (Completed)</font></b><br />" & "<b>Slip No               : </b>" & Trim(txtSlipNo.Text) & "<br />" & "<b>Department            : </b>" & Trim(lblFromDept.Text) & "<br />" & "<b>Completion Dated      : </b>" & VB6.Format(txtComptDate.Text) & "<br />" & "<b>Completion Time       : </b>" & Trim(txtComptTime.Text) & "<br />" & "<b>Total Break Down Time : </b>" & Trim(txtTotalTime.Text) & "<br />" & "<b>Machine No            : </b>" & Trim(txtMachineNo.Text) & "<br />" & "<b>Deputed Person        : </b>" & Trim(lblDeputPerson.Text) & "<br />" & "<b>Action Taken          : </b>" & Trim(txtDeputRemarks.Text) & "<br />" & "</body></html>"
        End If
...
ErrPart:
    ...
    End Function
    Private Function UpdateMachine(ByRef pIsSlipDeleted As Boolean) As Boolean
        On Error GoTo UpMacERR
        Dim SqlStr As String
        Dim mMachine_UB As String

        If pIsSlipDeleted = True Then
            mMachine_UB = "N"
        Else
            If Trim(txtSlipRecvdBy.Text) = "" And Trim(txtComptDate.Text) = "" Then
                mMachine_UB = "Y"
            Else
                mMachine_UB = "N"
            End If
        ...
        SqlStr = " UPDATE MAN_MACHINE_MST SET " & vbCrLf _
                    & " MACHINE_UB='" & mMachine_UB & "' " & vbCrLf _
                    & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf _
...
UpMacERR:
    ...
    Private Function AutoGenKeyNo() As Double
        ...
        mAutoGen = 1
        SqlStr = ""
        SqlStr = "SELECT Max(AUTO_KEY_BDSLIP)  " & vbCrLf & " FROM MAN_BREAKDOWN_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & ""
...
AutogenErr:
    ...
    Private Function UpdateDetail(ByRef mDivisionCode As Double) As Boolean
        ...
        If DeleteStockTRN(PubDBCn, ConStockRefType_BDM, (lblMkey.Text)) = False Then GoTo UpdateDetailERR
        PubDBCn.Execute("DELETE FROM MAN_BREAKDOWN_DET WHERE AUTO_KEY_BDSLIP=" & Val(lblMkey.Text) & "")
        ...
        With SprdMain
            For i = 1 To .MaxRows
                ...
                If mQty > 0 And chkItemConsumed.CheckState = System.Windows.Forms.CheckState.Checked Then
                    ...
                    SqlStr = " INSERT INTO  MAN_BREAKDOWN_DET ( " & vbCrLf & " COMPANY_CODE,AUTO_KEY_BDSLIP,SERIAL_NO,SLIP_DATE,BRK_DWN_TIME,FROM_DEPT_CODE, " & vbCrLf & " MACHINE_NO,PROBLEM_FACED,ITEM_CODE,ITEM_UOM,STOCK_TYPE,ITEM_QTY,ITEM_RATE,ITEM_AMOUNT ) " & vbCrLf & " VALUES ( " & vbCrLf & " " & RsCompany.Fields("COMPANY_CODE").Value & "," & Val(lblMkey.Text) & "," & i & ",TO_DATE('" & VB6.Format(txtSlipDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'),TO_DATE('" & txtSlipTime.Text & "', 'HH24:MI'), " & vbCrLf & " '" & MainClass.AllowSingleQuote(txtFromDept.Text) & "','" & MainClass.AllowSingleQuote(txtMachineNo.Text) & "', " & vbCrLf & " '" & MainClass.AllowSingleQuote(txtProblem.Text) & "','" & mItemCode & "','" & mUOM & "', " & vbCrLf & " 'ST'," & mQty & "," & mRate & "," & mAmount & ") "
                    PubDBCn.Execute(SqlStr)
                    If UpdateStockTRN(PubDBCn, ConStockRefType_BDM, CStr(Val(lblMkey.Text)), i, (txtSlipDate.Text), (txtSlipDate.Text), "ST", mItemCode, mUOM, CStr(-1), mQty, 0, "O", 0, 0, "", "", (txtToDept.Text), (txtFromDept.Text), "", "N", " From : " & txtToDept.Text & " To : " & txtFromDept.Text & "-" & ConStockRefType_BDM, "-1", ConSH, mDivisionCode, "", "") = False Then GoTo UpdateDetailERR
                End If
            Next
        End With
        UpdateDetail = True
...
UpdateDetailERR:
    ...
    Private Sub SearchEmp(ByRef pTextBax As System.Windows.Forms.TextBox, ByRef pLable As System.Windows.Forms.Label)
        ...
        If lblFormType.Text = "MAN" Then
            SqlStr = SqlStr & vbCrLf & "  AND EMP_DEPT_CODE='MNT'"
        End If
...
SrchERR:
    ...
    Private Sub cmdSearchMacNo_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSearchMacNo.Click
        ...
        SqlStr = "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND MACHINE_UB='N' AND STATUS='O' AND DIV_CODE=" & mDivisionCode & ""
...
SrchProbERR:
    ...
    Private Sub cmdSearchSlipNo_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSearchSlipNo.Click
        ...
        If MainClass.SearchGridMaster(txtSlipNo.Text, "MAN_BREAKDOWN_HDR", "AUTO_KEY_BDSLIP", "MACHINE_NO", "FROM_DEPT_CODE", "TO_DEPT_CODE", SqlStr) = True Then
    ...
    Private Sub frmBreakDownMan_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ...
        ElseIf lblFormType.Text = "DEL" Then
            Me.Text = "Break Down Complaint (Delivery)"
        ...
        SqlStr = "Select * From MAN_BREAKDOWN_HDR WHERE 1<>1"
        ...
        SqlStr = "Select * From MAN_BREAKDOWN_DET WHERE 1<>1"
...
ERR1:
    ...
    Private Sub AssignGrid(ByRef mRefresh As Boolean)
        ...
        SqlStr = " SELECT " & vbCrLf & " AUTO_KEY_BDSLIP AS BREAKDOWN_NUMBER,TO_CHAR(SLIP_DATE,'DD/MM/YYYY') AS SLIP_DATE,TO_CHAR(BRK_DWN_TIME,'HH24:MM') AS BRK_DWN_TIME, " & vbCrLf & " MACHINE_NO,FROM_DEPT_CODE,TO_DEPT_CODE,COMPLAINT_BY,SUSPECTED_REASON,  " & vbCrLf & " PROBLEM_FACED,DEPU_EMP_CODE,TO_CHAR(DEPU_DATE,'DD/MM/YYYY') AS DEPU_DATE,TO_CHAR(DEPU_TIME,'HH24:MM') AS DEPU_TIME, " & vbCrLf & " DEPU_REMARKS,SLIP_RECEIVED_BY,TO_CHAR(COMPLETION_DATE,'DD/MM/YYYY') AS COMPLETION_DATE,TO_CHAR(COMPLETION_TIME,'HH24:MM') AS COMPLETION_TIME, " & vbCrLf & " COMPLETION_REMARKS " & vbCrLf & " FROM MAN_BREAKDOWN_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " ORDER BY AUTO_KEY_BDSLIP"
...
ERR1:
    ...
    Private Sub frmBreakDownMan_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmBreakDownMan_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    Private Sub Clear1()
        ...
        lblMkey.Text = ""
        txtSlipNo.Text = ""
        txtSlipDate.Text = VB6.Format(PubCurrDate, "DD/MM/YYYY") ''Format(RunDate, "DD/MM/YYYY")
...
ERR1:
    ...
    Private Function FieldsVarification() As Boolean
        ...
        If Trim(txtSlipDate.Text) = "" Or Trim(txtSlipDate.Text) = "/  /" Then
        ...
        If lblFormType.Text = "REG" Then
            ...
            If MainClass.ValidateWithMasterTable(txtMachineNo.Text, "MACHINE_NO", "MACHINE_DESC", "MAN_MACHINE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND MACHINE_UB='N' AND STATUS='O' AND DIV_CODE=" & mDivisionCode & "") = False Then
...
err_Renamed:
    ...
    Private Sub frmBreakDownMan_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
    ...
    Private Sub SprdMain_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_ClickEvent) Handles SprdMain.ClickEvent
        ...
        If eventArgs.row = 0 And eventArgs.col = ColItemCode Then
            With SprdMain
                ...
                SqlStr = GetStockItemQry(.Text, "Y", VB6.Format(txtSlipDate.Text, "DD/MM/YYYY"), ConSH)
        ...
        If eventArgs.row = 0 And eventArgs.col = ColItemName Then
            With SprdMain
                ...
                .Col = ColItemName
                '            If RsCompany.fields("COMPANY_CODE").value = 12 Then
                SqlStr = GetStockItemQry(.Text, "N", VB6.Format(txtSlipDate.Text, "DD/MM/YYYY"), ConSH)
    ...
    Private Function FillGridRow(ByRef pItemCode As String, ByRef mDivisionCode As Double) As Boolean
        ...
        If RsMisc.EOF = False Then
            ...
            With RsMisc
                ...
                mStockQty = GetBalanceStockQty(pItemCode, (txtSlipDate.Text), mUnit, (txtToDept.Text), "ST", "", ConSH, mDivisionCode, ConStockRefType_BDM, Val(txtSlipNo.Text)) '''+ GetSavedQty(pItemCode)
                SprdMain.Col = ColStockQty
                SprdMain.Text = CStr(mStockQty)

                SprdMain.Col = ColRate
                SprdMain.Text = CStr(GetLatestItemCostFromMRR(mItemCode, mUnit, 1, VB6.Format(IIf((txtComptDate.Text = "" Or txtComptDate.Text = "/  /"), RunDate, txtComptDate.Text), "DD/MM/YYYY"), "L"))
            End With
            FillGridRow = True
        Else
...
ValEMP:
    ...
    Private Sub txtComptDate_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtComptDate.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtComptDate.Text) = "" Or Trim(txtComptDate.Text) = "/  /" Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub txtCComptDate_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtCComptDate.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtCComptDate.Text) = "" Or Trim(txtCComptDate.Text) = "/  /" Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub txtComptTime_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtComptTime.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtComptTime.Text) = "" Or Trim(txtComptTime.Text) = ":" Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub txtCComptTime_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtCComptTime.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtCComptTime.Text) = "" Or Trim(txtCComptTime.Text) = ":" Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub txtDeputDate_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtDeputDate.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtDeputDate.Text) = "" Or Trim(txtDeputDate.Text) = "/  /" Or txtDeputDate.Text = "/  /" Or txtDeputDate.Text = "__/__/" Or Trim(txtDeputDate.Text) = "  /  /    " Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Function CheckDate(ByRef pTextDate As System.Windows.Forms.MaskedTextBox) As Boolean
        ...
        If pTextDate.Name = txtSlipDate.Name Then
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /" And Trim(txtSlipDate.Text) <> "/  /") And (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /" And Trim(txtDeputDate.Text) <> "/  /") Then
            ...
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /" And Trim(txtSlipDate.Text) <> "/  /") And (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /" And Trim(txtComptDate.Text) <> "/  /") Then
            ...
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /" And Trim(txtSlipDate.Text) <> "/  /") And Trim(txtCComptDate.Text) <> "/  /" And (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") Then
        ...
        ElseIf pTextDate.Name = txtDeputDate.Name Then
            If (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") And (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") Then
            ...
            If (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") And (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") Then
            ...
            If (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") And (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") Then
        ...
        ElseIf pTextDate.Name = txtComptDate.Name Then
            If (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") And (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") Then
            ...
            If (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") And (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") Then
            ...
            If (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") And (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") Then
        ...
        ElseIf pTextDate.Name = txtCComptDate.Name Then
            If (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") And (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") Then
            ...
            If (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") And (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") Then
            ...
            If (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") And (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") Then
...
ERR1:
    ...
    Private Function CheckTime(ByRef pTextTime As System.Windows.Forms.MaskedTextBox) As Boolean
        ...
        If pTextTime.Text = txtSlipTime.Text Then
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") And (Trim(txtDeputDate.Text) <> "" And Trim(txtDeputDate.Text) <> "/  /") Then
                If CDate(txtSlipDate.Text) = CDate(txtDeputDate.Text) Then
            ...
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") And (Trim(txtComptDate.Text) <> "" And Trim(txtComptDate.Text) <> "/  /") Then
                If CDate(txtSlipDate.Text) = CDate(txtComptDate.Text) Then
            ...
            If (Trim(txtSlipDate.Text) <> "" And Trim(txtSlipDate.Text) <> "/  /") And (Trim(txtCComptDate.Text) <> "" And Trim(txtCComptDate.Text) <> "/  /") Then
                If CDate(txtSlipDate.Text) = CDate(txtCComptDate.Text) Then
    ...
    Private Sub CalcTot()
        ...
        If mStartDate = "" Or mStartDate = "/  /" Or mStartDate = "/  /" Then GoTo ForComplainerSite
        If mStartTime = "" Or mStartTime = ":" Or mStartTime = ":" Then GoTo ForComplainerSite
        If mEndDate = "" Or mEndDate = "/  /" Or mEndDate = "/  /" Then GoTo ForComplainerSite
        If mEndTime = "" Or mEndTime = ":" Or mEndTime = ":" Then GoTo ForComplainerSite
...
ForComplainerSite:
        ...
        If mStartDate = "" Or mStartDate = "/  /" Or mStartDate = "/  /" Then Exit Sub
        If mStartTime = "" Or mStartTime = ":" Or mStartTime = ":" Then Exit Sub
        If mEndDate = "" Or mEndDate = "/  /" Or mEndDate = "/  /" Then Exit Sub
        If mEndTime = "" Or mEndTime = ":" Or mEndTime = ":" Then Exit Sub
        mStartDateTime = mStartDate & " " & mStartTime
        mEndDateTime = mEndDate & " " & mEndTime
    ...
    Private Sub txtDeputTime_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtDeputTime.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtDeputTime.Text) = "" Or Trim(txtDeputTime.Text) = ":" Then GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub txtProblem_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtProblem.Validating
        ...
        SqlStr = "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & ""
        If MainClass.ValidateWithMasterTable(txtProblem.Text, "PROB_CODE", "PROB_DESC", "MAN_BDPROBLEMS_MST", PubDBCn, MasterNo, , SqlStr) = False Then
...
EventExitSub:
    ...
    Private Sub txtSlipDate_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtSlipDate.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtSlipDate.Text) = "" Or Trim(txtSlipDate.Text) = "/  /" Then GoTo EventExitSub
        ...
        If CDate(txtSlipDate.Text) > CDate(PubCurrDate) Then
...
EventExitSub:
    ...
    Private Sub Show1()
        ...
        If Not RsBreakDownMain.EOF Then
            ...
            txtSlipDate.Text = IIf(IsDBNull(RsBreakDownMain.Fields("SLIP_DATE").Value), "/  /", RsBreakDownMain.Fields("SLIP_DATE").Value)
            txtSlipTime.Text = VB6.Format(IIf(IsDBNull(RsBreakDownMain.Fields("BRK_DWN_TIME").Value), ":", RsBreakDownMain.Fields("BRK_DWN_TIME").Value), "HH:MM")
            ...
            txtMachineNo.Text = IIf(IsDBNull(RsBreakDownMain.Fields("MACHINE_NO").Value), "", RsBreakDownMain.Fields("MACHINE_NO").Value)
            ...
            txtDeputDate.Text = IIf(IsDBNull(RsBreakDownMain.Fields("DEPU_DATE").Value), "", RsBreakDownMain.Fields("DEPU_DATE").Value)
            txtDeputTime.Text = VB6.Format(IIf(IsDBNull(RsBreakDownMain.Fields("DEPU_TIME").Value), ":", RsBreakDownMain.Fields("DEPU_TIME").Value), "HH:MM")
            ...
            txtComptDate.Text = IIf(IsDBNull(RsBreakDownMain.Fields("COMPLETION_DATE").Value), "/  /", RsBreakDownMain.Fields("COMPLETION_DATE").Value)
            txtComptTime.Text = VB6.Format(IIf(IsDBNull(RsBreakDownMain.Fields("COMPLETION_TIME").Value), ":", RsBreakDownMain.Fields("COMPLETION_TIME").Value), "HH:MM")
            txtTotalTime.Text = IIf(IsDBNull(RsBreakDownMain.Fields("TOTHOUR").Value), "", RsBreakDownMain.Fields("TOTHOUR").Value)
            txtCComptDate.Text = IIf(IsDBNull(RsBreakDownMain.Fields("CCOMPLET_DATE").Value), "/  /", RsBreakDownMain.Fields("CCOMPLET_DATE").Value)
            txtCComptTime.Text = VB6.Format(IIf(IsDBNull(RsBreakDownMain.Fields("CCOMPLET_TIME").Value), ":", RsBreakDownMain.Fields("CCOMPLET_TIME").Value), "HH:MM")
...
ShowErrPart:
    ...
    Private Sub ShowDetail1(ByRef mDivisionCode As Double)
        ...
        SqlStr = ""
        SqlStr = " SELECT * " & vbCrLf & " FROM MAN_BREAKDOWN_DET " & vbCrLf & " WHERE AUTO_KEY_BDSLIP=" & Val(lblMkey.Text) & "" & vbCrLf & " ORDER BY SERIAL_NO"
        ...
        With RsBreakDownDetail
            ...
            Do While Not .EOF
                ...
                SprdMain.Col = ColStockQty
                SprdMain.Text = CStr(GetBalanceStockQty(mItemCode, (txtSlipDate.Text), Trim(IIf(IsDBNull(.Fields("ITEM_UOM").Value), "", .Fields("ITEM_UOM").Value)), (txtToDept.Text), "ST", "", ConSH, mDivisionCode, ConStockRefType_BDM, CDbl(txtSlipNo.Text))) '''+ GetSavedQty(pItemCode)
...
ERR1:
    ...
    Private Sub txtSlipNo_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtSlipNo.Validating
        ...
        SqlStr = "SELECT * FROM MAN_BREAKDOWN_HDR " & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND AUTO_KEY_BDSLIP=" & mSlipNo & ""
        ...
        Else
            If ADDMode = False And MODIFYMode = False Then
                ...
                Cancel = True
            ElseIf MODIFYMode = True Then
                SqlStr = "SELECT * FROM MAN_BREAKDOWN_HDR " & vbCrLf & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & " " & vbCrLf & " AND AUTO_KEY_BDSLIP=" & Val(CStr(xMKey)) & " "
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsBreakDownMain, ADODB.LockTypeEnum.adLockReadOnly)
            End If
        End If
        GoTo EventExitSub
...
EventExitSub:
    ...
    Private Sub MakeEnableDesableField(ByRef mMode As Boolean)
        txtSlipDate.Enabled = False '' mMode
        txtSlipTime.Enabled = False '' mMode
...
ErrPart:
    ...
    Private Sub txtSlipTime_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtSlipTime.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        If Trim(txtSlipTime.Text) = "" Or Trim(txtSlipTime.Text) = ":" Then GoTo EventExitSub
