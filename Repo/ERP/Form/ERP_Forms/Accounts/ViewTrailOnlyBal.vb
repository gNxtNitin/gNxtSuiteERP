...
Friend Class frmViewTrailOnlyBal
...
ERR1:
    ...
    Private Sub FormatSprdTrail(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            .set_ColWidth(0, 4)
            .Col = ColAcmName
            .set_ColWidth(ColAcmName, 50)
            .ColsFrozen = ColAcmName
            .Col = ColOpening
            .set_ColWidth(ColOpening, 10)
...
ERR1:
    ...
    Private Function ViewTrial() As Boolean
        ...
        ''        & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & vb6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf _
        ''        & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)>=0 THEN " & vbCrLf _
        ''        & " TO_CHAR(ABS(SUM(CASE WHEN VDate<TO_DATE('" & vb6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf _
        ...
        ''        & " TO_CHAR(SUM(CASE WHEN VDate>=TO_DATE('" & vb6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf _
        ...
        ''        & " ACM.SUPP_CUST_TYPE AS CATEGORY,TO_CHAR(ACM.SUPP_CUST_CODE),TO_CHAR(ACM.GROUPCODE),GEN.COMPANY_NAME "
        SqlStr = "SELECT ACM.SUPP_CUST_NAME,  " & vbCrLf & " '0.00' AS DROpening, " & vbCrLf & " '0.00' AS CROpening, " & vbCrLf & " '0.00' AS DEBIT, " & vbCrLf & " '0.00' AS CREDIT, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))>=0 THEN TO_CHAR(ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1)))) ELSE '0.00' END AS DEBITBAL, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))<0 THEN TO_CHAR(ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1)))) ELSE '0.00' END AS CREDITBAL, " & vbCrLf & " ACM.SUPP_CUST_TYPE AS CATEGORY,TO_CHAR(ACM.SUPP_CUST_CODE),TO_CHAR(ACM.GROUPCODE),GEN.COMPANY_NAME "
        ''********TABLEs..........
        SqlStr = SqlStr & vbCrLf & " FROM FIN_POSTED_TRN TRN,FIN_SUPP_CUST_MST ACM,GEN_COMPANY_MST GEN "
        ''********Joining..........
        SqlStr = SqlStr & vbCrLf & " WHERE " & vbCrLf & " GEN.COMPANY_CODE=TRN.COMPANY_CODE " & vbCrLf & " AND TRN.COMPANY_CODE=ACM.COMPANY_CODE " & vbCrLf & " AND TRN.ACCOUNTCODE=ACM.SUPP_CUST_CODE "
        ...
        '    If ChkHideZeroBal.Value = vbChecked Then
        SqlStr = SqlStr & " HAVING SUM(AMOUNT * DECODE(DC,'D',1,-1))<>0"
        '    ElseIf ChkHideZeroTrans.Value = vbChecked Then
        ...
        ''********ORDER BY CLAUSE..........
        SqlStr = SqlStr & vbCrLf & " ORDER BY " & vbCrLf & " ACM.SUPP_CUST_NAME,GEN.COMPANY_NAME "
...
ViewTrialErr:
    ...
    Private Function InsertIntoTempQry() As String
        ...
        Dim mDivisionCode As Double
        ''********SELECTION..........
        SqlStr = "SELECT '" & MainClass.AllowSingleQuote(PubUserID) & "', ACM.SUPP_CUST_CODE,  " & vbCrLf & " -1*ACM.GROUPCODE, ACM.SUPP_CUST_NAME, " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)>=0 THEN " & vbCrLf & " ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END))" & vbCrLf & " ELSE 0 END AS DROpening, " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)<=0 THEN " & vbCrLf & " ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END))" & vbCrLf & " ELSE 0 END AS CROpening, " & vbCrLf & " SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,0) END) AS DEBIT, " & vbCrLf & " SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',0,1) END) AS CREDIT, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))>=0 THEN ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1))) ELSE 0 END AS DEBITBAL, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))<0 THEN ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1))) ELSE 0 END AS CREDITBAL, " & vbCrLf & " ACM.SUPP_CUST_TYPE "
        ''********TABLEs..........
        SqlStr = SqlStr & vbCrLf & " FROM FIN_POSTED_TRN TRN,FIN_SUPP_CUST_MST ACM "
        ', CST_CENTER_MST COSTC,PAY_DEPT_MST DEPT
        ''********Joining..........
        SqlStr = SqlStr & vbCrLf & " WHERE " & vbCrLf & " TRN.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND ACM.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TRN.COMPANY_CODE=ACM.COMPANY_CODE" & vbCrLf & " AND TRN.ACCOUNTCODE=ACM.SUPP_CUST_CODE "
        ''********Conditions..........
...
ViewTrialErr:
    ...
    Private Function InsertTypeIntoTempQry(ByRef mType As String) As String
        ...
        Dim mDivisionCode As Double
        ''********SELECTION..........
        SqlStr = "SELECT '" & MainClass.AllowSingleQuote(PubUserID) & "', ACM.SUPP_CUST_CODE,  " & vbCrLf & " -1*ACM.GROUPCODE, ACM.SUPP_CUST_NAME, " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)>=0 THEN " & vbCrLf & " ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END))" & vbCrLf & " ELSE 0 END AS DROpening, " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)<=0 THEN " & vbCrLf & " ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END))" & vbCrLf & " ELSE 0 END AS CROpening, " & vbCrLf & " SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,0) END) AS DEBIT, " & vbCrLf & " SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',0,1) END) AS CREDIT, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))>=0 THEN ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1))) ELSE 0 END AS DEBITBAL, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))<0 THEN ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1))) ELSE 0 END AS CREDITBAL, " & vbCrLf & " ACM.SUPP_CUST_TYPE "
        ''********TABLEs..........
        SqlStr = SqlStr & vbCrLf & " FROM FIN_POSTED_TRN TRN,FIN_SUPP_CUST_MST ACM, " & vbCrLf & " FIN_GROUP_MST ACMGROUP"
        '', CST_CENTER_MST COSTC,PAY_DEPT_MST DEPT "
        ''********Joining..........
        SqlStr = SqlStr & vbCrLf & " WHERE " & vbCrLf & " TRN.COMPANY_CODE=ACM.COMPANY_CODE " & vbCrLf & " AND TRN.ACCOUNTCODE=ACM.SUPP_CUST_CODE " & vbCrLf & " AND ACM.COMPANY_CODE=ACMGROUP.COMPANY_CODE " & vbCrLf & " AND ACM.GROUPCODE=ACMGROUP.GROUP_CODE "
        ''& vbCrLf _
        ...
        SqlStr = SqlStr & vbCrLf & " AND TRN.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TRN.VDate <= TO_DATE('" & VB6.Format(txtDate(1).Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        If cboDivision.SelectedIndex > 0 Then
            If MainClass.ValidateWithMasterTable(Trim(cboDivision.Text), "DIV_DESC", "DIV_CODE", "INV_DIVISION_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mDivisionCode = CDbl(Trim(MasterNo))
            End If
...
ViewTrialErr:
    ...
    Private Function InsertGroupIntoTempQry() As String
        ...
        End If
        ''********ORDER BY CLAUSE..........
        SqlStr = SqlStr & vbCrLf & " ORDER BY " & vbCrLf & " GROUP_SCHEDULENO,GROUP_NAME "
...
ViewTrialErr:
    ...
    Private Function ViewTrialSumm() As Boolean
        ...
        Dim mDivisionCode As Double
        ''********SELECTION..........
        SqlStr = "SELECT ACMGROUP.GROUP_NAME,  " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)>=0 THEN " & vbCrLf & " TO_CHAR(ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)))" & vbCrLf & " ELSE '0.00' END AS DROpening, " & vbCrLf & " CASE WHEN SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)<=0 THEN " & vbCrLf & " TO_CHAR(ABS(SUM(CASE WHEN VDate<TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,-1) END)))" & vbCrLf & " ELSE '0.00' END AS CROpening, " & vbCrLf & " TO_CHAR(SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',1,0) END)) AS DEBIT, " & vbCrLf & " TO_CHAR(SUM(CASE WHEN VDate>=TO_DATE('" & VB6.Format(txtDate(0).Text, "DD-MMM-YYYY") & "') " & vbCrLf & " THEN AMOUNT * DECODE(DC,'D',0,1) END)) AS CREDIT, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))>=0 THEN TO_CHAR(ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1)))) ELSE '0.00' END AS DEBITBAL, " & vbCrLf & " CASE WHEN SUM(AMOUNT * DECODE(DC,'D',1,-1))<0 THEN TO_CHAR(ABS(SUM(AMOUNT * DECODE(DC,'D',1,-1)))) ELSE '0.00' END AS CREDITBAL, " & vbCrLf & " ACMGROUP.GROUP_CATEGORY,TO_CHAR(ACMGROUP.GROUP_CODE),TO_CHAR(GROUP_PARENTCODE) "
        ''********TABLEs..........
        ...
        SqlStr = SqlStr & vbCrLf & " AND TRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & ""
        SqlStr = SqlStr & vbCrLf & " AND TRN.COMPANY_CODE=ACM.COMPANY_CODE " & vbCrLf & " AND TRN.ACCOUNTCODE=ACM.SUPP_CUST_CODE " & vbCrLf & " AND ACM.COMPANY_CODE=ACMGROUP.COMPANY_CODE " & vbCrLf & " AND ACM.GROUPCODE=ACMGROUP.GROUP_CODE "
        ''********Conditions..........
        mDeptName = MainClass.AllowSingleQuote(UCase(Trim(CboDept.Text)))
        mCostCName = MainClass.AllowSingleQuote(UCase(Trim(CboCC.Text)))
        If cboDivision.SelectedIndex > 0 Then
...
ViewTrialErr:
    ...
    Private Function ViewTrialDetail() As Boolean
        ...
        SqlStr = SqlStr & vbCrLf & " ORDER SIBLINGS BY " & vbCrLf & " ACCOUNTNAME" ''1
        '    End If
        MainClass.AssignDataInSprd8(SqlStr, SprdMain, StrConn, "Y")
...
ErrPart:
        ...
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Function
    Private Sub frmViewTrailOnlyBal_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ...
        'Resume
    End Sub
    Private Sub frmViewTrailOnlyBal_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        FillComboBox()
        FormLoaded = False
        Call frmViewTrailOnlyBal_Activated(eventSender, eventArgs)
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        Exit Sub
err_Renamed:
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmViewTrailOnlyBal_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        ...
        MainClass.SetSpreadColor(SprdMain, -1)
    End Sub
    Private Sub frmViewTrailOnlyBal_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
