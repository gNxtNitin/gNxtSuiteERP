...
Friend Class frmParamWOReg
    ...
    Private Const ColPrice As Short = 9
    Private Const ColDiscPer As Short = 10
    Private Const ColDiscRate As Short = 11
    Private Const ColAmount As Short = 12
    Private Const ColNAVPONo As Short = 13
    ...
    Private Sub ReportonPO(ByRef Mode As Crystal.DestinationConstants)
        ...
        mTitle = "Work Order Register (ALL)"

        mSubTitle = "From : " & VB6.Format(txtDateFrom.Text, "DD/MM/YYYY") & " To : " & VB6.Format(txtDateTo.Text, "DD/MM/YYYY")
        Report1.ReportFileName = My.Application.Info.DirectoryPath & "\Reports\WOReg.RPT"
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamWOReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        ...
        Else
            Me.Text = "Purchase Order (Project) Register"
        End If
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamWOReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        Call frmParamWOReg_Activated(eventSender, eventArgs)
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        Exit Sub
BSLError:
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
    ...
    End Sub
    Private Sub frmParamWOReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamWOReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Dispose()
        Me.Close()
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT ''," & vbCrLf & " IH.AUTO_KEY_PO," & vbCrLf & " TO_CHAR(IH.PUR_ORD_DATE,'DD/MM/YYYY'),AMEND_NO," & vbCrLf & " CMST.SUPP_CUST_NAME," & vbCrLf & " ID.WO_DESCRIPTION," & vbCrLf & " ID.ITEM_UOM, TO_CHAR(ITEM_QTY), " & vbCrLf & " TO_CHAR(ITEM_PRICE),TO_CHAR(ITEM_DIS_PER)," & vbCrLf & " TO_CHAR((NVL(ITEM_PRICE,0) - ROUND((NVL(ITEM_PRICE,0) * ITEM_DIS_PER)/100,2))) AS DISCrate, " & vbCrLf & " TO_CHAR(GROSS_AMT), " & vbCrLf & " IH.NAV_PO_NO, IH.AUTO_KEY_PO "

        ''FROM CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " FROM PUR_PURCHASE_HDR IH, PUR_PURCHASE_DET ID," & vbCrLf & " FIN_SUPP_CUST_MST CMST "

        ''WHERE CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " WHERE " & vbCrLf & " IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IH.Mkey=ID.Mkey" & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE "
        ...
        MakeSQL = MakeSQL & vbCrLf & "AND IH.PUR_TYPE='" & lblBookType.Text & "'"
        ...
        If OptOrderBy(0).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf & "ORDER BY IH.MKEY,IH.SUPP_CUST_CODE, IH.PUR_ORD_DATE,ID.SERIAL_NO"
        ElseIf OptOrderBy(1).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf & "ORDER BY ID.WO_DESCRIPTION,IH.MKEY, IH.PUR_ORD_DATE"
        End If
        Exit Function
ERR1:
        MsgInformation(Err.Description)
