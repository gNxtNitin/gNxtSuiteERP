...
Friend Class frmVoucherCheckList
    ...
    Private Const ColTransDate As Short = 4
    Private Const ColVDate As Short = 5
    Private Const ColVNo As Short = 6
    Private Const ColParticulars As Short = 7
    ...
    Private Const ColADDUser As Short = 18
    Private Const ColADDDate As Short = 19
    Private Const ColMODUser As Short = 20
    Private Const ColMODDate As Short = 21
    Private Const ColBranch As Short = 22
    'Private Const ColDetails As Short = 23
...
ErrPart:
    ...
    Public Sub frmVoucherCheckList_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        Dim I As Integer
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmVoucherCheckList_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    Private Sub frmVoucherCheckList_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        ...
        MainClass.SetSpreadColor(SprdLedg, -1, False)
        Exit Sub
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub

    Private Sub frmVoucherCheckList_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
...
EventExitSub:
    ...
    Private Sub FormatSprdLedg(ByRef Arow As Integer)
        ...
        With SprdLedg
            ...
            MainClass.ProtectCell(SprdLedg, 1, .MaxRows, 1, ColChequeNo)
            MainClass.ProtectCell(SprdLedg, 1, .MaxRows, ColDept, ColBranch)
            ...
            SprdLedg.Row = 0
            SprdLedg.Col = ColTransDate
            If cboShow.SelectedIndex = 0 Then
                SprdLedg.Text = "Add Date"
            ElseIf cboShow.SelectedIndex = 1 Then
                SprdLedg.Text = "Add Date"
            ElseIf cboShow.SelectedIndex = 2 Then
                SprdLedg.Text = "Modify Date"
            End If

        End With
    End Sub
    Private Sub FormatSprdLedgOld(ByRef Arow As Integer)
    ...
    Private Function LedgInfo() As Boolean
        ...
        SqlStr = SqlStr & vbCrLf & " GROUP BY " & vbCrLf & " TRN.ADDDATE ," & vbCrLf _
            & " TRN.VDATE , TRN.VDATE," & vbCrLf _
            & " TRN.BOOKTYPE , TRN.BOOKSUBTYPE , " & vbCrLf _
            & " TRN.VNO ,  " & vbCrLf _
            & " TRN.DC,ACM.SUPP_CUST_NAME ," & vbCrLf _
            & " TRN.NARRATION, TRN.REMARKS ," & vbCrLf _
            & " TRN.MKEY ," & vbCrLf _
            & " CHEQUENO || ' ' || CHQDATE, BOOKCODE,TRN.ADDUSER,TRN.ADDDATE,TRN.MODUSER,TRN.MODDATE " & vbCrLf _
            & " ORDER BY " & vbCrLf _
            & " TRN.VDATE , " & vbCrLf _
            & " TRN.VNO,TRN.DC,ACM.SUPP_CUST_NAME "
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        SqlStr = " SELECT '' AS LOCKED, " & vbCrLf & " TRN.BOOKTYPE , TRN.BOOKSUBTYPE," ''''& vbCrLf |            & " TO_CHAR(TRN.ADDDATE,'DD/MM/YYYY'),  "

        If cboShow.SelectedIndex = 0 Then
            SqlStr = SqlStr & " TRN.ADDDATE,  "
        ElseIf cboShow.SelectedIndex = 1 Then
            SqlStr = SqlStr & " TRN.ADDDATE,  "
        ElseIf cboShow.SelectedIndex = 2 Then
            SqlStr = SqlStr & " TRN.MODDATE,  "
        End If
