...
Friend Class frmViewLedgerCal
    ...
    Private Const ColUnitName As Short = 14
    Private Const ColAdjustedAmount As Short = 15
    Private Const ColUnAdjustedAmount As Short = 16

    Private Const ColVendorCode As Short = 17
    Private Const ColDept As Short = 18
    Private Const ColEmp As Short = 19
    Private Const ColCostC As Short = 20
    Private Const ColExpDate As Short = 21

    Private Const ColExpenseHead As Short = 22
    ...
    'Private Const ColBillLDate As Short = 4
    ...
    'Private Const ColUnitName As Short = 13
    'Private Const ColAdjustedAmount As Short = 14
    'Private Const ColUnAdjustedAmount As Short = 15

    'Private Const ColVendorCode As Short = 16
    'Private Const ColDept As Short = 17
    'Private Const ColEmp As Short = 18
    'Private Const ColCostC As Short = 19
    'Private Const ColExpDate As Short = 20

    'Private Const ColExpenseHead As Short = 21
    'Private Const ColClearDate As Short = 22
    'Private Const ColAddUser As Short = 23
    'Private Const ColAddDate As Short = 24
    'Private Const ColModUser As Short = 25
    'Private Const ColModDate As Short = 26

    'Private Const ColMKEY As Short = 27
    'Private Const ColSubRowNo As Short = 28
    'Private Const ColBranch As Short = 29
    ...
    Private Sub InsertSelectedAcct(ByRef pDBCn As ADODB.Connection)
        ...
        For lngRow = 0 To UltraGrid1.Rows.Count - 1 '' UltraDataSource1.Rows.Count - 1
            ...
            SqlStr = "Insert into Temp_Ledger ( " & vbCrLf _
                ...
                & " CHQNO,ACCOUNTNAME,COSTCNAME,BILLDETAIL " & vbCrLf _
                & " ) Values (" & vbCrLf _
                ...
                & " '" & MainClass.AllowSingleQuote(Trim(mBillDetail)) & "')"
...
ERR1:
    ...
    Private Sub InsertAllLederAcct(ByRef pDBCn As ADODB.Connection, ByRef pAllAccount As Boolean, ByRef pGroupType As String)
        ...
        If pAllAccount = True Then
            SqlStr = "SELECT SUPP_CUST_CODE,SUPP_CUST_NAME FROM FIN_SUPP_CUST_MST " & vbCrLf _
        ...
        If RS.EOF = False Then
            Do While Not RS.EOF
                ...
                '
                If OptSumDet(0).Checked = True Then
                    SqlStr = SqlStr & vbCrLf & " GROUP BY CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_CHAR(LAST_DAY(TRN.VDATE),'YYYYMMDD') ELSE TO_CHAR(Vdate,'YYYYMMDD') END," & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_CHAR(LAST_DAY(TRN.VDATE),'DD/MM/YYYY') ELSE TO_CHAR(TRN.VDATE,'DD/MM/YYYY') END," & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_DATE(LAST_DAY(TRN.VDATE),'DD/MM/YYYY') ELSE TO_DATE(TRN.VDATE,'DD/MM/YYYY') END, " & vbCrLf & " TRN.BOOKTYPE , TRN.BOOKSUBTYPE , " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN  TO_CHAR(TRN.VDATE,'MONYYYY') ELSE TRN.VNO END,  " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN DECODE(BOOKCODE," & ConSalesBookCode & ",'Sales Month of :','Purchase Month of :') || TO_CHAR(TRN.VDATE,'MONYYYY') ELSE DECODE(TRN.NARRATION,NULL,'',TRN.NARRATION) END," & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN '' ELSE TRN.REMARKS END, " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN '-1' ELSE TRN.MKEY END," & vbCrLf & " CHEQUENO || ' ' || TO_CHAR(CHQDATE,'DD/MM/YYYY')" & vbCrLf & " ORDER BY " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_DATE(LAST_DAY(TRN.VDATE),'DD/MM/YYYY') ELSE TO_DATE(TRN.VDATE,'DD/MM/YYYY') END, " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN  TO_CHAR(TRN.VDATE,'MONYYYY') ELSE TRN.VNO END"
                ElseIf OptSumDet(1).Checked = True Then
                    SqlStr = SqlStr & vbCrLf & " GROUP BY TRN.VDATE" & vbCrLf & " ORDER BY TRN.VDATE"
                ElseIf OptSumDet(2).Checked = True Then
                    SqlStr = SqlStr & vbCrLf & " GROUP BY SUBSTR(Vdate,4,3),TO_CHAR(Vdate,'YYYYMM')" & vbCrLf & " ORDER BY TO_CHAR(Vdate,'YYYYMM')"
                End If
...
ERR1:
        ...
        'Resume
    End Sub
    Public Sub frmViewLedgerCal_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        Dim I As Integer
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmViewLedgerCal_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        Call frmViewLedgerCal_Activated(eventSender, eventArgs)
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        Exit Sub
BSLError:
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
...
ErrPart:
    ...
    Private Sub frmViewLedgerCal_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmViewLedgerCal_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
    ...
    Private Sub ViewAccountLedger(ByRef xIndex As Integer, ByRef pDateFrom As String, ByRef pDateTo As String)
        Dim ss As New frmViewLedgerCal
        ...
        If lblBookType.Text = ConLedger Then
            ...
            ss.Show()
            ss.frmViewLedgerCal_Activated(Nothing, New System.EventArgs())
...
ERR1:
    ...
    Private Function LedgInfo(pType As String) As Boolean
        ...
        Else
            ...
            mFromFY = CStr(GetCurrentFYNo(PubDBCn, VB6.Format(txtDateFrom.Text, "DD/MM/YYYY")))
            mToFY = CStr(GetCurrentFYNo(PubDBCn, VB6.Format(txtDateTo.Text, "DD/MM/YYYY")))
        ...
        If OptSumDet(0).Checked = True Then
            SqlStr = SqlStr & vbCrLf & " GROUP BY TRN.COMPANY_CODE, GCMT.COMPANY_SHORTNAME, CASE WHEN BOOKCODE=" & ConSalesBookCode & " THEN GETVENDORCODE(TRN.COMPANY_CODE,TRN.MKEY,TRN.VNO) ELSE '' END," & vbCrLf _
            ...
            SqlStr = SqlStr & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN '' ELSE REPLACE(TRN.REMARKS, chr(13)||chr(10),' ') END," & vbCrLf _
                ...
                & " TRN.BOOKTYPE, TRN.BOOKSUBTYPE, CHEQUENO || ' ' || TO_CHAR(CHQDATE,'DD/MM/YYYY'),MONTHWISE_LDGR, BOOKCODE, TRN.ADDUSER, TRN.ADDDATE"          '', TRN.MODUSER,TRN.MODDATE TRN.CLEARDATE,"
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        If OptSumDet(0).Checked = True Then
            SqlStr = " SELECT TRN.COMPANY_CODE AS LOCKED, " & vbCrLf _
                ...
                & " MIN(CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN  '' ELSE TO_CHAR(TRN.BILLDATE,'DD/MM/YYYY') END) AS BILLDATE, " & vbCrLf _
            ...
            SqlStr = SqlStr & vbCrLf _
                ...
                & " MAX(GETLEDGEREXPHEAD (TRN.COMPANY_CODE,  TRN.FYEAR, TRN.BOOKCODE, TRN.MKEY, TRN.VNO, TRN.ACCOUNTCODE)) AS EXPENSES_HEAD," & vbCrLf _
            ...
            'SqlStr = " SELECT '' AS LOCKED, " & vbCrLf _
            '    & " TRN.BOOKTYPE , " & vbCrLf _
            '    & " TRN.BOOKSUBTYPE, '','','','','','' "
...
ERR1:
    ...
    Private Function MakeSQLForInsert(ByRef pAccountName As String) As String
        ...
        If OptSumDet(0).Checked = True Then
            SqlStr = SqlStr & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_CHAR(LAST_DAY(TRN.VDATE),'YYYYMMDD') ELSE TO_CHAR(Vdate,'YYYYMMDD') END," & vbCrLf & " '" & MainClass.AllowSingleQuote(pAccountName) & "'," & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN TO_CHAR(LAST_DAY(TRN.VDATE),'DD/MM/YYYY') ELSE TO_CHAR(TRN.VDATE,'DD/MM/YYYY') END, " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN  TO_CHAR(TRN.VDATE,'MONYYYY') ELSE TRN.VNO END, " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN DECODE(BOOKCODE," & ConSalesBookCode & ",'Sales Month of :','Purchase Month of :') || TO_CHAR(TRN.VDATE,'MONYYYY') ELSE DECODE(TRN.NARRATION,NULL,'',TRN.NARRATION) END AS NARRATION," & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))>=0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1)) ELSE 0 END , " & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))<0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',-1,1)) ELSE 0 END , " & vbCrLf & " CHEQUENO || ' ' || TO_CHAR(CHQDATE,'DD/MM/YYYY') AS CHEQUENO, " & vbCrLf & " '','', " & vbCrLf & " CASE WHEN MONTHWISE_LDGR='Y' AND (BOOKCODE=" & ConPurchaseBookCode & " OR BOOKCODE=" & ConSalesBookCode & ") THEN '' ELSE TRN.REMARKS END"
            ''& " DEPT.DEPT_CODE,EMP.EMP_CODE,COSTC.COST_CENTER_CODE,TRN.MKEY,'','' "
        ElseIf OptSumDet(1).Checked = True Then
            SqlStr = SqlStr & vbCrLf & " TO_CHAR(Vdate,'YYYYMMDD'),'', TO_CHAR(TRN.VDATE,'DD/MM/YYYY'),'','', " & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))>=0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1)) ELSE 0 END AS DEBIT, " & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))<0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',-1,1)) ELSE 0 END AS CREDIT, " & vbCrLf & " '','', '',''"
        ElseIf OptSumDet(2).Checked = True Then
            SqlStr = SqlStr & vbCrLf & " TO_CHAR(Vdate,'YYYYMM'),'',SUBSTR(Vdate,4,3),'','', " & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))>=0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1)) ELSE 0 END AS DEBIT, " & vbCrLf & " CASE WHEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',1,-1))<0 THEN SUM(TRN.AMOUNT*DECODE(TRN.DC,'D',-1,1)) ELSE 0 END AS CREDIT, " & vbCrLf & " '','','',''"
        End If
...
ERR1:
        ...
        MakeSQLForInsert = ""
    End Function
    Private Function MakeOPSQLOLD(ByRef pAccountCode As String) As String
        On Error GoTo ERR1
        Dim SqlStr As String
...
ERR1:
    ...
    Private Function MakeOPSQL(ByRef pAccountCode As String) As String
        ...
        mFromFY = GetCurrentFYNo(PubDBCn, VB6.Format(txtDateFrom.Text, "DD/MM/YYYY"))
        mToFY = GetCurrentFYNo(PubDBCn, VB6.Format(txtDateTo.Text, "DD/MM/YYYY"))

        SqlStr = " SELECT " & vbCrLf & " SUM(DECODE(TRN.DC,'D',1,-1) * TRN.AMOUNT)  AS OPENING , 0 AS ADJUST_AMOUNT,0 AS UNADJUST_AMOUNT"

        SqlStr = SqlStr & vbCrLf _
            & " FROM FIN_POSTED_TRN TRN " & vbCrLf _
            & " WHERE  " & vbCrLf _
            & " TRN.ACCOUNTCODE='" & pAccountCode & "'" & vbCrLf _
...
ERR1:
        ...
        MakeOPSQL = ""
    End Function
    Private Function MakeSQLCondOld(ByRef mIsOpening As Boolean) As String
        On Error GoTo ERR1
        Dim SqlStr As String
...
ERR1:
    ...
    Private Function MakeSQLCond(ByRef mIsOpening As Boolean) As String
        ...
        SqlStr = SqlStr & vbCrLf & " TRN.Company_Code=ACM.Company_Code AND TRN.COMPANY_CODE=GCMT.COMPANY_CODE" & vbCrLf _
...
EventExitSub:
    ...
    Private Sub frmViewLedgerCal_Click(sender As Object, e As EventArgs) Handles Me.Click
