    ...
    Private Sub CmdPopFromFile_Click(sender As Object, e As EventArgs) Handles CmdPopFromFile.Click
        Try
            ...
            If Trim(txtBankName.Text) = "" Then
                MsgBox("Please select Bank First.")
                Exit Sub
            End If
...
NormalExit:
    ...
    Private Sub PopulateFromXLSFile(ByVal strXLSFile As String)
        ...
        Dim mChqNo As String
        Dim mChqDate As String
        Dim mChqAmount As Double

        Dim mCheckChqNo As String
        Dim mCheckClearDate As String
        Dim mCheckChqAmount As Double

        Dim mCheckSNO As String
        Dim mCheckDesc As String
        ...
        FPath = mPubBarCodePath & "\BankImportError.txt"
        ...
        If txtSendDate.Text = "" Then Exit Sub
        ...
        For Each dtRow In dt.Rows
            mCheckChqNo = UCase(Trim(IIf(IsDBNull(dtRow.Item(3)), "", dtRow.Item(3))))    ''Trim(IIf(IsDBNull(RsFile.Fields(0).Value), "", RsFile.Fields(0).Value))

            mCheckChqNo = VB6.Format(mCheckChqNo, "000000")
            mCheckClearDate = Trim(IIf(IsDBNull(dtRow.Item(1)), "", dtRow.Item(1)))
            mCheckChqAmount = Val(IIf(IsDBNull(dtRow.Item(4)), 0, dtRow.Item(4)))

            mCheckSNO = Trim(IIf(IsDBNull(dtRow.Item(0)), "", dtRow.Item(0)))
            mCheckDesc = Trim(IIf(IsDBNull(dtRow.Item(2)), "", dtRow.Item(2)))

            If IsDate(mCheckClearDate) = True Then
                If CDate(mCheckClearDate) <> CDate(txtSendDate.Text) Then
                    GoTo NextRecord
                End If
            End If

            For lngRow = 0 To UltraGrid1.Rows.Count - 1
                cntRow = lngRow
                mRow = UltraGrid1.Rows(lngRow)

                mChqNo = mRow.GetCellText(Me.UltraGrid1.DisplayLayout.Bands(0).Columns(ColChqNo - 1))
                mChqNo = UCase(mChqNo)
                mChqNo = VB6.Format(mChqNo, "000000")

                'mChqDate = mRow.GetCellText(Me.UltraGrid1.DisplayLayout.Bands(0).Columns(ColChqDate - 1))
                mChqAmount = mRow.GetCellText(Me.UltraGrid1.DisplayLayout.Bands(0).Columns(ColAmount - 1))

                If UCase(Trim(mCheckChqNo)) = UCase(Trim(mChqNo)) Then
                    If Math.Abs(Val(mCheckChqAmount)) = Math.Abs(Val(mChqAmount)) Then
                        'mRow.Cells(ColPostStatus - 1).Value = System.Windows.Forms.CheckState.Checked ' 1
                        UltraGrid1.Rows(cntRow).Cells(ColPostStatus - 1).Value = True
                        GoTo NextRecord
                    End If
                End If
            Next

            ErrorFile.WriteLine(mCheckSNO & " : " & mCheckDesc)

            '            Sl.No.Date    Description	Chq / Ref number	Amount	Dr / Cr	Balance	Dr / Cr
            '1   03/12/2024	CLG TO TRI WALL PAK PVT LTD IDBI BANK LTD.	50866	150,000.00	DR	-83,231,712.01	DR
