...
Friend Class frmMRRSendPurchase
    ...
    Private Sub CmdSave_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdSave.Click
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                ...
                If .Value = CStr(System.Windows.Forms.CheckState.Checked) Then
                    SqlStr = "UPDATE INV_GATE_HDR SET SEND_PUR_FLAG='Y', SEND_PUR_DATE=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY'),UPDATE_FROM='N'," & vbCrLf & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "'," & vbCrLf & " Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf & " WHERE AUTO_KEY_MRR=" & mMRRNO & ""
        ...
        End With
        PubDBCn.CommitTrans()
        MsgBox("Total " & mUpdateCount & " MRR Send to Purchase.", MsgBoxStyle.Information)
        Exit Sub
ErrPart:
        ''Resume
...
ERR1:
    ...
    End Function
    Public Sub frmMRRSendPurchase_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
    ...
    End Sub
    Private Sub frmMRRSendPurchase_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    Private Sub Show1()
        On Error GoTo ErrPart
        Dim SqlStr As String
        SqlStr = "SELECT GRMain.AUTO_KEY_MRR,GRMain.MRR_DATE,  " & vbCrLf & " ACM.SUPP_CUST_NAME, DECODE(SEND_PUR_FLAG,'Y','1','0') AS PFLAG " & vbCrLf & " FROM INV_GATE_HDR GRMain,FIN_SUPP_CUST_MST ACM" & vbCrLf & " WHERE GRMain.SUPP_CUST_CODE=ACM.SUPP_CUST_CODE " & vbCrLf & " AND GRMain.Company_Code=ACM.Company_Code " & vbCrLf & " AND GRMain.Company_Code=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND GRMain.MRR_FINAL_FLAG='N' AND GRMain.SEND_AC_FLAG='Y' "
        If chkShow.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            SqlStr = SqlStr & vbCrLf & " AND GRMain.SEND_PUR_FLAG='Y'"
            If txtDateFrom.Text <> "" Then
                SqlStr = SqlStr & vbCrLf & " AND GRMain.SEND_PUR_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND GRMain.SEND_PUR_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
            End If
        Else
            SqlStr = SqlStr & vbCrLf & " AND GRMain.SEND_PUR_FLAG='N'"
        End If
...
ErrPart:
    ...
    Private Sub WriteColHeadings()
        ...
        End With
    End Sub
    Private Sub frmMRRSendPurchase_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
    End Sub
