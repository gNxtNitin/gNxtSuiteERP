...
Friend Class frmParamDailyConsumtionReg
    ...
    Private Sub ReportOnIssue(ByRef Mode As Crystal.DestinationConstants)
        ...
        If OptDetSumm(0).Checked = True Then
            ...
            If OptOrderBy(0).Checked = True Then
                '            Report1.ReportFileName = App.path & "\Reports\MatIssueReg_Refno.rpt"	
            Else
        ...
        Else
            ...
            '        Report1.ReportFileName = App.path & "\Reports\MatIssueRegSumm.rpt"	
            SqlStr = MakeSQLSumm
        End If
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamDailyConsumtionReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamDailyConsumtionReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamDailyConsumtionReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamDailyConsumtionReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
    ...
    Private Sub SprdMain_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_DblClickEvent) Handles SprdMain.DblClick
        ...
        XRIGHT = MainClass.STRMenuRight(PubUserID, CurrModuleID, "MNUDAILYCONSUMPTION", PubDBCn)
        If InStr(1, XRIGHT, "M", CompareMethod.Text) = 0 Then
            Exit Sub
        End If

        frmConsumption.MdiParent = Me.MdiParent
        frmConsumption.Show()

        frmConsumption.frmConsumption_Activated(Nothing, New System.EventArgs())

        frmConsumption.txtNumber.Text = CStr(xIssueNo)

        frmConsumption.txtNumber_Validating(Nothing, New System.ComponentModel.CancelEventArgs(False))
    End Sub
...
LedgError:
    ...
    Private Function MakeSQLDet() As String
        ...
        MakeSQLDet = " SELECT ''," & vbCrLf & " IGH.AUTO_KEY_CONS," & vbCrLf & " TO_CHAR(IGH.CONS_DATE,'DD/MM/YYYY')," & vbCrLf & " IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC," & vbCrLf & " IGD.ITEM_UOM, " & vbCrLf & " TO_CHAR(IGD.CONS_QTY),IGH.AUTO_KEY_CONS"

        ''FROM CLAUSE...	
        MakeSQLDet = MakeSQLDet & vbCrLf & " FROM PRD_CONSUMPTION_HDR IGH, PRD_CONSUMPTION_DET IGD," & vbCrLf & " INV_ITEM_MST INVMST"

        ''WHERE CLAUSE...	
        MakeSQLDet = MakeSQLDet & vbCrLf & " WHERE " & vbCrLf & " IGH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IGH.AUTO_KEY_CONS=IGD.AUTO_KEY_CONS" & vbCrLf & " AND IGD.COMPANY_CODE=INVMST.COMPANY_CODE" & vbCrLf & " AND IGD.ITEM_CODE=INVMST.ITEM_CODE "
        ...
        MakeSQLDet = MakeSQLDet & vbCrLf & " AND IGH.CONS_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IGH.CONS_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"



        If OptOrderBy(0).Checked = True Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "ORDER BY IGH.AUTO_KEY_CONS, IGH.CONS_DATE ,IGD.SERIAL_NO"
        ElseIf OptOrderBy(1).Checked = True Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "ORDER BY INVMST.ITEM_SHORT_DESC,IGH.AUTO_KEY_CONS, IGH.CONS_DATE"
        End If
        'End If	
        Exit Function
ERR1:
    ...
    Private Function MakeSQLSumm() As String
        ...
        MakeSQLSumm = " SELECT ''," & vbCrLf & " ''," & vbCrLf & " ''," & vbCrLf & " IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC," & vbCrLf & " IGD.ITEM_UOM,  " & vbCrLf & " TO_CHAR(SUM(IGD.CONS_QTY)),''"

        ''FROM CLAUSE...	
        MakeSQLSumm = MakeSQLSumm & vbCrLf & " FROM PRD_CONSUMPTION_HDR IGH, PRD_CONSUMPTION_DET IGD," & vbCrLf & " INV_ITEM_MST INVMST"

        ''WHERE CLAUSE...	
        MakeSQLSumm = MakeSQLSumm & vbCrLf & " WHERE " & vbCrLf & " IGH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IGH.AUTO_KEY_CONS=IGD.AUTO_KEY_CONS" & vbCrLf & " AND IGH.COMPANY_CODE=INVMST.COMPANY_CODE" & vbCrLf & " AND IGD.ITEM_CODE=INVMST.ITEM_CODE "

        MakeSQLSumm = MakeSQLSumm & vbCrLf & " AND IGH.CONS_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IGH.CONS_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        MakeSQLSumm = MakeSQLSumm & vbCrLf & "GROUP BY IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC,IGD.ITEM_UOM "
