...
Friend Class frmParamPendIndReg
    ...
    Private Const ColMRRQty As Short = 16
    Private Const ColBalQty As Short = 17
    Private Const ColRate As Short = 18
    Private Const ColBalAmount As Short = 19
    ...
    Private Sub ReportonIndent(ByRef Mode As Crystal.DestinationConstants)
        ...
        Else
            mRPTName = "IndentPendRegMRRDet.RPT"
        End If
    ...
    Private Function InsertPrintDummy() As Boolean
        ...
        With SprdMain
            For CntRow = 1 To .MaxRows
                ...
                SqlStr = "Insert into Temp_PrintDummyData (UserID,SubRow, " & vbCrLf & " Field1,Field2,Field3,Field4,Field5," & vbCrLf & " Field6,Field7,Field8,Field9, " & vbCrLf & " Field10,Field11,Field12,Field13,Field14, " & vbCrLf & " Field15,Field16,Field17) Values (" & vbCrLf & " '" & MainClass.AllowSingleQuote(PubUserID) & "', " & vbCrLf & " " & CntRow & ", " & vbCrLf & " '" & mGroup & "', " & vbCrLf & " '" & mIndentNo & "', " & vbCrLf & " '" & mIndentDate & "', " & vbCrLf & " '" & mDeptCode & "', " & vbCrLf & " '" & mIndEmpCode & "', " & vbCrLf & " '" & mItemCode & "', " & vbCrLf & " '" & mItemDesc & "', " & vbCrLf & " '" & mUnit & "', " & vbCrLf & " '" & mIndentQty & "','" & mPONo & "', " & vbCrLf & " '" & mPODate & "','" & mSupplier & "', " & vbCrLf & " '" & mPOQty & "','" & mBalQty & "', " & vbCrLf & " '" & mMRRNO & "','" & mMRRDATE & "','" & mMRRQty & "') "
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamPendIndReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPendIndReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
        ...
        MsgInformation(Err.Description)
    End Sub
    Private Sub frmParamPendIndReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPendIndReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
    ...
    Private Sub SprdMain_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_DblClickEvent) Handles SprdMain.DblClick
        'Dim SqlStr As String = ""
        'Dim RsTemp As ADODB.Recordset=Nothing= Nothing=Nothing
...
EventExitSub:
    ...
    Private Sub txtIndentor_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtIndentor.Validating
        ...
        If MainClass.ValidateWithMasterTable((txtIndentor.Text), "USER_ID", "EMP_NAME", "ATH_PASSWORD_MST", PubDBCn, MasterNo, , SqlStr) = True Then
        ...
        If RsShow.EOF = False Then
            With SprdMain
                Do While Not RsShow.EOF
                    ...
                    mBalQty = IIf(IsDbNull(RsShow.Fields("BAL_QTY").Value), "", RsShow.Fields("BAL_QTY").Value)
                    ...
                    mSqlStr = "SELECT IH.AUTO_KEY_PO,IH.PUR_ORD_DATE,IH.SUPP_CUST_CODE, " & vbCrLf _
                        & " SUM(IT.INDENT_QTY) AS ITEM_QTY, " & vbCrLf _
                        & " SUM(((NVL(ITEM_PRICE,0) - ROUND((NVL(ITEM_PRICE,0) * ITEM_DIS_PER)/100,4)) * EXCHANGERATE)) AS ITEM_RATE " & vbCrLf _
                        & " FROM " & vbCrLf _
                        & " PUR_PURCHASE_HDR IH, PUR_PURCHASE_DET ID, PUR_POCONS_IND_TRN IT " & vbCrLf _
                        & " WHERE IH.MKEY=ID.MKEY " & vbCrLf _
                        & " AND ID.MKEY=IT.MKEY " & vbCrLf _
                    ...
                    If RsPO.EOF = False Then
                        Do While Not RsPO.EOF
                            ...
                            .Col = ColRate
                            .Text = VB6.Format(IIf(IsDbNull(RsPO.Fields("ITEM_RATE").Value), "", RsPO.Fields("ITEM_RATE").Value), "0.00")

                            .Col = ColBalAmount
                            .Text = VB6.Format(IIf(IsDbNull(RsPO.Fields("ITEM_RATE").Value), "", RsPO.Fields("ITEM_RATE").Value) * mBalQty, "0.00")

                            mSqlStr = "SELECT IH.AUTO_KEY_MRR,IH.MRR_DATE, " & vbCrLf _
                                & " SUM(ID.RECEIVED_QTY) AS RECD_QTY FROM " & vbCrLf _
                                & " INV_GATE_HDR IH, INV_GATE_DET ID " & vbCrLf _
                                & " WHERE IH.AUTO_KEY_MRR=ID.AUTO_KEY_MRR " & vbCrLf _
                                & " AND ID.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND IH.REF_TYPE='P' AND ID.REF_AUTO_KEY_NO=" & RsPO.Fields("AUTO_KEY_PO").Value & "" & vbCrLf _
                                & " AND ID.ITEM_CODE='" & MainClass.AllowSingleQuote(mItemCode) & "' " & vbCrLf _
                                & " GROUP BY IH.AUTO_KEY_MRR,IH.MRR_DATE"

                            MainClass.UOpenRecordSet(mSqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsMRR, ADODB.LockTypeEnum.adLockReadOnly)
                            ...
                            If RsMRR.EOF = False Then
                                Do While Not RsMRR.EOF
                                    ...
                                    .Col = ColMRRNo
                                    .Text = CStr(IIf(IsDbNull(RsMRR.Fields("AUTO_KEY_MRR").Value), "", RsMRR.Fields("AUTO_KEY_MRR").Value))

                                    .Col = ColMRRDate
                                    .Text = VB6.Format(IIf(IsDbNull(RsMRR.Fields("MRR_DATE").Value), "", RsMRR.Fields("MRR_DATE").Value), "DD/MM/YYYY")

                                    .Col = ColMRRQty
                                    .Text = VB6.Format(IIf(IsDbNull(RsMRR.Fields("RECD_QTY").Value), "", RsMRR.Fields("RECD_QTY").Value), "0.00")
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT '',IH.AUTO_KEY_INDENT, IH.INDENT_DATE, " & vbCrLf & " IH.DEPT_CODE, EMP.EMP_NAME, IH.ITEM_CODE," & vbCrLf & " INVMST.ITEM_SHORT_DESC, IH.ITEM_UOM, " & vbCrLf & " TO_CHAR(IH.INDENT_QTY) INDENT_QTY, " & vbCrLf & " '', '', '',0, '','',TO_CHAR(PO_QTY), " & vbCrLf & " TO_CHAR(IH.INDENT_QTY - NVL(PO_QTY,0)) BAL_QTY,IH.AUTO_KEY_INDENT"
        ...
        If CboStatus.SelectedIndex = 1 Then
            MakeSQL = MakeSQL & vbCrLf & "AND (IH.INDENT_QTY - NVL(PO_QTY,0) > 0 AND IH.INDENT_STATUS = 'N')" ''PO_QTY>INDENT_QTY AND
        ...
        End If
        ''AND PO_QTY>=REQ_QTY AND PO_QTY>REC_QTY
        ...
        '    If cboAppStatus.ListIndex = 1 Then
        '        MakeSQL = MakeSQL & vbCrLf & "AND IH.APP_STATUS = 'Y'"
        '    ElseIf cboAppStatus.ListIndex = 2 Then
        '        MakeSQL = MakeSQL & vbCrLf & "AND IH.APP_STATUS = 'N'"
        '    End If


        MakeSQL = MakeSQL & vbCrLf & " AND IH.INDENT_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IH.INDENT_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
...
FillErr2:
    ...
    Private Function InsertIntoTempTable() As Boolean
        ...
        SqlStr1 = " INSERT INTO TEMP_INDENT_REG (" & vbCrLf & " USERID, COMPANY_CODE, AUTO_KEY_INDENT, INDENT_DATE, " & vbCrLf & " DEPT_CODE , SERIAL_NO, ITEM_CODE, ITEM_UOM, INDENT_STATUS, " & vbCrLf & " APP_STATUS, INDENT_QTY, PO_QTY, IND_EMP_CODE,DIV_CODE) "



        SqlStr2 = " SELECT " & vbCrLf & " '" & MainClass.AllowSingleQuote(PubUserID) & "', IH.COMPANY_CODE, IH.AUTO_KEY_INDENT, IH.INDENT_DATE," & vbCrLf & " IH.DEPT_CODE , ID.SERIAL_NO, ID.ITEM_CODE, ID.ITEM_UOM, ID.INDENT_STATUS, " & vbCrLf & " IH.APPROVAL_STATUS, " & vbCrLf & " ID.REQ_QTY, GETMRRPOIndentQty(" & RsCompany.Fields("COMPANY_CODE").Value & ",IH.AUTO_KEY_INDENT,ID.ITEM_CODE) AS POQTY, IH.IND_EMP_CODE, IH.DIV_CODE" & vbCrLf & " FROM PUR_INDENT_HDR IH, PUR_INDENT_DET ID, INV_ITEM_MST INVMST" & vbCrLf & " WHERE IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IH.AUTO_KEY_INDENT=ID.AUTO_KEY_INDENT AND ID.COMPANY_CODE=INVMST.COMPANY_CODE AND ID.ITEM_CODE=INVMST.ITEM_CODE AND IH.HOD_EMP_CODE IS NOT NULL"
        ...
        If chkAllCategory.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            If MainClass.ValidateWithMasterTable(txtCategory.Text, "GEN_DESC", "GEN_CODE", "INV_GENERAL_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND GEN_TYPE='C'") = True Then
                mCatCode = MasterNo
                SqlStr2 = SqlStr2 & vbCrLf & "AND INVMST.CATEGORY_CODE='" & MainClass.AllowSingleQuote(mCatCode) & "'"
            End If
        End If
        ...
        If chkAllSubCat.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            If MainClass.ValidateWithMasterTable(txtSubCategory.Text, "SUBCATEGORY_DESC", "SUBCATEGORY_CODE", "INV_SUBCATEGORY_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND CATEGORY_CODE='" & mCatCode & "'") = True Then
                ...
                SqlStr2 = SqlStr2 & vbCrLf & "AND INVMST.SUBCATEGORY_CODE='" & MainClass.AllowSingleQuote(mSubCatCode) & "'"
            End If
        End If
