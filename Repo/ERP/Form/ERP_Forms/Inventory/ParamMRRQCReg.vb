...
Friend Class frmParamMrrQCReg
    Inherits System.Windows.Forms.Form
    Dim XRIGHT As String
    Private Const RowHeight As Short = 20

    Private Const ColCatDesc As Short = 1
    Private Const ColTotMRR As Short = 2
    Private Const ColQC1 As Short = 3
    Private Const colQC2 As Short = 4
    Private Const ColQC3 As Short = 5
    Private Const ColQC4 As Short = 6
    Private Const ColQC5 As Short = 7
    Private Const ColQC6 As Short = 8
    Private Const ColQC1Week As Short = 9
    Private Const ColQCNotDone As Short = 10
...
ERR1:
    ...
    Private Sub ReportONMRR(ByRef Mode As Crystal.DestinationConstants)
        ...
        Report1.ReportFileName = My.Application.Info.DirectoryPath & "\Reports\MrrQCStatus.rpt"
...
ErrPart:
    ...
    Private Sub frmParamMrrQCReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        If FormActive = True Then Exit Sub

        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor

        Me.Text = "MRR QC Status"
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        FormActive = True
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamMrrQCReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamMrrQCReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamMrrQCReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Close()
    End Sub
...
EventExitSub:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            .set_ColWidth(ColCatDesc, 22)
            .ColsFrozen = ColCatDesc

            For cntCol = ColTotMRR To ColQCNotDone
                .Col = cntCol
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT CMST.GEN_DESC,COUNT(DISTINCT ID.AUTO_KEY_MRR),"

        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE<=1 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE=2 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE=3 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE=4 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE=5 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE=6 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE-ID.MRR_DATE>6 THEN ID.AUTO_KEY_MRR END),"
        MakeSQL = MakeSQL & vbCrLf & " COUNT(DISTINCT CASE WHEN ID.MRR_QCDATE IS NULL OR ID.MRR_QCDATE='' THEN ID.AUTO_KEY_MRR END)"


        ''FROM CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " FROM INV_GATE_HDR IH, INV_GATE_DET ID," & vbCrLf & " INV_ITEM_MST INVMST, INV_GENERAL_MST CMST"

        ''WHERE CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " WHERE " & vbCrLf & " IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND IH.AUTO_KEY_MRR=ID.AUTO_KEY_MRR" & vbCrLf & " AND ID.COMPANY_CODE=INVMST.COMPANY_CODE" & vbCrLf & " AND ID.ITEM_CODE=INVMST.ITEM_CODE " & vbCrLf & " AND INVMST.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND INVMST.CATEGORY_CODE=CMST.GEN_CODE " & vbCrLf & " AND GEN_TYPE='C'"

        If chkAllCategory.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            MakeSQL = MakeSQL & vbCrLf & "AND CMST.GEN_DESC='" & MainClass.AllowSingleQuote(txtCategory.Text) & "'"
        ...
        If cboRefType.SelectedIndex <> 0 Then
            MakeSQL = MakeSQL & vbCrLf & "AND ID.REF_TYPE='" & VB.Left(cboRefType.Text, 1) & "' "
        End If
        ...
        If chkAllSupp.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            If MainClass.ValidateWithMasterTable(txtSupplier.Text, "SUPP_CUST_NAME", "SUPP_CUST_CODE", "FIN_SUPP_CUST_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mSupplier = MasterNo
                MakeSQL = MakeSQL & vbCrLf & "AND ID.SUPP_CUST_CODE='" & MainClass.AllowSingleQuote(mSupplier) & "'"
            End If
        End If
        ...
        If chkAllName.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            If MainClass.ValidateWithMasterTable(txtEmpName.Text, "EMP_NAME", "EMP_CODE", "PAY_EMPLOYEE_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mEmployee = MasterNo
                MakeSQL = MakeSQL & vbCrLf & "AND ID.QC_EMP_CODE='" & MainClass.AllowSingleQuote(mEmployee) & "'"
            End If
        End If

        MakeSQL = MakeSQL & vbCrLf & " AND ID.MRR_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND ID.MRR_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"

        'GROUP BY CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & "GROUP BY CMST.GEN_DESC"

        'ORDER CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & "ORDER BY CMST.GEN_DESC"
        'End If
        Exit Function
ERR1:
        MsgInformation(Err.Description)
