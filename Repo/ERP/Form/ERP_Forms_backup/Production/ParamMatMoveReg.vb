...
Friend Class frmParamMatMoveReg
    ...
    Private Const ColShift As Short = 6
    ...
    Private Sub ReportOnIssue(ByRef Mode As Crystal.DestinationConstants)
        ...
        If OptDetSumm(0).Checked = True Then
            ...
            Report1.ReportFileName = My.Application.Info.DirectoryPath & "\Reports\MatMoveReg.rpt"
            '        Else	
        ...
        Else
            ...
            Report1.ReportFileName = My.Application.Info.DirectoryPath & "\Reports\MatMoveRegSumm.rpt"
            SqlStr = MakeSQLSumm
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamMatMoveReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamMatMoveReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamMatMoveReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamMatMoveReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
    ...
    Private Sub SprdMain_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_DblClickEvent) Handles SprdMain.DblClick
        ...
        FrmProdIssuRecvNote.txtIssueNo.Text = CStr(xIssueNo)
...
LedgError:
    ...
    Private Function MakeSQLDet() As String
        ...
        MakeSQLDet = " SELECT ''," & vbCrLf & " IGH.AUTO_KEY_ISSREC, TO_CHAR(IGH.ISSREC_DATE,'DD/MM/YYYY'), " & vbCrLf & " IGH.FROM_DEPT, IGH.TO_DEPT, IGH.SHIFT_CODE, " & vbCrLf & " IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC, IGD.ITEM_UOM, " & vbCrLf & " TO_CHAR(IGD.ISSUE_QTY), TO_CHAR(IGD.RECV_QTY), " & vbCrLf & " IGH.AUTO_KEY_ISSREC"

        ''FROM CLAUSE...	
        MakeSQLDet = MakeSQLDet & vbCrLf & " FROM PRD_ISSREC_HDR IGH, PRD_ISSREC_DET IGD," & vbCrLf & " INV_ITEM_MST INVMST"

        ''WHERE CLAUSE...	
        MakeSQLDet = MakeSQLDet & vbCrLf & " WHERE " & vbCrLf & " IGH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IGH.AUTO_KEY_ISSREC=IGD.AUTO_KEY_ISSREC" & vbCrLf & " AND IGD.COMPANY_CODE=INVMST.COMPANY_CODE" & vbCrLf & " AND IGD.ITEM_CODE=INVMST.ITEM_CODE "
        ...
        If cboDept.Text <> "ALL" Then
            If MainClass.ValidateWithMasterTable(cboDept.Text, "DEPT_DESC", "DEPT_CODE", "PAY_DEPT_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mDept = MasterNo
                MakeSQLDet = MakeSQLDet & vbCrLf & "AND IGH.FROM_DEPT='" & MainClass.AllowSingleQuote(mDept) & "'"
            End If
        End If
        ...
        If cboStatus.SelectedIndex = 1 Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "AND IGH.RECV_STATUS='N'"
        ElseIf cboStatus.SelectedIndex = 2 Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "AND IGH.RECV_STATUS='Y'"
        End If

        MakeSQLDet = MakeSQLDet & vbCrLf & " AND IGH.ISSREC_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IGH.ISSREC_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        If OptOrderBy(0).Checked = True Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "ORDER BY IGH.ISSREC_DATE ,IGH.AUTO_KEY_ISSREC, IGD.SERIAL_NO"
        ElseIf OptOrderBy(1).Checked = True Then
            MakeSQLDet = MakeSQLDet & vbCrLf & "ORDER BY INVMST.ITEM_SHORT_DESC, IGH.ISSREC_DATE,IGH.AUTO_KEY_ISSREC"
        End If
        'End If	
        Exit Function
ERR1:
    ...
    Private Function MakeSQLSumm() As String
        ...
        MakeSQLSumm = " SELECT ''," & vbCrLf & " '', '', " & vbCrLf & " IGH.FROM_DEPT, IGH.TO_DEPT, '', " & vbCrLf & " IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC," & vbCrLf & " IGD.ITEM_UOM, TO_CHAR(SUM(IGD.ISSUE_QTY)), " & vbCrLf & " TO_CHAR(SUM(IGD.RECV_QTY)),''"

        ''FROM CLAUSE...	
        MakeSQLSumm = MakeSQLSumm & vbCrLf & " FROM PRD_ISSREC_HDR IGH, PRD_ISSREC_DET IGD," & vbCrLf & " INV_ITEM_MST INVMST"

        ''WHERE CLAUSE...	
        MakeSQLSumm = MakeSQLSumm & vbCrLf & " WHERE " & vbCrLf & " IGH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IGH.AUTO_KEY_ISSREC=IGD.AUTO_KEY_ISSREC" & vbCrLf & " AND IGH.COMPANY_CODE=INVMST.COMPANY_CODE" & vbCrLf & " AND IGD.ITEM_CODE=INVMST.ITEM_CODE "

        MakeSQLSumm = MakeSQLSumm & vbCrLf & " AND IGH.ISSREC_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf & " AND IGH.ISSREC_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        If cboDept.Text <> "ALL" Then
            If MainClass.ValidateWithMasterTable(cboDept.Text, "DEPT_DESC", "DEPT_CODE", "PAY_DEPT_MST", PubDBCn, MasterNo, , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mDept = MasterNo
                MakeSQLSumm = MakeSQLSumm & vbCrLf & "AND IGH.FROM_DEPT='" & MainClass.AllowSingleQuote(mDept) & "'"
            End If
        End If
        ...
        If cboStatus.SelectedIndex = 1 Then
            MakeSQLSumm = MakeSQLSumm & vbCrLf & "AND IGH.RECV_STATUS='N'"
        ElseIf cboStatus.SelectedIndex = 2 Then
            MakeSQLSumm = MakeSQLSumm & vbCrLf & "AND IGH.RECV_STATUS='Y'"
        End If

        MakeSQLSumm = MakeSQLSumm & vbCrLf & "GROUP BY IGH.FROM_DEPT, IGH.TO_DEPT, IGD.ITEM_CODE, INVMST.ITEM_SHORT_DESC,IGD.ITEM_UOM "

        If OptOrderBy(0).Checked = True Then
            MakeSQLSumm = MakeSQLSumm & vbCrLf & "ORDER BY IGH.FROM_DEPT, IGH.TO_DEPT, IGD.ITEM_CODE,INVMST.ITEM_SHORT_DESC"
        ElseIf OptOrderBy(1).Checked = True Then
            MakeSQLSumm = MakeSQLSumm & vbCrLf & "ORDER BY IGH.FROM_DEPT, IGH.TO_DEPT, INVMST.ITEM_SHORT_DESC,IGD.ITEM_CODE"
        End If
