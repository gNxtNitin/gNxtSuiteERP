...
Friend Class frmParamPOTermsReg
    ...
    Private Const colSupplier As Short = 8
    Private Const colPayDays As Short = 9
    Private Const colDelivery As Short = 10
    Private Const colExcise As Short = 11
    Private Const colPayments As Short = 12
    Private Const colMode As Short = 13
    Private Const colInspection As Short = 14
    Private Const colPF As Short = 15
    ...
    Private Sub ReportonPO(ByRef Mode As Crystal.DestinationConstants)
        ...
        Report1.ReportFileName = My.Application.Info.DirectoryPath & "\Reports\POTermsReg.RPT"
...
ErrPart:
    ...
    End Sub
    Private Sub frmParamPOTermsReg_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        If FormActive = True Then Exit Sub

        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor

        Me.Text = "PO Payment Terms  Register"
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        FormActive = True
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPOTermsReg_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmParamPOTermsReg_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamPOTermsReg_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
    ...
    Private Sub SprdMain_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As AxFPSpreadADO._DSpreadEvents_DblClickEvent) Handles SprdMain.DblClick
        ...
        '        frmPO.Form_Activate
        '
        '        frmPO.txtPONo = RsTemp!AUTO_KEY_PO
        '        frmPO.txtAmendNo = RsTemp!AMEND_NO
        '
        '        frmPO.txtPONO_Validate False
        '
        '
        '
        '
...
ERR1:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For cntCol = colPayDays To colRemarks
                .Col = cntCol
                .CellType = SS_CELL_TYPE_EDIT
                .TypeHAlign = SS_CELL_H_ALIGN_LEFT
                .TypeEditLen = 255
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT ''," & vbCrLf & " IH.AUTO_KEY_PO, TO_CHAR(IH.PUR_ORD_DATE,'DD/MM/YYYY')," & vbCrLf & " AMEND_NO, TO_CHAR(IH.AMEND_DATE,'DD/MM/YYYY')," & vbCrLf & " TO_CHAR(IH.AMEND_WEF_DATE,'DD/MM/YYYY')," & vbCrLf & " PUR_TYPE || ORDER_TYPE AS PUR_TYPE, CMST.SUPP_CUST_NAME," & vbCrLf & " IH.PAYDAYS, IH.DELIVERY, IH.EXCISE_OTHERS, " & vbCrLf & " PAYMST.PAY_TERM_DESC, IH.MODE_DESPATCH, " & vbCrLf & " IH.INSPECTION, IH.PACKING_FORWARDING, " & vbCrLf & " IH.REMARKS, " & vbCrLf & " IH.AUTO_KEY_PO "

        ''''FROM CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " FROM PUR_PURCHASE_HDR IH, " & vbCrLf & " FIN_SUPP_CUST_MST CMST, FIN_PAYTERM_MST PAYMST"

        ''''WHERE CLAUSE...
        MakeSQL = MakeSQL & vbCrLf & " WHERE " & vbCrLf & " IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE " & vbCrLf & " AND IH.COMPANY_CODE=PAYMST.COMPANY_CODE(+)" & vbCrLf & " AND IH.PAYMENT_CODE=PAYMST.PAY_TERM_CODE(+)" & vbCrLf
        ...
        MakeSQL = MakeSQL & vbCrLf & "ORDER BY IH.MKEY,IH.SUPP_CUST_CODE, IH.PUR_ORD_DATE"
        '    ElseIf optOrderBy(1).Value = True Then
        '        MakeSQL = MakeSQL & vbCrLf & "ORDER BY INVMST.ITEM_SHORT_DESC,IH.MKEY, IH.PUR_ORD_DATE"
        '    End If
        Exit Function
ERR1:
        MsgInformation(Err.Description)
