...
Friend Class frmPOPrintApproval
    ...
    Private Sub CmdSave_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdSave.Click
        ...
        mAuthorisation = GetUserPermission("ALLOW_POPRINT_APP", "N", PubUserID, RsCompany.Fields("COMPANY_CODE").Value)

        If mAuthorisation = "N" Then
            MsgBox("You have no Right to Approve PO for Printing. ", MsgBoxStyle.Critical)
            Exit Sub
        End If
        ...
        With SprdMain
            For CntRow = 1 To .MaxRows
                ...
                If .Value = CStr(System.Windows.Forms.CheckState.Checked) Then
                    ...
                    SqlStr = "UPDATE PUR_PURCHASE_HDR SET PO_PRINT_APP='Y', UPDATE_FROM='N'," & vbCrLf _
                        & " ModUser='" & MainClass.AllowSingleQuote(PubUserID) & "'," & vbCrLf _
                        & " Moddate=TO_DATE('" & VB6.Format(PubCurrDate, "dd-MMM-yyyy") & "','DD-MON-YYYY') " & vbCrLf _
                        & " WHERE MKEY=" & mMKEY & "" & vbCrLf & " AND COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & ""
        ...
        End With
        PubDBCn.CommitTrans()
        MsgBox("Total " & mUpdateCount & " PO approved for Print.", MsgBoxStyle.Information)
        Exit Sub
ErrPart:
        '    Resume
...
ERR1:
        FieldsVerification = False
    End Function
    Public Sub frmPOPrintApproval_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
    ...
    End Sub
    Private Sub frmPOPrintApproval_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    Private Sub Show1()
        ...
        SqlStr = "SELECT POMain.MKEY, POMain.AUTO_KEY_PO, TO_CHAR(POMAIN.AMEND_WEF_DATE,'DD/MM/YYYY') AS AMEND_WEF_DATE ,AMEND_NO,  " & vbCrLf & " CASE WHEN POMain.PUR_TYPE='P' THEN 'Purchase Order'  " & vbCrLf & " WHEN POMain.PUR_TYPE='W' THEN 'Work Order' " & vbCrLf & " WHEN POMain.PUR_TYPE='J' THEN 'Job Work Order' END AS Purchase_Type, " & vbCrLf & " CASE WHEN POMain.ORDER_TYPE='O' THEN 'Open'  " & vbCrLf & " WHEN POMain.ORDER_TYPE='C' THEN 'Close' END AS Order_Type, " & vbCrLf & " ACM.SUPP_CUST_NAME,To_CHAR(SUM(GROSS_AMT)) AS ITEM_VALUE,'' " & vbCrLf & " FROM PUR_PURCHASE_HDR POMain, PUR_PURCHASE_DET POD, FIN_SUPP_CUST_MST ACM" & vbCrLf & " WHERE " & vbCrLf & " POMain.MKEY=POD.MKEY " & vbCrLf & " AND POMain.SUPP_CUST_CODE=ACM.SUPP_CUST_CODE " & vbCrLf & " AND POMain.Company_Code=ACM.Company_Code " & vbCrLf & " AND POMain.Company_Code=" & RsCompany.Fields("COMPANY_CODE").Value & " "
        ...
        SqlStr = SqlStr & vbCrLf & " AND POMain.PO_PRINT_APP='N' AND POMain.PO_STATUS='N' "
        ...
        SqlStr = SqlStr & vbCrLf & " AND POMain.PUR_ORD_DATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') AND POMain.PUR_ORD_DATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') "
...
ErrPart:
    ...
    Private Sub WriteColHeadings()
        ...
        End With
    End Sub
    Private Sub frmPOPrintApproval_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
    End Sub
