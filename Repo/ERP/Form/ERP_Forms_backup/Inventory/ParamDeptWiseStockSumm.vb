...
Friend Class frmParamDeptWiseStockSumm
            ...
            If FormLoaded = True Then
                ...
                If RsTemp.EOF = False Then
                    Do While RsTemp.EOF = False
                        ...
                        .Col = ColTotalClosing
                        .Text = IIf(IsDbNull(RsTemp.Fields("DEPT_CODE_TO").Value), "", RsTemp.Fields("DEPT_CODE_TO").Value) & " Value"
                    ...
                    Do While mItemCode = Trim(IIf(IsDbNull(RsTemp.Fields("ITEM_CODE").Value), "", RsTemp.Fields("ITEM_CODE").Value))
                        ...
                        For cntCol = ColRate + 1 To .MaxCols - 1
                            ...
                            If Trim(mDept) = Trim(.Text) Then
                                ...
                                If chkDeptQty.CheckState = System.Windows.Forms.CheckState.Checked Then
                                    mDeptBalanceQty = GetBalanceStockQty(mItemCode, VB6.Format(mFromDate, "DD/MM/YYYY"), IIf(IsDbNull(RsTemp.Fields("ITEM_UOM").Value), "", RsTemp.Fields("ITEM_UOM").Value), Trim(IIf(IsDbNull(RsTemp.Fields("DEPT_CODE_TO").Value), "", RsTemp.Fields("DEPT_CODE_TO").Value)), "ST", "", ConPH, -1)
                                    mDeptBalanceQty = mDeptBalanceQty - GetBalanceStockQty(mItemCode, VB6.Format(txtDateTo.Text, "DD/MM/YYYY"), IIf(IsDbNull(RsTemp.Fields("ITEM_UOM").Value), "", RsTemp.Fields("ITEM_UOM").Value), Trim(IIf(IsDbNull(RsTemp.Fields("DEPT_CODE_TO").Value), "", RsTemp.Fields("DEPT_CODE_TO").Value)), "ST", "", ConPH, -1)
                                Else
                                    mDeptBalanceQty = 0
                                End If
                                ...
                                .Col = cntCol + 1
                                .Text = CStr(Val(IIf(IsDbNull(RsTemp.Fields("QTY").Value), "", RsTemp.Fields("QTY").Value)) * mRate)
...
InsertErr:
    ...
    Private Function MakeSQL() As String
        ...
        SqlStr = " SELECT '" & MainClass.AllowSingleQuote(UCase(PubUserID)) & "', " & vbCrLf & " CMST.GEN_DESC, " & vbCrLf & " ITEM.SUBCATEGORY_CODE, ITEM.ITEM_CODE, " & vbCrLf & " ITEM.ITEM_SHORT_DESC, INV.ITEM_UOM, DEPT_CODE_FROM AS DEPT_CODE_TO, "

        SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(ITEM_QTY * DECODE(ITEM_IO,'I',-1,1))) AS QTY "

        SqlStr = SqlStr & vbCrLf & CondSQL

        SqlStr = SqlStr & vbCrLf & "GROUP BY " & vbCrLf & " CMST.GEN_DESC, ITEM.SUBCATEGORY_CODE, ITEM.ITEM_CODE, " & vbCrLf & " ITEM.ITEM_SHORT_DESC, INV.ITEM_UOM, DEPT_CODE_FROM "


        SqlStr = SqlStr & vbCrLf & "ORDER BY " & vbCrLf & " CMST.GEN_DESC,  ITEM.SUBCATEGORY_CODE, ITEM.ITEM_CODE,DEPT_CODE_FROM "
...
InsertErr:
    ...
    Private Function MakeDeptSQL() As String
        ...
        SqlStr = " SELECT DISTINCT DEPT_CODE_FROM AS DEPT_CODE_TO "


        SqlStr = SqlStr & vbCrLf & CondSQL

        SqlStr = SqlStr & vbCrLf & "ORDER BY " & vbCrLf & " DEPT_CODE_FROM"
...
InsertErr:
    ...
    Private Function CondSQL() As String
        ...
        SqlStr = SqlStr & vbCrLf & " AND INV.REF_TYPE IN ('ADJ','DTN','FBU','ISS','PDM','PIS','PMD','PMO','PMS','PRW','RWP','SCP','SIS','WBU')"
        ...
        SqlStr = SqlStr & vbCrLf & " AND INV.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND INV.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND INV.STOCK_ID = '" & ConWH & "'"


        SqlStr = SqlStr & vbCrLf & " AND INV.REF_TYPE IN ('" & ConStockRefType_ISS & "', '" & ConStockRefType_SRN & "') "

        SqlStr = SqlStr & vbCrLf & " AND INV.STOCK_TYPE IN ('ST','RJ') "

        If cboExportItem.SelectedIndex >= 1 Then
            SqlStr = SqlStr & vbCrLf & " AND INV.STOCK_ID = '" & VB.Left(cboExportItem.Text, 1) & "'"
        End If
...
InsertErr:
    ...
    Private Sub frmParamDeptWiseStockSumm_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
    ...
    Private Sub frmParamDeptWiseStockSumm_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
err_Renamed:
        ...
        MsgBox(Err.Description)
    End Sub
    Private Sub frmParamDeptWiseStockSumm_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
ErrPart:
    ...
    Private Sub frmParamDeptWiseStockSumm_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
