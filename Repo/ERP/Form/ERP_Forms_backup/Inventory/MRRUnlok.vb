...
AddErr:
    ...
    Private Sub cmdRequestSearch_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdRequestSearch.Click
        ...
        Else
            SqlStr = SqlStr & vbCrLf & " AND (EMP_LEAVE_DATE IS NULL OR (EMP_LEAVE_DATE >= TO_DATE('" & VB6.Format(txtMRRDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')))"
        End If

        If MainClass.SearchGridMaster((txtRequestCode.Text), "PAY_EMPLOYEE_MST", "EMP_CODE", "EMP_NAME", , , SqlStr) = True Then
            txtRequestCode.Text = AcName
            txtRequestName.Text = AcName1
            txtRequestCode_Validating(txtRequestCode, New System.ComponentModel.CancelEventArgs(False))
    ...
    Private Sub CmdDelete_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDelete.Click
        ...
        If Not RsTransMain.EOF Then
            If MsgQuestion("Want to Delete ? ") = CStr(MsgBoxResult.Yes) Then ' User chose Yes.
                ...
                PubDBCn.BeginTrans()
                If InsertIntoDelAudit(PubDBCn, "INV_MRR_UNLOCK_TRN", (txtMRRNo.Text), RsTransMain, "AUTO_KEY_MRR") = False Then GoTo DelErrPart

                mSqlStr = " COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND AUTO_KEY_MRR=" & Val(txtMRRNo.Text) & ""

                PubDBCn.Execute("Delete from INV_MRR_UNLOCK_TRN Where " & mSqlStr)
...
ErrorHandler:
        ...
        'Resume
    End Sub
    Private Sub frmMRRUnlock_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Me.Hide()
        Me.Dispose()
        Me.Close()
    End Sub
...
err_Renamed:
    ...
    Public Sub frmMRRUnlock_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ...
        SqlStr = ""
        SqlStr = "Select * from INV_MRR_UNLOCK_TRN Where 1<>1"
...
ERR1:
    ...
    Private Sub AssignGrid(ByRef mRefresh As Boolean)
        ...
        SqlStr = "SELECT  IH.AUTO_KEY_MRR, MRR.MRR_DATE, CMST.SUPP_CUST_NAME "

        ''FROM CLAUSE...

        SqlStr = SqlStr & vbCrLf & " FROM INV_MRR_UNLOCK_TRN IH, INV_GATE_HDR MRR, FIN_SUPP_CUST_MST CMST"

        ''WHERE CLAUSE...

        SqlStr = SqlStr & vbCrLf & " WHERE IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND IH.COMPANY_CODE=MRR.COMPANY_CODE AND IH.AUTO_KEY_MRR=MRR.AUTO_KEY_MRR" & vbCrLf & " AND MRR.COMPANY_CODE=CMST.COMPANY_CODE AND MRR.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE"

        ''ORDER BY CLAUSE...

        SqlStr = SqlStr & vbCrLf & " Order by AUTO_KEY_MRR"
    ...
    Private Sub Show1()
        ...
        With RsTransMain
            If Not .EOF Then
                ...
                txtMRRNo.Text = Trim(IIf(IsDbNull(.Fields("AUTO_KEY_MRR").Value), "", .Fields("AUTO_KEY_MRR").Value))

                SqlStr = " SELECT IH.SUPP_CUST_CODE, IH.MRR_DATE, CMST.SUPP_CUST_NAME " & vbCrLf & " FROM INV_GATE_HDR IH, FIN_SUPP_CUST_MST CMST" & vbCrLf & " WHERE IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE" & vbCrLf & " AND SUBSTR(AUTO_KEY_MRR,LENGTH(AUTO_KEY_MRR)-5,4)=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND AUTO_KEY_MRR=" & Val(txtMRRNo.Text) & " "
...
ERR1:
    ...
    Private Sub frmMRRUnlock_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmMRRUnlock_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
        Dim KeyCode As Short = eventArgs.KeyCode
        Dim Shift As Short = eventArgs.KeyData \ &H10000
        '    MainClass.DoFunctionKey Me, KeyCode
    End Sub

    Public Sub frmMRRUnlock_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    Private Sub TxtMRRNo_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtMRRNo.Validating
        ...
        SqlStr = " SELECT IH.SUPP_CUST_CODE, IH.MRR_DATE, CMST.SUPP_CUST_NAME " & vbCrLf & " FROM INV_GATE_HDR IH, FIN_SUPP_CUST_MST CMST" & vbCrLf & " WHERE IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE" & vbCrLf & " AND SUBSTR(AUTO_KEY_MRR,LENGTH(AUTO_KEY_MRR)-5,4)=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND AUTO_KEY_MRR=" & Val(mMRRNO) & " "
        ...
        SqlStr = " SELECT * " & vbCrLf & " FROM INV_MRR_UNLOCK_TRN" & vbCrLf & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND AUTO_KEY_MRR=" & Val(mMRRNO) & " "
        ...
        Else
            If ADDMode = False And MODIFYMode = False Then
                ...
                Cancel = True
            ElseIf MODIFYMode = True Then
                SqlStr = "SELECT * FROM INV_MRR_UNLOCK_TRN " & " WHERE AUTO_KEY_MRR=" & Val(xMkey) & " "
                MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTransMain, ADODB.LockTypeEnum.adLockReadOnly)
            End If
        End If
        GoTo EventExitSub
