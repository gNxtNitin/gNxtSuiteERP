...
Friend Class frmInspectionStnd
    ...
    Private Const ColDetailType As Short = 1
    Private Const ColParamDesc As Short = 2
    Private Const ColStdClass As Short = 3
    Private Const ColSpecification As Short = 4
    Private Const ColInspectionMth As Short = 5
    Private Const ColDataFrom As Short = 6
    Private Const ColControlMth As Short = 7
    ...
    Private Function Update1() As Boolean
        ...
        If ADDMode = True Then
            ...
            SqlStr = " INSERT INTO QAL_INSPECTION_STD_HDR " & vbCrLf _
                            ...
                            & " '" & MainClass.AllowSingleQuote(txtDocNo.Text) & "',TO_DATE('" & VB6.Format(txtDocDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                            & " '" & MainClass.AllowSingleQuote(txtRevNo.Text) & "',TO_DATE('" & VB6.Format(txtRevDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
        ...
        ElseIf MODIFYMode = True Then
            SqlStr = " UPDATE QAL_INSPECTION_STD_HDR SET " & vbCrLf _
                    ...
                    & " DOC_NO='" & MainClass.AllowSingleQuote(txtDocNo.Text) & "',DOC_DATE=TO_DATE('" & VB6.Format(txtDocDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
                    & " REV_NO='" & MainClass.AllowSingleQuote(txtRevNo.Text) & "',REV_DATE=TO_DATE('" & VB6.Format(txtRevDate.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'), " & vbCrLf _
        ...
        With SprdMain
            For i = 1 To .MaxRows
                ...
                If mDetailType <> "" And mParamDesc <> "" And mSpecification <> "" Then
                    SqlStr = " INSERT INTO  QAL_INSPECTION_STD_DET ( " & vbCrLf & " AUTO_KEY_STD,SERIAL_NO,DETAIL_TYPE,PARAM_DESC, " & vbCrLf & " STD_CLASS,SPECIFICATION,INSP_MTH,DATA_FORM,CONTROL_METHOD ) " & vbCrLf & " VALUES ( " & vbCrLf & " " & Val(lblMkey.Text) & "," & i & ",'" & mDetailType & "', " & vbCrLf & " '" & mParamDesc & "','" & mStdClass & "', " & vbCrLf & " '" & mSpecification & "','" & mInspectionMth & "','" & mDataFrom & "', " & vbCrLf & " '" & mControlMth & "') "
                    PubDBCn.Execute(SqlStr)
                End If
            Next
        End With
...
UpdateRevERR:
    ...
    Private Sub cmdSearchICode_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSearchICode.Click
        ...
        End If
    End Sub
    Private Sub cmdSearchICodeCopy_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSearchICodeCopy.Click
        Dim SqlStr As String
        '    SqlStr = "COMPANY_CODE=" & RsCompany.fields("COMPANY_CODE").value & ""
        '    If MainClass.SearchGridMaster("", "INV_ITEM_MST", "ITEM_SHORT_DESC", "ITEM_CODE", , , SqlStr) = True Then
        '        txtItemCodeCopy.Text = AcName1
        '        If txtItemCodeCopy.Enabled = True Then txtItemCodeCopy.SetFocus
        '    End If
        Call CopyInspectionStnd()
    End Sub
    ...
    End Sub
    Public Sub frmInspectionStnd_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
...
ERR1:
    ...
    Private Sub frmInspectionStnd_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmInspectionStnd_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    Private Sub FillCombo()
        ...
        '    cboStage.AddItem "Process Inspection"
    ...
    Private Sub Clear1()
        ...
        txtItemCodeCopy.Enabled = True
        cmdSearchICodeCopy.Enabled = True
...
ERR1:
    ...
    Private Sub SetTextLengths()
        ...
        txtItemCodeCopy.Maxlength = RsInspectionMain.Fields("ITEM_CODE").DefinedSize
...
err_Renamed:
    ...
    Private Sub frmInspectionStnd_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
...
EventExitSub:
    ...
    Private Sub txtItemCodeCopy_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtItemCodeCopy.TextChanged

        MainClass.SaveStatus(Me.cmdsave, ADDMode, MODIFYMode)
    End Sub

    Private Sub txtItemCodeCopy_DoubleClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtItemCodeCopy.DoubleClick
        Call cmdSearchICode_Click(cmdSearchICode, New System.EventArgs())
    End Sub

    Private Sub txtItemCodeCopy_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles txtItemCodeCopy.KeyPress
        Dim KeyAscii As Short = Asc(eventArgs.KeyChar)

        KeyAscii = MainClass.UpperCase(KeyAscii, txtItemCodeCopy.Text)
        eventArgs.KeyChar = Chr(KeyAscii)
        If KeyAscii = 0 Then
            eventArgs.Handled = True
        End If
    ...
    Private Sub txtItemCodeCopy_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles txtItemCodeCopy.KeyUp
        Dim KeyCode As Short = eventArgs.KeyCode
        Dim Shift As Short = eventArgs.KeyData \ &H10000
        If KeyCode = System.Windows.Forms.Keys.F1 Then Call cmdSearchICode_Click(cmdSearchICode, New System.EventArgs())
    End Sub
    Private Sub txtItemCodeCopy_Validating(ByVal eventSender As System.Object, ByVal eventArgs As System.ComponentModel.CancelEventArgs) Handles txtItemCodeCopy.Validating
        Dim Cancel As Boolean = eventArgs.Cancel
        On Error GoTo ValMacERR
        Dim SqlStr As String

        If Trim(txtItemCodeCopy.Text) = "" Then GoTo EventExitSub
        SqlStr = "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & ""
        If MainClass.ValidateWithMasterTable(txtItemCodeCopy.Text, "ITEM_CODE", "ITEM_SHORT_DESC", "INV_ITEM_MST", PubDBCn, MasterNo, , SqlStr) = False Then
            MsgBox("Item Code Does Not Exist In Master.")
            Cancel = True
        End If
...
EventExitSub:
    ...
    Private Sub Show1()
        ...
        txtItemCodeCopy.Enabled = False
        cmdSearchICodeCopy.Enabled = False
        MainClass.ButtonStatus(Me, XRIGHT, RsInspectionMain, ADDMode, MODIFYMode, CmdAdd, CmdModify, CmdClose, CmdSave, CmdDelete, cmdSavePrint, cmdSavePrint, cmdPrint, CmdPreview, cmdSavePrint, CmdView, True)
        Exit Sub
ShowErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
...
ERR1:
    ...
    Private Sub CopyInspectionStnd()

        On Error GoTo ERR1
        Dim i As Integer
        Dim SqlStr As String
        Dim RsCopyDetail As ADODB.Recordset
        Dim mStage As String
        ...
        SqlStr = " SELECT ID.* " & vbCrLf _
                    ...
                    & " WHERE IH.AUTO_KEY_STD=ID.AUTO_KEY_STD" & vbCrLf _
                    & " AND IH.COMPANY_CODE=" & RsCompany.fields("COMPANY_CODE").value & " " & vbCrLf _
                    & " AND IH.ITEM_CODE='" & MainClass.AllowSingleQuote(txtItemCodeCopy.Text) & "' " & vbCrLf _
                    & " AND IH.INSP_TYPE='" & mStage & "' " & vbCrLf _
                    & " ORDER BY ID.SERIAL_NO"

        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsCopyDetail, ADODB.LockTypeEnum.adLockReadOnly)
        With RsCopyDetail
            If .EOF = True Then Exit Sub
            FormatSprdMain(-1)
            i = 1
