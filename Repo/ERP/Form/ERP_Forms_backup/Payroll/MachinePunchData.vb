...
Friend Class frmPunchData
    ...
    Private Sub FillHeading()
        ...
        With sprdAttn
            ...
            .set_RowHeight(-1, ConRowHeight * 3)
...
ErrPart:
    ...
    Private Sub frmPunchData_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ' RefreshScreen	
    ...
    Private Sub frmPunchData_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    Private Sub frmPunchData_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
refreshErrPart:
    ...
    Private Function GetPunchTime(ByRef mEmpCode As String, ByRef mDate As String, ByRef mTable As String) As String
        ...
        mTableName = "vwTempdata"
        GetPunchTime = ""

        If RsCompany.Fields("ERP_CUSTOMER_ID").Value = 104 Then
            SqlStr = "SELECT LISTAGG(REFTIME, ', ') WITHIN GROUP (ORDER BY OFFICEPUNCH) AS OFFICEPUNCH  FROM vwTempdata WHERE TRIM(CARDNO)= TRIM(REPLACE(REPLACE(REPLACE('" & MainClass.AllowSingleQuote(mEmpCode) & "', 'G-', '7'), 'B-', '2'),'K-', '11')) AND REFDATE= '" & VB6.Format(mDate, "YYYYMMDD") & "' " & vbCrLf

        Else
            SqlStr = "SELECT LISTAGG(REFTIME, ', ') WITHIN GROUP (ORDER BY OFFICEPUNCH) AS OFFICEPUNCH FROM vwTempdata WHERE TRIM(CARDNO)= '" & MainClass.AllowSingleQuote(mEmpCode) & "' AND REFDATE= '" & VB6.Format(mDate, "YYYYMMDD") & "' " & vbCrLf
...
refreshErrPart:
    ...
    Private Function GetPunchIOTime(ByRef mEmpCode As String, ByRef mDate As String, ByRef mTable As String, ByRef mIO As String) As String
        ...
        If mIsRoundClock = "N" Then
            ...
            End If
            SqlStr = SqlStr & vbCrLf & " FROM " & mTable & " EMP, " & mTableName & " SMST " & vbCrLf & " WHERE EMP.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & ""
            ...
            ElseIf RsCompany.Fields("COMPANY_CODE").Value = 35 Or RsCompany.Fields("COMPANY_CODE").Value = 41 Then
                SqlStr = SqlStr & vbCrLf & " AND TRIM(EMP.EMP_CODE) = TRIM(TO_CHAR(SMST.CARDNO,'000000')) " & vbCrLf & " AND TO_CHAR(SMST.OFFICEPUNCH,'YYYYMMDD')= '" & VB6.Format(mDate, "YYYYMMDD") & "'"
            Else
        ...
        If mIO = "I" And mIsRoundClock = "N" Then
            '            SqlStr = SqlStr & vbCrLf & " HAVING MIN(OFFICEPUNCH) IS NOT NULL "	
        ElseIf mIO = "O" And mIsRoundClock = "N" Then
            SqlStr = SqlStr & vbCrLf & " HAVING MAX(TO_CHAR(OFFICEPUNCH,'DD-MON-YYYY HH24:MI'))> MIN(TO_CHAR(OFFICEPUNCH,'DD-MON-YYYY HH24:MI')) "
        End If
