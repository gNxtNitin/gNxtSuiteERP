...
Friend Class frmMonthlyPF
    ...
    Private Const ColWages1 As Short = 7
    Private Const ColBSalary As Short = 8
    Private Const ColEPF As Short = 9
    Private Const ColEmployer_8 As Short = 10
    Private Const ColEmployer_3 As Short = 11
    Private Const ColTot_Employer As Short = 12
    Private Const ColVPF As Short = 13
    Private Const ColNetTotal As Short = 14
    Private Const ColAC1 As Short = 15
    Private Const ColAC2 As Short = 16
    Private Const ColAC10 As Short = 17
    Private Const ColAC21 As Short = 18
    Private Const ColAC22 As Short = 19
    ...
    Private Function CreatePFTextFile() As Boolean
        ...
        With SprdView
            For CntRow = 1 To .MaxRows - 1
                ...
                '1
                .Col = ColPFNo
                mMemberID = Mid(Trim(.Text), Len(mCompanyPFEst) + 1)
                mMainString = CStr(Val(mMemberID))
...
ERR1:
    ...
    Private Sub frmMonthlyPF_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ' RefreshScreen
    End Sub

    Private Sub frmMonthlyPF_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
ErrPart:
    ...
    End Sub
    Private Sub frmMonthlyPF_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
ErrPart:
    ...
    Private Function MakeSQL() As String
        ...
        If CDate(lblRunDate.Text) >= CDate("01/11/2015") Then
            mAcct2Per = 0.85
        Else
            mAcct2Per = 1.1
        End If

        MakeSQL = " SELECT  EMP.EMP_CODE, EMP.EMP_NAME, EMP.EMP_FNAME," & vbCrLf & " EMP.EMP_DESG_CODE, "

        If OptFields(0).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf & " EMP.EMP_PF_ACNO, "
        Else
            MakeSQL = MakeSQL & vbCrLf & " EMP.UID_NO, "
        End If

        MakeSQL = MakeSQL & vbCrLf & " TO_CHAR(PFESI.WDAYS,'999.9'), " & vbCrLf & " PENSIONWAGES, PFESI.PFABLEAMT, ROUND(PFAMT,0),PENSIONFUND,PFESI.EPFAMT, " & vbCrLf & " TO_CHAR((NVl(PFESI.PENSIONFUND,0)+NVL(PFESI.EPFAMT,0))) AS A, " & vbCrLf & " TO_CHAR(ROUND(NVl(PFESI.VPFAMT,0),0)) AS B, " & vbCrLf & " TO_CHAR(ROUND(NVL(PFESI.PFAMT,0),0)+NVL(PFESI.EPFAMT,0)) AS C, ROUND(EPFAMT,0) AS AC1, ROUND(PFESI.PFABLEAMT * " & mAcct2Per & " * 0.01, 2) AC2," & vbCrLf & " ROUND(PENSIONFUND,0) AS AC10, ROUND(PENSIONWAGES * 0.5 * 0.01, 2) AS AC21, ROUND(PENSIONWAGES * 0.01 * 0.01, 2) AS AC22" & vbCrLf & " FROM PAY_EMPLOYEE_MST EMP, PAY_PFESI_TRN PFESI " & vbCrLf & " WHERE EMP.COMPANY_CODE =" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND EMP.EMP_STOP_SALARY='N' " & vbCrLf & " AND EMP.COMPANY_CODE=PFESI.COMPANY_CODE" & vbCrLf & " AND EMP.EMP_CODE=PFESI.EMP_CODE" & vbCrLf & " AND TO_CHAR(SAL_DATE,'MON-YYYY')='" & UCase(VB6.Format(lblRunDate.Text, "MMM-YYYY")) & "'"
        ...
        ElseIf OptPF.Checked = True Then
            If OptFields(0).Checked = True Then
                MakeSQL = MakeSQL & vbCrLf & " Order by EMP.EMP_PF_ACNO"
            Else
...
ERR1:
    ...
    Private Sub FormatSprd(ByRef mRow As Integer)
        ...
        With SprdView
            ...
            For cntCol = ColWages1 To ColAC22
                .Col = cntCol
        ...
        MainClass.ProtectCell(SprdView, 1, SprdView.MaxRows, ColCard, ColAC22)
...
ERR1:
    ...
    Private Sub DisplayTotals()
        ...
        With SprdView
            ...
            .Col = ColEmployer_8
            .Text = VB6.Format(mTot4, "0.00")
            .Font = VB6.FontChangeBold(.Font, True)

            .Col = ColEmployer_3
            .Text = VB6.Format(mTot5, "0.00")
            .Font = VB6.FontChangeBold(.Font, True)

            .Col = ColTot_Employer
            .Text = VB6.Format(mTot6, "0.00")
            .Font = VB6.FontChangeBold(.Font, True)
