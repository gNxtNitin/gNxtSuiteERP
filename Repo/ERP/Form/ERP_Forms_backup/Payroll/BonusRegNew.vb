...
Friend Class frmBonusRegNew
    ...
    Private Const ColDept As Short = 22
    Private Const ColBankIFSC As Short = 23
...
ERR1:
    ...
    Private Sub ReportForPrint(ByRef Mode As Crystal.DestinationConstants)
        ...
        If frmPrintOTReg.optCheckList.Checked = True Then
            mTitle = "Bonus -  Register" & IIf(chkDiv.CheckState = System.Windows.Forms.CheckState.Unchecked, " - " & cboDivision.Text, "") & IIf(chkCategory.CheckState = System.Windows.Forms.CheckState.Unchecked, " - " & cboCategory.Text, "")
...
ErrPart1:
    ...
    Private Function GetBonusPer(ByRef mCode As String, ByRef mSalDate As String) As Double
        ...
        SqlStr = " SELECT SALARYDEF.PERCENTAGE, SALARYDEF.AMOUNT " & vbCrLf & " FROM PAY_SALARYDEF_MST SALARYDEF, PAY_SALARYHEAD_MST ADD_DEDUCT" & vbCrLf & " WHERE " & vbCrLf & " SALARYDEF.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND ADD_DEDUCT.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND SALARYDEF.COMPANY_CODE=ADD_DEDUCT.COMPANY_CODE" & vbCrLf & " AND SALARYDEF.ADD_DEDUCTCODE=ADD_DEDUCT.CODE" & vbCrLf & " AND SALARYDEF.EMP_CODE = '" & mCode & "'" & vbCrLf & " AND ADD_DEDUCT.TYPE=" & ConBonus & "" & vbCrLf & " AND SALARYDEF.SALARY_EFF_DATE=( SELECT MAX(SALARY_EFF_DATE)" & vbCrLf & " FROM PAY_SALARYDEF_MST WHERE " & vbCrLf & " COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND EMP_CODE = '" & mCode & "'" & vbCrLf & " AND SALARY_EFF_DATE - ADDDAYS_IN <= TO_DATE('" & VB6.Format(xSalDate, "DD-MMM-YYYY") & "','DD-MON-YYYY')) " ''SALARY_APP_DATE
...
ErrPart:
    ...
    End Sub
    Private Sub frmBonusRegNew_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        'RefreshScreen
    End Sub
    Private Sub frmBonusRegNew_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
refreshErrPart:
    ...
    Private Sub RefreshScreen()
        ...
        Dim mTotPayableArrearBS As Double
        Dim mArrearBonusAmount As Double
        ...
        SqlStr = " Select EMP.EMP_CODE, EMP.EMP_NAME, GETEMPDESG ('" & RsCompany.Fields("COMPANY_CODE").Value & "',EMP.EMP_CODE,TO_DATE('" & VB6.Format(txtTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')) AS DESG_DESC, " & vbCrLf & " EMP.EMP_BANK_NO, DECODE(EMP.PAYMENTMODE,1,'CASH','CHEQUE') AS PAYMENTMODE, EMP.EMP_LEAVE_DATE,EMP_DOJ, DEPT.DEPT_DESC"


        SqlStr = SqlStr & vbCrLf & " From PAY_EMPLOYEE_MST EMP, PAY_DEPT_MST DEPT" & vbCrLf & " WHERE  EMP.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND  EMP.COMPANY_CODE=DEPT.COMPANY_CODE" & vbCrLf & " AND  EMP.EMP_DEPT_CODE=DEPT.DEPT_CODE"

        If optExisting.Checked = True Then
            SqlStr = SqlStr & vbCrLf & " AND (EMP_LEAVE_DATE>=TO_DATE('" & VB6.Format(txtAsOn.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') OR EMP_LEAVE_DATE IS NULL OR EMP_LEAVE_DATE='')"
        ElseIf optTransfer.Checked = True Then
            '        SqlStr = SqlStr & vbCrLf & " AND (EMP_LEAVE_DATE>='" & VB6.Format(txtFrom.Text, "DD-MMM-YYYY") & "' AND EMP_LEAVE_DATE<='" & VB6.Format(txtAsOn.Text, "DD-MMM-YYYY") & "')"
            SqlStr = SqlStr & vbCrLf & " AND (EMP_LEAVE_DATE>=TO_DATE('" & VB6.Format(txtFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'))"
            SqlStr = SqlStr & vbCrLf & " AND EMP_CODE IN (SELECT DISTINCT EMP_CODE FROM PAY_FFSETTLE_HDR WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " AND IS_TRANSFER='Y')"
        Else
            SqlStr = SqlStr & vbCrLf & " AND EMP_CODE IN ( " & vbCrLf & " SELECT DISTINCT EMP_CODE FROM PAY_SAL_TRN " & vbCrLf & " WHERE  EMP.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND SAL_DATE>=TO_DATE('" & VB6.Format(txtFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY') " & vbCrLf & " AND SAL_DATE<=TO_DATE('" & VB6.Format(txtTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY'))"
        End If

        SqlStr = SqlStr & vbCrLf & " AND EMP_DOJ<=TO_DATE('" & VB6.Format(txtTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"

        '    SqlStr = SqlStr & vbCrLf & " AND EMP.EMP_CODE IN ('001156','001141')"
        ...
        If RsCompany.Fields("BONUS_TYPE").Value = "C" Then
            SqlStr = SqlStr & vbCrLf & "AND EMP.IS_BONUS_PAYABLE='Y'"
        End If
        ...
        '    SqlStr = SqlStr & vbCrLf & " AND EMP.EMP_CODE='000055'"
        ...
        With sprdAddDeduct
            ...
            If RsTempReg.EOF = False Then
                Do While RsTempReg.EOF = False
                    ...
                    .Col = ColDept
                    .Text = IIf(IsDbNull(RsTempReg.Fields("DEPT_DESC").Value), "", RsTempReg.Fields("DEPT_DESC").Value)
                    ...
                    For CntMonth = ColBonusApr To ColBonusMar
                        ...
                        mPayableArrearBS = GetPayableBasic(mEmpCode, mFromDate, "V")
                        mPayableArrearBS = mPayableArrearBS + GetPayableBasic(mEmpCode, mFromDate, "Y")
                        mPayableArrearBS = mPayableArrearBS + GetPayableBasic(mEmpCode, mFromDate, "O")



                        If mPayableBS > 0 Then
                            If RsCompany.Fields("BONUS_TYPE").Value = "B" Then
                                mBonusAmount = ((GetBonusPer(mEmpCode, mFromDate) * mPayableBS * 0.01)) ''s.LastDay(Month(mFromDate), Year(mFromDate)))
                            Else
                                If RsCompany.Fields("COMPANY_CODE").Value = 16 And RsCompany.Fields("FYEAR").Value >= 2016 Then
                                    mBonusAmount = (GetBonusCeilingAmount(mEmpCode, mFromDate) * mMonthWDays / MainClass.LastDay(Month(CDate(mFromDate)), Year(CDate(mFromDate))))
                                    '                                mBonusAmount = Format(Round(mBonusAmount * IIf(IsNull(RsCompany!BonusLimit), 0, RsCompany!BonusLimit) * 0.01, 0), "0.00")
                                Else
                                    If CDate(mFromDate) >= CDate("01/04/2016") Then
                                        mBonusAmount = (GetBonusAmount(mEmpCode, mFromDate) * mMonthWDays / MainClass.LastDay(Month(CDate(mFromDate)), Year(CDate(mFromDate))))
                                    Else
                                        mBonusAmount = (GetBonusCeilingAmount(mEmpCode, mFromDate) * mMonthWDays / MainClass.LastDay(Month(CDate(mFromDate)), Year(CDate(mFromDate))))
                                        '                                    mBonusAmount = Format(Round(mBonusAmount * IIf(IsNull(RsCompany!BonusLimit), 0, RsCompany!BonusLimit) * 0.01, 0), "0.00")
                                    End If
                        ...
                        If RsCompany.Fields("BONUS_TYPE").Value = "B" Then
                            If mPayableArrearBS > 0 Then
                                mArrearBonusAmount = GetBonusPer(mEmpCode, mFromDate) * mPayableArrearBS * 0.01
                                mPayableArrearBS = IIf(mArrearBonusAmount = 0, mArrearBonusAmount, mPayableArrearBS)
                            Else
                                mArrearBonusAmount = 0
                            End If
                            mTotBonusAmount = mTotBonusAmount + mArrearBonusAmount
                            mTotPayableArrearBS = mTotPayableArrearBS + mPayableArrearBS
                        ...
                        Else
                            .Text = VB6.Format(mBonusAmount, "0.00")
                        End If
                        mFromDate = CStr(DateAdd(Microsoft.VisualBasic.DateInterval.Month, 1, CDate(mFromDate)))
                    Next

                    '               mTotBonusAmount = mTotBonusAmount + (mPayableArrearBS * 20 / 100)

                    .Col = ColBonusArrear
                    .Text = VB6.Format(mTotPayableArrearBS, "0.00")

                    .Col = ColTotBasic
                    If RsCompany.Fields("BONUS_TYPE").Value = "B" Then
                        .Text = VB6.Format(mTotPayableBS + mTotPayableArrearBS, "0.00")
                    Else
                        .Text = CStr(0) '' Format(mTotBonusAmount, "0.00")
                    End If
                    ...
                    Else
                        .Text = VB6.Format(System.Math.Round(mTotBonusAmount, 0), "0.00")
                        '                    .Text = Format(Round(mTotBonusAmount * IIf(IsNull(RsCompany!BonusLimit), 0, RsCompany!BonusLimit) * 0.01, 0), "0.00")
                    End If
                    I = I + 1
                    .MaxRows = I
                    RsTempReg.MoveNext()
...
refreshErrPart:
    ...
    Private Function GetPayableBasic(ByRef pEmpCode As String, ByRef mSalDate As String, ByRef mIsArrear As String) As Double
        ...
        GetPayableBasic = 0
        '    If mIsArrear = "V" Then
        '        SqlStr = " SELECT DISTINCT PAYABLESALARY,SAL_TYPE " & vbCrLf _
        ''                & " FROM PAY_SALVOUCHER_TRN" & vbCrLf _
        ''                & " WHERE COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf _
        ''                & " AND EMP_CODE='" & MainClass.AllowSingleQuote(pEmpCode) & "'" & vbCrLf _
        ''                & " AND TO_CHAR(SAL_DATE,'YYYYMM')='" & VB6.Format(mSalDate, "YYYYMM") & "'" & vbCrLf _
        ''                & " AND SAL_TYPE IN ('S','B')"
        '        MainClass.UOpenRecordSet SqlStr, PubDBCn, adOpenStatic, RsTemp, adLockReadOnly
        '
        '        If RsTemp.EOF = False Then
        '            Do While RsTemp.EOF = False
        '                GetPayableBasic = GetPayableBasic + IIf(IsNull(RsTemp!PAYABLESALARY), 0, RsTemp!PAYABLESALARY)
        ...
        If RsTemp.EOF = False Then
            If mIsArrear = "V" Then
                If GetBonusPayableVoucherPayment(RsCompany.Fields("COMPANY_CODE").Value, pEmpCode, VB6.Format(mSalDate, "DD/MMM/YYYY")) = True Then
...
ERR1:
    ...
    Private Sub frmBonusRegNew_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
