...
Friend Class frmFADespRegisterNew
...
ErrPart:
        ...
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
    End Sub
    Private Sub frmFADespRegisterNew_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        If FormActive = True Then Exit Sub

        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor

        Me.Text = "Fixed Assets Register (New)"
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        FormActive = True
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmFADespRegisterNew_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    End Sub
    Private Sub frmFADespRegisterNew_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmFADespRegisterNew_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Dispose()
        Me.Close()
    ...
    Private Function UpdateTempTable() As Boolean
        ...
        Do While RsTemp.EOF = False
            ...
            If CDate(mPurchaseDate) < CDate(mFYStartDate) Then
                If CalcSaleAmount(mRefNo, mCompanyCode, (lblFYStartDate.Text), (lblFYEndDate.Text), mSaleAmount1, mSaleAmount2, mSaleAmount3, mSaleAmount4, mSaleDate1, mSaleDate2, mSaleDate3, mSaleDate4, "O") = False Then GoTo LedgError
                If CalcDepreciationAmount(mRefNo, mCompanyCode, mPurchaseDate, pPurchaseYear, pTRNType, pPurchaseAmount, mDepreMode, pCurrentYearDep, pSaleAmount, pSaleDesp, pSaleDate, mCummDepAmount, mCalcDepAsOn, pOPCummDesp, "O", IIf(optShowType(0).Checked = True, "O", "N")) = False Then GoTo LedgError
            End If
            ...
            ''For the Year						
            If CalcSaleAmount(mRefNo, mCompanyCode, (lblFYStartDate.Text), (lblFYEndDate.Text), mSaleAmount1, mSaleAmount2, mSaleAmount3, mSaleAmount4, mSaleDate1, mSaleDate2, mSaleDate3, mSaleDate4, "") = False Then GoTo LedgError
            If CalcDepreciationAmount(mRefNo, mCompanyCode, mPurchaseDate, pPurchaseYear, pTRNType, pPurchaseAmount, mDepreMode, pCurrentYearDep, pSaleAmount, pSaleDesp, pSaleDate, mCummDepAmount, mCalcDepAsOn, pOPCummDesp, "", IIf(optShowType(0).Checked = True, "O", "N")) = False Then GoTo LedgError
            ...
            mDeprRate = GetDepreciationRate(mCompanyCode, pTRNType, mDepreMode, IIf(optShowType(0).Checked = True, "O", "N"))
...
LedgError:
    ...
    Private Function InsertDataInTempFile() As Boolean
        ...
        If optOption(1).Checked = True Then
            '        SqlStr = SqlStr & vbCrLf & "AND TOTAL_COST+CD_AMOUNT+OTH_AMOUNT-(MODVAT_AMOUNT+CESS_AMOUNT+SHEC_AMOUNT+AED_AMOUNT+SALETAX_REFUND)>ORIGINAL_COST+ORIGINAL_COST1+ORIGINAL_COST2+ORIGINAL_COST3"						
        ...
        If optDate(1).Checked = True Then
            ...
            ''                    & " AND PV_DATE <= TO_DATE('" & vb6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "')"						
            '        Else						
        ...
        Else
            '        If chkOpening.Value = vbChecked Then						
            '            SqlStr = SqlStr & vbCrLf _						
            ''                    & " AND PUT_DATE <= TO_DATE('" & vb6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "')"						
            '        Else						
            SqlStr = SqlStr & vbCrLf & " AND PUT_DATE BETWEEN TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "') " & vbCrLf & " AND TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "')"
            '        End If						
        End If
        ...
        If InsertOPDataInTempFile() = False Then GoTo LedgError
...
LedgError:
    ...
    End Function
    Private Function InsertOPDataInTempFile() As Boolean
        On Error GoTo LedgError
        Dim SqlStr As String
        Dim mGroupCode As String
        Dim mTrnCode As Integer
        ...
        SqlStr = " SELECT  '" & MainClass.AllowSingleQuote(PubUserID) & "', " & vbCrLf & " COMPANY_CODE, COMPANY_NAME, " & vbCrLf & " FYEAR, BOOKTYPE, BOOKSUBTYPE, " & vbCrLf & " TRNCODE, TRNNAME, SUPP_CUST_NAME, ITEM_DESC, " & vbCrLf & " AUTO_KEY_ASSET, LOCATION, PV_DATE, PUT_DATE," & vbCrLf & " TOTAL_COST, DAYS, PUR_YEAR, " & vbCrLf & " CURRENT_DESP, CUMULATIVE_DESP, SALE_AMOUNT, " & vbCrLf & " SALE_DATE, SALE_DESP, PHY_DATE, " & vbCrLf & " PHY_WHOM, GROSS_BLOCK, NET_BLOCK, " & vbCrLf & " ITEM_TYPE, ASSET_TYPE,OP_GROSS_BLOCK "

        SqlStr = SqlStr & vbCrLf & " FROM AST_OP_TRN2007 TRN"
        ...
        If chkAll.CheckState = System.Windows.Forms.CheckState.Unchecked Then
            ...
            If MainClass.ValidateWithMasterTable(mAccountCode, "ACCOUNTPOSTCODE", "CODE", "FIN_INVTYPE_MST", PubDBCn, MasterNo,  , "COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "") = True Then
                mGroupCode = IIf(IsDBNull(MasterNo), "", MasterNo)
            End If
            SqlStr = SqlStr & vbCrLf & "AND TRNCODE='" & MainClass.AllowSingleQuote(mGroupCode) & "'"
        End If
        ...
        InsertOPDataInTempFile = True
        Exit Function
LedgError:
        '    Resume						
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
        InsertOPDataInTempFile = False
...
LedgError:
    ...
    Private Function CalcSaleAmount(ByRef pRefNo As Double, ByRef pCompanyCode As Integer, ByRef mStartDate As String, ByRef mEndDate As String, ByRef mSaleAmount1 As Double, ByRef mSaleAmount2 As Double, ByRef mSaleAmount3 As Double, ByRef mSaleAmount4 As Double, ByRef mSaleDate1 As String, ByRef mSaleDate2 As String, ByRef mSaleDate3 As String, ByRef mSaleDate4 As String, ByRef pIsOpening As String) As Boolean
        On Error GoTo LedgError
        ...
        Else
            SqlStr = SqlStr & vbCrLf & " AND SALE_BILL_DATE >='" & VB6.Format(mStartDate, "DD-MMM-YYYY") & "' AND SALE_BILL_DATE <='" & VB6.Format(mEndDate, "DD-MMM-YYYY") & "'"
        End If
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        ...
        Else
            SqlStr = SqlStr & vbCrLf & " AND SALE_BILL_DATE1 >='" & VB6.Format(mStartDate, "DD-MMM-YYYY") & "' AND SALE_BILL_DATE1 <='" & VB6.Format(mEndDate, "DD-MMM-YYYY") & "'"
        End If
        ...
        Else
            SqlStr = SqlStr & vbCrLf & " AND SALE_BILL_DATE2 >='" & VB6.Format(mStartDate, "DD-MMM-YYYY") & "' AND SALE_BILL_DATE2 <='" & VB6.Format(mEndDate, "DD-MMM-YYYY") & "'"
        End If
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)
        ...
        Else
            SqlStr = SqlStr & vbCrLf & " AND SALE_BILL_DATE3 >='" & VB6.Format(mStartDate, "DD-MMM-YYYY") & "' AND SALE_BILL_DATE3 <='" & VB6.Format(mEndDate, "DD-MMM-YYYY") & "'"
        End If
...
LedgError:
    ...
    Private Function GetDepreciationRate(ByRef pCompanyCode As Integer, ByRef pTRNType As Double, ByRef pModCode As String, ByRef pDespType As String) As Double
        On Error GoTo LedgError
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        If optShow(0).Checked = True And chkOpening.CheckState = System.Windows.Forms.CheckState.Checked Then
            SqlStr = " SELECT  '', '', '', '1', COMPANY_NAME, " & vbCrLf & " TRNNAME, 'OPENING', '', 0, '', " & vbCrLf & " TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "'),TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "'), SUM(OP_GROSS_BLOCK), 0, '', SUM(CURRENT_DESP), SUM(CUMULATIVE_DESP), " & vbCrLf & " SUM(SALE_AMOUNT), '', SUM(SALE_DESP), TO_DATE(''), '', " & vbCrLf & " SUM(GROSS_BLOCK), SUM(NET_BLOCK), '', '' "
        ...
        If optShow(0).Checked = True Then
            SqlStr = SqlStr & vbCrLf & " SELECT  '', FYEAR, BOOKTYPE, BOOKSUBTYPE, COMPANY_NAME, " & vbCrLf & " TRNNAME, SUPP_CUST_NAME, ITEM_DESC, AUTO_KEY_ASSET, LOCATION, " & vbCrLf & " PV_DATE, PUT_DATE, OP_GROSS_BLOCK, DAYS, PUR_YEAR, CURRENT_DESP, CUMULATIVE_DESP, " & vbCrLf & " SALE_AMOUNT, SALE_DATE, SALE_DESP, PHY_DATE, PHY_WHOM, " & vbCrLf & " GROSS_BLOCK, NET_BLOCK, ITEM_TYPE, ASSET_TYPE "
        Else
            SqlStr = " SELECT  '', '', '', '1', '', " & vbCrLf & " TRNNAME, '', '', '', '', " & vbCrLf & " '', '',SUM(OP_GROSS_BLOCK), 0, '', SUM(CURRENT_DESP), SUM(CUMULATIVE_DESP), " & vbCrLf & " SUM(SALE_AMOUNT), '', SUM(SALE_DESP), '', '', " & vbCrLf & " SUM(GROSS_BLOCK), SUM(NET_BLOCK), '', '' "
        End If
