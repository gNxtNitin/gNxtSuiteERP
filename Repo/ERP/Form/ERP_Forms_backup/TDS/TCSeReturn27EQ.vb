...
Friend Class frmTCSeReturn27EQ
    ...
    Private Sub ReportForTDS(ByRef Mode As Crystal.DestinationConstants)
        ...
        If frmPrintTDS.OptForm26.Checked = True Then
            ...
            mReportFileName = "TDSeReturn27EQ.rpt"
            SqlStr = ""
        ...
        ElseIf frmPrintTDS.OptAnnexure.Checked = True Then
            ...
            mReportFileName = "TDSeReturn27EQAnnx.rpt"
            SqlStr = ""
            SqlStr = FetchRecordForReport(SqlStr, 1)
...
ERR1:
    ...
    Private Sub InsertIntoPrintDummy()
        ...
        If frmPrintTDS.OptForm26.Checked = True Then
            If InsertGridDetail(SprdView27EQ, 1, (SprdView27EQ.MaxRows), (SprdView27EQ.MaxCols)) = False Then GoTo ERR1
        ElseIf frmPrintTDS.OptAnnexure.Checked = True Then
...
ERR1:
        FieldsVerification = False
    End Function
    Public Sub frmTCSeReturn27EQ_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
...
ERR1:
        MsgInformation(Err.Description)
    End Sub
    Private Sub frmTCSeReturn27EQ_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    Private Sub Show1()
        ...
        If ShowDetail27EQ() = False Then GoTo ErrPart
...
ErrPart:
    ...
    Private Function ShowDetailAnnex() As Boolean
        ...
        '    SqlStr = " Select COUNT(1) AS TOTDEDUCTEE, " & vbCrLf _						
        ''            & " SUM(NETVALUE) AS SALE_AMOUNT, " & vbCrLf _						
        ''            & " SUM(TCSAMOUNT) AS NETAMOUNT, SUM(NETTAXAMOUNT) AS DEPOSITAMOUNT,"						
        '						
        '						
        '    SqlStr = SqlStr & vbCrLf _						
        ''            & " SUM(NETTAXAMOUNT*100*.100/112.2) AS SURAMT, " & vbCrLf _						
        ''            & " SUM(NETTAXAMOUNT*100*.022/112.2) AS CESS,  " & vbCrLf _						
        ''            & " SUM(NETTAXAMOUNT - ((NETTAXAMOUNT*100*.022/112.2)+ (NETTAXAMOUNT*100*.100/112.2))) AS TCSAMT" & vbCrLf _						
        ''						
        '						
        '						

        SqlStr = " SELECT " & vbCrLf & " TRN.COLLECTIONCODE,  CMST.PAN_NO, " & vbCrLf _
            & " CMST.SUPP_CUST_NAME, " & vbCrLf & " CMST.SUPP_CUST_ADDR, CMST.SUPP_CUST_CITY, " & vbCrLf _
            & " CMST.SUPP_CUST_STATE, CMST.SUPP_CUST_PIN, CMST.CType," & vbCrLf _
            & " IH.NETVALUE-IH.TCSAMOUNT AS NETVALUE, IH.INVOICE_DATE, " & vbCrLf _
            & " IH.TCSPER, IH.TCSAMOUNT, IH.NETTAXAMOUNT, IH.INVOICE_DATE, " & vbCrLf _
            & " TRN.BANKCODE, TRN.CHALLANDATE, TRN.CHALLANNO, " & vbCrLf _
            & " 0 AS SURAMT, " & vbCrLf _
            & " 0 AS CESS,  " & vbCrLf _
            & " IH.NETTAXAMOUNT AS TCSAMT," & vbCrLf _
            & " '' AS CDate, IH.COMPANY_CODE, TRN.MKEY CHALLANMKEY, TRN.REFDATE, TRN.REFDATE"
        ...
        SqlStr = SqlStr & vbCrLf & " IH.COMPANY_CODE=TRN.COMPANY_CODE" & vbCrLf _
            & " AND IH.FYEAR=TRN.FYEAR" & vbCrLf _
            & " AND IH.TCSCHALLANMKEY =TRN.MKEY" & vbCrLf _
            & " AND IH.COMPANY_CODE=CMST.COMPANY_CODE" & vbCrLf _
            & " AND IH.SUPP_CUST_CODE=CMST.SUPP_CUST_CODE" & vbCrLf _
            & " AND IH.ISTCSPAID= 'Y'" & vbCrLf _
            & " AND IH.CANCELLED='N'" & vbCrLf _
            & " AND IH.CHALLANDATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf _
            & " AND IH.CHALLANDATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        SqlStr = SqlStr & vbCrLf & " ORDER BY  TRN.COMPANY_CODE,TRN.REFDATE, TRN.MKEY, CMST.SUPP_CUST_NAME, TRN.REFDATE"
        ...
        With SprdViewAnnex
            If RsTemp.EOF = False Then
                Do While Not RsTemp.EOF
                    ...
                    .Col = 6
                    '                mAmountPaid = IIf(IsNull(RsTemp!NETVALUE), "", RsTemp!NETVALUE)						
                    '                mAmountPaid = mAmountPaid - IIf(IsNull(RsTemp!NETTAXAMOUNT), "", RsTemp!NETTAXAMOUNT)						

                    .Text = VB6.Format(IIf(IsDBNull(RsTemp.Fields("NETVALUE").Value), "", RsTemp.Fields("NETVALUE").Value), "0.00") ''  '' Format(IIf(IsNull(RsTemp!TDSAMOUNT), "", RsTemp!TDSAMOUNT), "0.00")						
                    ...
                    .Col = 8
                    .Text = CStr(System.Math.Round(CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("NETTAXAMOUNT").Value), "", RsTemp.Fields("NETTAXAMOUNT").Value), "0.00")), CInt("0")) - System.Math.Round(CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("SURAMT").Value), "", RsTemp.Fields("SURAMT").Value), "0.00")), CInt("0")) - System.Math.Round(CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("CESS").Value), "", RsTemp.Fields("CESS").Value), "0.00")), CInt("0")))

                    .Col = 9
                    .Text = CStr(System.Math.Round(CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("SURAMT").Value), "", RsTemp.Fields("SURAMT").Value), "0.00")), CInt("0")))

                    .Col = 10
                    .Text = CStr(System.Math.Round(CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("CESS").Value), "", RsTemp.Fields("CESS").Value), "0.00")), CInt("0")))

                    mNetAmount = IIf(IsDBNull(RsTemp.Fields("NETTAXAMOUNT").Value), 0, RsTemp.Fields("NETTAXAMOUNT").Value)

                    .Col = 11
                    .Text = VB6.Format(mNetAmount, "0.00")

                    .Col = 12
                    .Text = VB6.Format(IIf(IsDBNull(RsTemp.Fields("NETTAXAMOUNT").Value), "", RsTemp.Fields("NETTAXAMOUNT").Value), "0.00")

                    .Col = 13
                    .Text = VB6.Format(IIf(IsDBNull(RsTemp.Fields("INVOICE_DATE").Value), "", RsTemp.Fields("INVOICE_DATE").Value), "DD/MM/YYYY")

                    .Col = 14
                    .Text = VB6.Format(IIf(IsDBNull(RsTemp.Fields("TCSPER").Value), "", RsTemp.Fields("TCSPER").Value), "0.0000")
...
ErrPart1:
    ...
    Private Function ShowDetail27EQ() As Boolean
        ...
        SqlStr = "Select MKEY,COLLECTIONCODE,  TCS_AMOUNT, SURCHARGE, EDU_CESS, INTEREST_AMOUNT," & vbCrLf _
            & " OTHER_AMOUNT, PAIDAMOUNT, " & vbCrLf _
            & " BANKCODE, CHQ_NO, CHQ_DATE," & vbCrLf _
            & " CHALLANNO , CHALLANDATE, COMPANY_CODE,REFDATE,REFNO "

        SqlStr = SqlStr & vbCrLf & " FROM TCS_CHALLAN TRN " & vbCrLf & " WHERE "

        SqlStr = SqlStr & vbCrLf & " REFDATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf _
            & " AND REFDATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        SqlStr = SqlStr & vbCrLf & " ORDER BY TRN.COMPANY_CODE, REFDATE,REFNO, CHALLANDATE, CHALLANNO "
        ...
        With SprdView27EQ
            If RsTemp.EOF = False Then
                Do While Not RsTemp.EOF
                    ...
                    .Col = 2
                    .Text = IIf(IsDBNull(RsTemp.Fields("COLLECTIONCODE").Value), "", RsTemp.Fields("COLLECTIONCODE").Value) ''IIf(IsNull(RsTemp!Name), "", RsTemp!Name)						
...
ERR1:
    ...
    Private Sub FormatSprdView()
        Call FormatSprdView27EQ()
        Call FormatSprdViewAnnex()
    End Sub



    Private Sub FormatSprdView27EQ()
        ...
        With SprdView27EQ
            ...
            FillHeadingSprdView27EQ()
            MainClass.SetSpreadColor(SprdView27EQ, -1)
            MainClass.ProtectCell(SprdView27EQ, 1, .MaxRows, 1, .MaxCols)
    ...
    Private Sub FillHeadingSprdView27EQ()
        ...
        End With
    End Sub
    Private Sub frmTCSeReturn27EQ_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
    End Sub
    ...
    Private Sub Clear1()
        ...
        MainClass.ClearGrid(SprdView27EQ, RowHeight)
...
ERR1:
    ...
    Private Function ShowDosReport(ByRef pPrintMode As String, ByRef mQTR As String) As Boolean
        ...
        pFileName = mPubTDSPath & "\eRtn27EQ.txt"
    ...
    Private Function PrintCD(ByRef mLineCount As Integer) As Boolean
        ...
        With SprdView27EQ
            For cntRow = 1 To .MaxRows
                ...
                '29						
                .Col = 8
                mString = VB6.Format(mDepositAmt, "0.00") ''Format(Val(.Text), "0.00")						
                mMainString = mMainString & mString
                mMainString = mMainString & mDelimited


                '            .Col = 12						
                '                mString = Format(.Text, "0.00")						
...
ErrPart:
    ...
    Private Function GetChallanDetail(ByRef pTotChallanNo As Double, ByRef pTotDeductee As Double, ByRef pChallanAmount As Double, ByRef pDeducteeAmount As Double) As Boolean
        ...
        SqlStr = "Select COUNT(CHALLANNO) TOTCHALLANNO, SUM(PAIDAMOUNT) AS TDSAMOUNT "

        SqlStr = SqlStr & vbCrLf & " FROM TCS_CHALLAN TRN " & vbCrLf & " WHERE "

        SqlStr = SqlStr & vbCrLf & " TRN.REFDATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf _
            & " AND TRN.REFDATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        SqlStr = " Select COUNT(1) AS TOTDEDUCTEE, " & vbCrLf & " SUM(NETTAXAMOUNT) TOTTDSAMOUNT "

        SqlStr = SqlStr & vbCrLf & " FROM TCS_TRN IH, TCS_CHALLAN TRN "

        SqlStr = SqlStr & vbCrLf & " WHERE " & vbCrLf & " IH.COMPANY_CODE=TRN.COMPANY_CODE" & vbCrLf & " AND IH.FYEAR=TRN.FYEAR" & vbCrLf & " AND IH.TCSCHALLANMKEY=TRN.MKEY" & vbCrLf & " AND IH.CANCELLED='N'" & vbCrLf _
            & " AND TRN.REFDATE>=TO_DATE('" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')" & vbCrLf _
            & " AND TRN.REFDATE<=TO_DATE('" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "','DD-MON-YYYY')"
        ...
        If RsTemp.EOF = False Then
            ...
            pDeducteeAmount = System.Math.Round(pDeducteeAmount, 0)
        End If
...
ErrPart1:
    ...
    End Function
    Private Function GetChallan_DedDetail(ByRef pDepositAmt As Double, ByRef pTCSAmount As Double, ByRef pSurchargeAmt As Double, ByRef pCessAmt As Double, ByRef pNetAmount As Double, ByRef pIntAmt As Double, ByRef pOthAmt As Double, ByRef pTotDeductee As Double, ByRef pCompany_Code As Integer, ByRef pMkey As String) As Boolean
        ...
        SqlStr = " Select COUNT(1) AS TOTDEDUCTEE, " & vbCrLf & " SUM(NETVALUE) AS SALE_AMOUNT, " & vbCrLf & " SUM(TCSAMOUNT) AS NETAMOUNT, SUM(NETTAXAMOUNT) AS DEPOSITAMOUNT,"


        If RsCompany.Fields("FYEAR").Value <= 2014 Then
            SqlStr = SqlStr & vbCrLf & " SUM(NETTAXAMOUNT*100*.100/112.2) AS SURAMT, " & vbCrLf & " SUM(NETTAXAMOUNT*100*.022/112.2) AS CESS,  " & vbCrLf & " SUM(NETTAXAMOUNT - ((NETTAXAMOUNT*100*.022/112.2)+ (NETTAXAMOUNT*100*.100/112.2))) AS TCSAMT"
        Else
            SqlStr = SqlStr & vbCrLf & " 0 AS SURAMT, " & vbCrLf & " 0 AS CESS,  " & vbCrLf & " SUM(NETTAXAMOUNT) AS TCSAMT"
        End If

        SqlStr = SqlStr & vbCrLf & " FROM TCS_TRN "

        SqlStr = SqlStr & vbCrLf & " WHERE " & vbCrLf & " TCSCHALLANMKEY='" & pMkey & "' " & vbCrLf & " AND CANCELLED='N'"

        SqlStr = SqlStr & vbCrLf & " AND COMPANY_CODE=" & pCompany_Code & ""

        SqlStr = SqlStr & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf
        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsTemp, ADODB.LockTypeEnum.adLockReadOnly)

        If RsTemp.EOF = False Then
            pTotDeductee = IIf(IsDBNull(RsTemp.Fields("TOTDEDUCTEE").Value), 0, RsTemp.Fields("TOTDEDUCTEE").Value)
            pTCSAmount = CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("TCSAMT").Value), 0, RsTemp.Fields("TCSAMT").Value), "0.00"))
            pSurchargeAmt = CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("SURAMT").Value), 0, RsTemp.Fields("SURAMT").Value), "0.00"))
            pSurchargeAmt = System.Math.Round(pSurchargeAmt, 0)
            pCessAmt = CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("CESS").Value), 0, RsTemp.Fields("CESS").Value), "0.00"))
            pCessAmt = System.Math.Round(pCessAmt, 0)
            pTCSAmount = CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("DEPOSITAMOUNT").Value), 0, RsTemp.Fields("DEPOSITAMOUNT").Value), "0.00")) - pSurchargeAmt - pCessAmt
            pNetAmount = pTCSAmount + pSurchargeAmt + pCessAmt
            pDepositAmt = CDbl(VB6.Format(IIf(IsDBNull(RsTemp.Fields("DEPOSITAMOUNT").Value), 0, RsTemp.Fields("DEPOSITAMOUNT").Value), "0.00"))
            pDepositAmt = System.Math.Round(pDepositAmt, 0)

            '        pIntAmt = IIf(IsNull(RsTemp!TOTINTEREST_AMOUNT), 0, RsTemp!TOTINTEREST_AMOUNT)						
            '        pOthAmt = IIf(IsNull(RsTemp!TOTOTHER_AMOUNT), 0, RsTemp!TOTOTHER_AMOUNT)						
        End If
