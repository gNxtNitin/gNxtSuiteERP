...
Friend Class frmTCSCertProcess
...
ERR1: 
	...
	Private Sub CmdUnProcess_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdUnProcess.Click
		...
		If MsgQuestion("Want to unprocess All Certificate No As Given Period?") = CStr(MsgBoxResult.No) Then
			Exit Sub
		End If
		...
		SqlStr = " UPDATE TCS_TRN SET CERTIFICATENO='', " & vbCrLf & " Place='' , " & vbCrLf & " PrintDate='', " & vbCrLf & " Authorized='' , " & vbCrLf & " Authorized_FName='' , " & vbCrLf & " Authorized_Desig='', UPDATE_FROM='" & PubRun_IN & "', " & vbCrLf & " ASSESSMENT_YEAR='',  " & vbCrLf & " PERIOD_FROM='',  " & vbCrLf & " PERIOD_TO='',  " & vbCrLf & " QTR_NAME='',  " & vbCrLf & " MODUSER='" & MainClass.AllowSingleQuote(PubUserID) & "', " & vbCrLf & " MODDATE=TO_DATE('" & VB6.Format(PubCurrDate, "DD-MMM-YYYY") & "') " & vbCrLf & " WHERE " & vbCrLf & " COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & ""
		
		If Trim(txtDateFrom.Text) <> "" Then
			SqlStr = SqlStr & vbCrLf & " AND INVOICE_DATE>='" & VB6.Format(txtDateFrom.Text, "DD-MMM-YYYY") & "'"
		End If
		
		If Trim(txtDateTo.Text) <> "" Then
			SqlStr = SqlStr & vbCrLf & " AND INVOICE_DATE<='" & VB6.Format(txtDateTo.Text, "DD-MMM-YYYY") & "'"
		End If
		...
		If RsCompany.Fields("FYEAR").Value >= 2010 Then
			MsgInformation(mQTR & " All Certificates are Unprocess For Given Process.")
		Else
			MsgInformation("All Certificates are Unprocess For Given Process.")
		End If
...
ErrPart: 
	...
	'UPGRADE_WARNING: Form event frmTCSCertProcess.Activate has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
	Private Sub frmTCSCertProcess_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
		Me.Text = "TCS CERTIFICATE PROCESS"
	End Sub
	'UPGRADE_NOTE: MainClass was upgraded to MainClass_Renamed. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A9E4979A-37FA-4718-9994-97DD76ED70A7"'
	Private Sub frmTCSCertProcess_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		Dim MainClass_Renamed As Object
		On Error GoTo ERR1
		'UPGRADE_WARNING: Screen property Screen.MousePointer has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
		...
		txtDateTo.Text = MainClass.LastDay(Month(RunDate), Year(RunDate)) & "/" & VB6.Format(RunDate, "MM/YYYY")
...
err_Renamed: 
	...
	Private Function MakeSQL() As String
		...
		pSqlStr = " SELECT TCSTRN.MKEY, " & vbCrLf & " TCSTRN.INVOICE_DATE, FIN_SUPP_CUST_MST.SUPP_CUST_NAME, TCSTRN.NETVALUE, TCSTRN.TCSPER, " & vbCrLf & " TCSTRN.TCSAMOUNT, TCSCHALLAN.CHALLANDATE, TCSCHALLAN.CHALLANNO, " & vbCrLf & " TCSCHALLAN.BANKNAME, PAN_NO, " & vbCrLf & " TCSCHALLAN.REFDATE, " & vbCrLf & " Place, PrintDate, Authorized, Authorized_FNAME, Authorized_Desig, " & vbCrLf & " ASSESSMENT_YEAR, PERIOD_FROM, PERIOD_TO, QTR_NAME," & vbCrLf & " FIN_SUPP_CUST_MST.SUPP_CUST_ADDR "
		
		pSqlStr = pSqlStr & vbCrLf & " FROM  TCS_TRN TCSTRN ,  TCS_CHALLAN TCSCHALLAN, FIN_SUPP_CUST_MST " & vbCrLf & " WHERE TCSTRN.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" & vbCrLf & " AND TCSTRN.FYEAR=" & RsCompany.Fields("FYEAR").Value & "" & vbCrLf & " AND TCSTRN.COMPANY_CODE = TCSCHALLAN.COMPANY_CODE " & vbCrLf & " AND TCSTRN.FYEAR = TCSCHALLAN.FYEAR " & vbCrLf & " AND TCSTRN.TCSCHALLANMKEY = TCSCHALLAN.MKEY " & vbCrLf & " AND TCSTRN.COMPANY_CODE=FIN_SUPP_CUST_MST.COMPANY_CODE " & vbCrLf & " AND TCSTRN.SUPP_CUST_CODE=FIN_SUPP_CUST_MST.SUPP_CUST_CODE "
		...
		pSqlStr = pSqlStr & vbCrLf & " ORDER BY FIN_SUPP_CUST_MST.SUPP_CUST_NAME,TCSTRN.INVOICE_DATE"
	...
	Private Function UpdateCertificateNo() As Boolean
		...
		If RsTemp.EOF = False Then
			...
			If OptNew(0).Checked = True Then 'if Generate New Only
				If RsCompany.Fields("FYEAR").Value < 2010 Then
					mCretificateNo = GetMaxCretificateNo("")
				Else
					mCretificateNo = GetMaxCretificateNo(mQTR)
				End If
			End If
			Do While Not RsTemp.EOF
				If mPartyName <> RsTemp.Fields("SUPP_CUST_NAME").Value Then
					If OptNew(0).Checked = True Then 'if Generate New Only
						mCretificateNo = mCretificateNo + 1
						If RsCompany.Fields("FYEAR").Value < 2010 Then
							mCretificateNoStr = VB6.Format(mCretificateNo, "000000")
						Else
							mCretificateNoStr = VB6.Format(RsCompany.Fields("FYEAR").Value, "00") & "-" & VB6.Format(RsCompany.Fields("FYEAR").Value + 1, "00") & "/" & mQTR & "/" & VB6.Format(mCretificateNo, "000000")
						End If
					ElseIf OptNew(1).Checked = True Then  'if Append to latest one
						If RsCompany.Fields("FYEAR").Value < 2010 Then
							mCretificateNoStr = GetMaxPartyCretificateNo(RsTemp.Fields("PARTYNAME").Value, "")
						Else
							mCretificateNoStr = GetMaxPartyCretificateNo(RsTemp.Fields("PARTYNAME").Value, mQTR)
						End If
					End If
				End If
...
ErrPart: 
	...
	End Function
	'UPGRADE_NOTE: MainClass was upgraded to MainClass_Renamed. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A9E4979A-37FA-4718-9994-97DD76ED70A7"'
	Private Function GetMaxPartyCretificateNo(ByRef pPartyName As String, ByRef pQTR As String) As String
		Dim MainClass_Renamed As Object
		On Error GoTo ErrPart
		Dim SqlStr As String
		...
		If RsMaxPartyCNO.RecordCount > 0 Then
			...
			If IsDbNull(RsMaxPartyCNO.Fields("CNo").Value) = True Then
				'            GetMaxPartyCretificateNo = Format(1, "000000")
				If RsCompany.Fields("FYEAR").Value < 2010 Then
					GetMaxPartyCretificateNo = VB6.Format(1, "000000")
				Else
					GetMaxPartyCretificateNo = VB6.Format(RsCompany.Fields("FYEAR").Value, "00") & "-" & VB6.Format(RsCompany.Fields("FYEAR").Value + 1, "00") & "/" & pQTR & "/" & VB6.Format(1, "000000")
				End If
			Else
				If RsCompany.Fields("FYEAR").Value < 2010 Then
					GetMaxPartyCretificateNo = VB6.Format(RsMaxPartyCNO.Fields("CNo").Value, "000000")
				Else
					GetMaxPartyCretificateNo = VB6.Format(RsCompany.Fields("FYEAR").Value, "00") & "-" & VB6.Format(RsCompany.Fields("FYEAR").Value + 1, "00") & "/" & pQTR & "/" & VB6.Format(RsMaxPartyCNO.Fields("CNo").Value, "000000")
				End If
			End If
		Else
			If RsCompany.Fields("FYEAR").Value < 2010 Then
...
ErrPart: 
	...
	'UPGRADE_NOTE: MainClass was upgraded to MainClass_Renamed. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A9E4979A-37FA-4718-9994-97DD76ED70A7"'
	Private Function GetMaxCretificateNo(ByRef pQTR As String) As Integer
		Dim MainClass_Renamed As Object
		On Error GoTo ErrPart
		Dim SqlStr As String
		Dim RsMax As ADODB.Recordset
		
		SqlStr = "Select MAX(CERTIFICATENO) as CNo from TCS_TRN WHERE " & vbCrLf & " COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & " " & vbCrLf & " AND FYEAR=" & RsCompany.Fields("FYEAR").Value & " "
