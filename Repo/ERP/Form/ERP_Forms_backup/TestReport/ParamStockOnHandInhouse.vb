...
Friend Class frmParamStockOnHandInhouse
    ...
    Private Const ColThickness As Short = 9
    Private Const ColColor As Short = 10
    Private Const ColMinQty As Short = 11
    Private Const ColMaxQty As Short = 12

    Private Const ColOpening As Short = 13
    Private Const ColOpeningValue As Short = 14
    Private Const ColMRRQty As Short = 15
    Private Const ColMRRValue As Short = 16
    Private Const ColReceipt As Short = 17
    Private Const ColReceiptValue As Short = 18
    Private Const ColIssue As Short = 19
    Private Const ColIssueValue As Short = 20
    Private Const ColClosing As Short = 21
    Private Const ColRejection As Short = 22
    Private Const ColUnderQC As Short = 23
    Private Const ColDeptStock As Short = 24
    Private Const ColTotalClosing As Short = 25
    Private Const ColRate As Short = 26
    Private Const ColValue As Short = 27
    Private Const ColConsumptionQty As Short = 28
    Private Const ColConsumptionValue As Short = 29
    Private Const ColRGPQty As Short = 30
    Private Const ColRGPValue As Short = 31
    Private Const ColAssetsValue As Short = 32
...
ERR1:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For I = ColMinQty To ColAssetsValue
                .Col = I
    ...
    Private Function Show1() As Boolean
        ...
        If optType(0).Checked = True Then
            SqlStr = " SELECT '', GROUP_NAME, CATEGORY_NAME, SUBCATEGORY_CODE, SUBCATEGORY_NAME, " & vbCrLf _
                    & " ITEM_CODE,ITEM_NAME,ITEM_UOM, MAT_THICHNESS, ITEM_COLOR, SUM(MINIMUM_QTY), SUM(MAXIMUM_QTY), " & vbCrLf _
                    & " SUM(OPENING), SUM(OPENING_VALUE), SUM(MRR_QTY), SUM(MRR_VALUE), SUM(RECEIPT), SUM(RECEIPT_VALUE),SUM(ISSUE),SUM(ISSUE_VALUE),SUM(CLOSING)," & vbCrLf _
                    & " SUM(REJ_QTY),SUM(UNDERQC_QTY),SUM(DEPT_QTY),SUM(TOTAL_QTY)," & vbCrLf _
                    & " SUM(DECODE(TOTAL_QTY,0,RATE,VALUE/TOTAL_QTY)),SUM(VALUE), SUM(CONSUMPTION_QTY),SUM(CONSUMPTION_VALUE), " & vbCrLf _
                    & " SUM(OTHER_ISSUE_QTY), SUM(OTHER_ISSUE_VALUE), SUM(ASSETS_VALUE),MAX(LAST_TRANS_DATE),LEDGER_HEAD," & vbCrLf _
                    ...
                    & " GROUP BY GROUP_NAME, CATEGORY_NAME,SUBCATEGORY_CODE,SUBCATEGORY_NAME,ITEM_CODE,ITEM_NAME,ITEM_UOM,ITEM_UOM, MAT_THICHNESS,ITEM_COLOR,LEDGER_HEAD"
        Else
            SqlStr = " SELECT '', GROUP_NAME, CATEGORY_NAME,'', ''," & vbCrLf _
                    & " '','','', '','','',''," & vbCrLf _
                    & " SUM(OPENING),SUM(OPENING_VALUE), SUM(MRR_QTY), SUM(MRR_VALUE),SUM(RECEIPT),SUM(ISSUE),SUM(RECEIPT_VALUE),SUM(ISSUE_VALUE),SUM(CLOSING)," & vbCrLf _
                    & " SUM(REJ_QTY),SUM(UNDERQC_QTY),SUM(DEPT_QTY),SUM(TOTAL_QTY)," & vbCrLf _
                    ...
                    & " WHERE USERID ='" & MainClass.AllowSingleQuote(UCase(PubUserID)) & "'" & vbCrLf _
                    & " GROUP BY GROUP_NAME, CATEGORY_NAME,LEDGER_HEAD "
        End If
...
InsertErr:
        ...
        ''Resume
    End Function
    Private Sub frmParamStockOnHandInhouse_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
...
ERR1:
    ...
    Private Sub frmParamStockOnHandInhouse_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
err_Renamed:
    ...
    Private Sub lstCompanyName_ItemCheck(sender As Object, e As ItemCheckEventArgs) Handles lstCompanyName.ItemCheck
        ...
        End Try
    End Sub
    Private Sub frmParamStockOnHandInhouse_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        On Error GoTo ErrPart
...
ErrPart:
    ...
    Private Sub frmParamStockOnHandInhouse_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
