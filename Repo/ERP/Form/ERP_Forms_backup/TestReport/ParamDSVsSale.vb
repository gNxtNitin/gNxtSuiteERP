...
Friend Class frmParamDSVsSale
    ...
    Private Const ColTotalQty As Short = 8
    Private Const ColTotalPer As Short = 9
        ...
        With SprdMain
            For cntRow = 1 To .MaxRows
                ...
                If Trim(.Text) = "P" Then
                    ...
                    For cntCol = ColTotalQty + 1 To .MaxCols
                        .Col = cntCol
...
ErrPart:
    ...
    Private Sub ReportonPO(ByRef Mode As Crystal.DestinationConstants)
        ...
        mTitle = "Schedule Vs Sale for the month of " & VB6.Format(lblNewDate.Text, "MMMM , YYYY")
        mSubTitle = ""
        If OptShow(0).Checked = True Then
            Report1.ReportFileName = My.Application.Info.DirectoryPath & "\reports\ScheduleVsSale.rpt"
        Else
            Report1.ReportFileName = My.Application.Info.DirectoryPath & "\reports\ScheduleVsSaleWeek.rpt"
        End If
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamDSVsSale_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        On Error GoTo ERR1
        If FormActive = True Then Exit Sub

        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor

        Me.Text = "Schedule Vs Sale Register"
        System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
        FormActive = True
        Exit Sub
ERR1:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamDSVsSale_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
BSLError:
    ...
    Private Sub frmParamDSVsSale_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
...
ErrPart:
        ErrorMsg(Err.Description, CStr(Err.Number), MsgBoxStyle.Critical)
    End Sub
    Private Sub frmParamDSVsSale_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        FormActive = False
        Me.Hide()
        Me.Close()
    End Sub
...
EventExitSub:
    ...
    Private Sub FormatSprdMain(ByRef Arow As Integer)
        ...
        With SprdMain
            ...
            For cntCol = ColTotalQty To ColMaxCol
                .Col = cntCol
                .CellType = SS_CELL_TYPE_FLOAT
                .TypeFloatDecimalPlaces = 0
                .TypeFloatMin = CDbl("-99999999999")
...
LedgError:
    ...
    Private Function MakeSQL() As String
        ...
        MakeSQL = " SELECT " & vbCrLf _
            ...
            & " TRIM(ID.ITEM_CODE), INVMST.ITEM_SHORT_DESC, 'P' AS PLAN, "

        MakeSQL = MakeSQL & vbCrLf & " TO_CHAR(SUM(PLANNED_QTY)) AS PLANNED_QTY, '' AS SCHEDULE_PER,"

        If OptShow(0).Checked = True Then
            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='01' THEN PLANNED_QTY ELSE 0 END)) AS DAY1," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='02' THEN PLANNED_QTY ELSE 0 END)) AS DAY2," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='03' THEN PLANNED_QTY ELSE 0 END)) AS DAY3," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='04' THEN PLANNED_QTY ELSE 0 END)) AS DAY4," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='05' THEN PLANNED_QTY ELSE 0 END)) AS DAY5," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='06' THEN PLANNED_QTY ELSE 0 END)) AS DAY6," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='07' THEN PLANNED_QTY ELSE 0 END)) AS DAY7,"

            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='08' THEN PLANNED_QTY ELSE 0 END)) AS DAY8," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='09' THEN PLANNED_QTY ELSE 0 END)) AS DAY9," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='10' THEN PLANNED_QTY ELSE 0 END)) AS DAY10," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='11' THEN PLANNED_QTY ELSE 0 END)) AS DAY11," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='12' THEN PLANNED_QTY ELSE 0 END)) AS DAY12," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='13' THEN PLANNED_QTY ELSE 0 END)) AS DAY13," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='14' THEN PLANNED_QTY ELSE 0 END)) AS DAY14,"

            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='15' THEN PLANNED_QTY ELSE 0 END)) AS DAY15," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='16' THEN PLANNED_QTY ELSE 0 END)) AS DAY16," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='17' THEN PLANNED_QTY ELSE 0 END)) AS DAY17," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='18' THEN PLANNED_QTY ELSE 0 END)) AS DAY18," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='19' THEN PLANNED_QTY ELSE 0 END)) AS DAY19," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='20' THEN PLANNED_QTY ELSE 0 END)) AS DAY20," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='21' THEN PLANNED_QTY ELSE 0 END)) AS DAY21,"

            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='22' THEN PLANNED_QTY ELSE 0 END)) AS DAY22," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='23' THEN PLANNED_QTY ELSE 0 END)) AS DAY23," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='24' THEN PLANNED_QTY ELSE 0 END)) AS DAY24," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='25' THEN PLANNED_QTY ELSE 0 END)) AS DAY25," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='26' THEN PLANNED_QTY ELSE 0 END)) AS DAY26," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='27' THEN PLANNED_QTY ELSE 0 END)) AS DAY27," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='28' THEN PLANNED_QTY ELSE 0 END)) AS DAY28,"

            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='29' THEN PLANNED_QTY ELSE 0 END)) AS DAY29," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='30' THEN PLANNED_QTY ELSE 0 END)) AS DAY30," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD')='31' THEN PLANNED_QTY ELSE 0 END)) AS DAY31"

        Else
            MakeSQL = MakeSQL & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD') <='07' THEN PLANNED_QTY ELSE 0 END)) AS Week1," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD') >'07' AND TO_CHAR(SERIAL_DATE,'DD') <='14' THEN PLANNED_QTY ELSE 0 END)) AS Week2," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD') >'14' AND TO_CHAR(SERIAL_DATE,'DD') <='21' THEN PLANNED_QTY ELSE 0 END)) AS Week3," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD') >'21' AND TO_CHAR(SERIAL_DATE,'DD') <='28' THEN PLANNED_QTY ELSE 0 END)) AS Week4," & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(SERIAL_DATE,'DD') >'28' THEN PLANNED_QTY ELSE 0 END)) AS Week5"
        End If
        ...
        MakeSQL = MakeSQL & vbCrLf _
            & " AND IH.COMPANY_CODE=" & RsCompany.Fields("COMPANY_CODE").Value & "" ''& vbCrLf |            & " AND SUBSTR(IH.AUTO_KEY_DELV,LENGTH(IH.AUTO_KEY_DELV)-5,4)=" & RsCompany.fields("FYEAR").value & ""
        ...
        MakeSQL = MakeSQL & vbCrLf _
            & " AND TO_CHAR(IH.SCHLD_DATE,'MON-YYYY')='" & UCase(VB6.Format(lblNewDate.Text, "MMM-YYYY")) & "'"
...
FillErr2:
    ...
    Private Sub FillGridHeader()
        ...
        With SprdMain
            I = 1
            For cntCol = ColTotalPer + 1 To ColMaxCol
                .Row = 0
                .Col = cntCol
...
ErrPart:
    ...
    Private Function GetRecdQty(ByRef pSupplierCode As String, ByRef pItemCode As String, ByRef pDateSerial As String) As Double
        ...
        If OptShow(0).Checked = True Then
            SqlStr = SqlStr & vbCrLf _
                & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD')='" & pDateSerial & "' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
        Else
            If pDateSerial = "01" Then
                SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD') <='07' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
            ElseIf pDateSerial = "02" Then
                SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD') >'07' AND TO_CHAR(IH.INVOICE_DATE,'DD') <='14' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
            ElseIf pDateSerial = "03" Then
                SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD') >'14' AND TO_CHAR(IH.INVOICE_DATE,'DD') <='21' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
            ElseIf pDateSerial = "04" Then
                SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD') >'21' AND TO_CHAR(IH.INVOICE_DATE,'DD') <='28' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
            ElseIf pDateSerial = "05" Then
                SqlStr = SqlStr & vbCrLf & " TO_CHAR(SUM(CASE WHEN TO_CHAR(IH.INVOICE_DATE,'DD') >'28' THEN ITEM_QTY ELSE 0 END)) AS TOTQTY"
            End If
        End If
        ...
        SqlStr = SqlStr & vbCrLf & " FROM FIN_INVOICE_HDR IH,FIN_INVOICE_DET ID" & vbCrLf _
            ...
            & " AND ID.ITEM_CODE='" & MainClass.AllowSingleQuote(pItemCode) & "' AND IH.CANCELLED='N'"


        'SqlStr = SqlStr & vbCrLf & " AND IH.REF_TYPE ='P' "

        SqlStr = SqlStr & vbCrLf & " AND TO_CHAR(IH.INVOICE_DATE,'MON-YYYY')='" & UCase(VB6.Format(lblNewDate.Text, "MMM-YYYY")) & "'"
...
ErrPart:
    ...
    Private Function FillRecdQty(ByRef pRow As Integer, ByRef pSupplierCode As String, ByRef pItemCode As String, ByRef mScheduleQty As Double) As Boolean
        ...
        Next
        I = I
        For I = ColTotalPer + 1 To ColMaxCol
            SprdMain.Row = pRow
            SprdMain.Col = I
        ...
        SqlStr = "SELECT IH.INVOICE_DATE, SUM(ITEM_QTY) AS TOTQTY"
        ...
        SqlStr = SqlStr & vbCrLf _
            & " AND TO_CHAR(IH.INVOICE_DATE,'MON-YYYY')='" & UCase(VB6.Format(lblNewDate.Text, "MMM-YYYY")) & "'"

        SqlStr = SqlStr & vbCrLf & " GROUP BY IH.INVOICE_DATE "
        SqlStr = SqlStr & vbCrLf & " ORDER BY IH.INVOICE_DATE "


        MainClass.UOpenRecordSet(SqlStr, PubDBCn, ADODB.CursorTypeEnum.adOpenStatic, RsMRR, ADODB.LockTypeEnum.adLockReadOnly)
        If Not RsMRR.EOF Then
            Do While Not RsMRR.EOF
                mMRRDATE = IIf(IsDBNull(RsMRR.Fields("INVOICE_DATE").Value), "", RsMRR.Fields("INVOICE_DATE").Value)
                If OptShow(0).Checked = True Then
